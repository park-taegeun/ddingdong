...
- **(iii) 동일 위치 대조 원칙** — **미등재 + §9 사용자 판단 요청**. 근거 = 등재 쪽(측정 결정에 직접 영향 + 9.3(H) "측정 환경 유효성" 층과 동일 계열) vs 미등재 쪽(9.3(e)/(H)가 이미 환경 통제를 다루므로 중복 등재 위험, 배치처도 9.3 확장 / 신설 카테고리로 갈림)이 **팽팽해 임의 결정하지 않음**.
