...
## 2026-09-19 (토) — PoC-(53) Set 1 후속 — 🔴 원천 귀속 감사 등재(33.7(h) 신설 · (f) 부분 해소 2 · (i) 조각내기 코드 부재 신규 미결) + 5.2(f) 논증 해소 + NCP 감시 기준점 09-18(30.9) + PR #65 정합성 정정 4세트 등재(6.2 · 6.3(k-2) · 27.8(m)) + `jsonpeek` 컴파일 명령 부재(6.3(o)) + 검증 자산 베이스라인 신설(17.2) + PR #66 FAQ 카피 정정 등재 · 같은 계열 미정정 2건(8.7(a)) + MCP 격리 워크트리 관측(카테고리 20 · 21) + 회차 라벨 오기 정정(33.9 · 27.8(o) 신설) (카테고리 5.2/6.2/6.3/8.4/8.7/17/20/21/27.8/30.9/33.7/33.9)

2026-09-18 저녁~2026-09-19 노트북 단독 작업(보드 무접촉)의 산출을 등재했다. 🔴 **회차 = PoC-(53)이다** — 2026-09-18 Set 1(`c48779a`)이 이미 PoC-(53)으로 등재했고 채팅방이 갈리지 않아 **같은 회차가 이틀에 걸쳤다**. 2026-09-19 세션의 MCP 보고서·위임과 PR #65 `4e9e942`에 박힌 「(54)」는 **인계 패키지 제목의 다음 회차 예고를 현재 회차로 오독한 라벨 오기**이며, SSoT 유입분 1건(33.9)을 본 Set에서 정정했다(27.8(o)①). 축은 **다섯** — ① **데이터셋 원천 귀속 감사**(33.7(h) 신설 · (f) 부분 해소 2건 · (g) 논증 입력 실측 · (i) 신규 미결 · 5.2(f) 해소) ② **외부 과금 감시**(30.9 09-18 기준점) ③ **PR #65 · #66 등재**(6.2 · 6.3(k-2) · 27.8(m) · 33.9 · 8.7(a) · 8.4(f)) ④ **검증 자산 베이스라인**(17.2 신설 · 6.3(o) 보강) ⑤ **작업 방식 관측**(카테고리 20 워크트리 · 카테고리 21 프로세스). 문서 2파일만 수정, **코드 0줄 · 보드 무접촉 · 노션 무접촉**, main 직 push.

**Step 0 가정 대조 — 9항 중 불일치 2건(정지 임계 3건 미달, 방향 = 둘 다 ① 위임 미완성)**: 착수 시점 HEAD **`765624b`** · `main` · 워킹트리 **`?? .claude/` 잔존(정상 — 등재 대상)** · `git worktree list` = 본 체크아웃 + `feat+dashboard-faq-copy` `e1df298` **locked** · 원격 `refs/heads/main` 단일(`git ls-remote`) · 열린 PR 0. `git log -4` = `765624b` / `601e7a2` / `c48779a` / `d093d70` 일치. §4 12건의 대상 절 문구 **전건 실존**(33.7(f) 「두 클래스 동시 배치 경위 미실증」·「`00_source_raw` 미감사」 / 33.7(g) 「선두 약 9초 구간만 공통」 [논증] / 5.2(f) 「[논증 — 미실증]」 / 30.9 감시 기준점 블록 09-17 / 6.2 G29 stale 등재 / 27.8(m)②④ / 6.3(k-2) 「범위 밖」 / 6.3(o) 「본문 복사」 / 8.7(a) 「처리 방침 미확정」 / 카테고리 10 규약 이탈 2건 미결 / 카테고리 21 「계통 원인의 신호」). 33.9 · 카테고리 1 머리는 **`601e7a2`로 이미 정정돼 있음을 확인**해 재정정하지 않았다(PR #65 기록만). 내부 커밋 5개(`4e9e942` `fe2b9f9` `a918173` `8200669` `e1df298`) **전건 `git log -1` 실존**.
- **불일치 ① — SSoT에 이미 (54)가 있었다**: 위임 §2는 (54)를 「2026-09-19 세션의 MCP 보고서·위임」의 오기로만 적었으나, **「(54)」 라벨 grep = `docs/` 1건**(33.9, PR #65 `4e9e942` 2026-09-18 21:23 유입). ⇒ 위임의 「SSoT에 (54)를 쓰지 말 것」을 지키려면 **기존 1건 정정이 필요**했고, 6.3(n) C-0 라벨 정정 선례 형식으로 처리했다. 사후 「PoC-」 접두가 붙은 (54) 라벨 grep = 두 파일 **0 / 0**.
- **불일치 ② — 8.7(a)에 위임이 「틀렸다」고 한 문구가 그대로 있었다**: 위임 ⑪은 「`fire_alarm`은 ToF 우회라 무조건 도착 약속이 정확하다」를 **PR #66 위임 작성자 오류**로만 적었으나, **8.7(a) 본문이 같은 문구**(「무조건 알림 약속이 정확하다(G12 · 카테고리 3 정합)」)로 등재돼 있었다. `server/app/utils.py` 실독 = 신뢰도 게이트 docstring **「신뢰도 < 임계값 → 1차 알림 skip … 클래스 무관」** + `if confidence < CONFIDENCE_THRESHOLD` 분기가 `_tof_decision` 앞에 클래스 조건 없이 위치 ⇒ **`fire_alarm`도 신뢰도 게이트는 받는다**. 8.7(a) 문구는 **PR #56 시점 판정의 이력**이라 취소선이 아니라 **경계 병기**로 처리했다(결론 「카피 무변경」은 옳다).
- **로그 요약줄 catch 1건(위임 §4 ⑩ 동일 문장)**: 「파일 수는 `.DS_Store` 포함이라 SSoT의 2792/9655/12447과 1씩 다르다」 — `05_final_dataset`은 **12447 = 12447로 일치**하고 1씩 다른 것은 `02` · `03`뿐이다. 17.2에 그대로 갈라 적었다.

**로그 원본 접근 — 3 / 3 전건 읽혔다**: `~/ddingdong-측정결과/2026-09-18/ncp_console_0918_check.log` **35줄** · `2026-09-18/source_raw_provenance_audit.log` **93줄** · `2026-09-19/baseline_601e7a2.log` **101줄**(계수 단위 = 줄 수). ⇒ 본문 수치는 **위임 인용이 아니라 로그 원본에서 직접** 대조했고, 위임 §4와 로그가 어긋난 수치는 **0건**(위 `05` 12447 건은 로그 요약줄 자체의 표현 문제).

**find-skills**: `"markdown section editing"` **0건** / `"technical decision log"` **0건**(2026-09-18과 동일 — 레지스트리 변동 없음). 같은 배치가 전건 0이라 대조 질의 `"markdown"` **315건**(도구 생존 확인). **채택 0건 · 미채택 사유 = ③ 0건**(①②는 해당 없음) · install 0 · info 0. 위임 지정 재실행 금지 항목(ADR 신규 파일류 · changelog 자동 생성류 등)은 재실행하지 않았다.

### 신설 절 번호 결정 근거 (위임 미지정 — MCP 판정, 근거유형 = 실측 grep)

- **33.7(h) · (i)** — 33.7의 하위 기호 체계 = 문자이고 마지막이 **(g)**이므로 (h)(i). ★ 원천 귀속 감사(①②③)를 **한 항 (h)**에 모은 근거 = 위임이 「①과 같은 절에」로 지정했고 (g)가 같은 방식(하나의 감사 = 하나의 항)이다. ★ 조각내기 코드 부재(④)를 **별도 (i)**로 뺀 근거 = **신규 미결**이라 찾기 쉬워야 하고, (h)의 한계 ⑥이 그것을 가리킨다.
- **17.2** — 카테고리 17 「별도 검증 항목」의 마지막이 **17.1**(2026-05 정적 budget 검증)이므로 **17.2**. ★ 카테고리 20(무번호) · 32(PoC-(17) 부팅 검증 전용)를 택하지 않은 근거 = 17.1이 **RAM/Flash 정적 검증 기록**이라 본 베이스라인(13 env RAM%·Flash% 포함)과 **같은 성격의 선행 절**이고, 카테고리 17은 **번호 체계가 있어 무번호 선례 미적용**. 16.1 RAM/Flash 표는 2026-05 이력이라 무변경 + 「관련」 포인터.
- **27.8(o)** — 27.8의 마지막이 **(n)**이므로 **(o)**. 하위 5건은 ① ② ③ ④ ⑤. ★ 별도 항으로 만든 근거 = (n)이 「PoC-(53) 오류 4건」으로 **1일차 전용**이고, 본 5건은 **2일차(09-18 저녁~09-19)** 발견분이라 날짜 축이 다르다.
- **카테고리 20 무번호 `###`** — 워크트리 관측. 카테고리 20은 **무번호 `###` 체계**(직전 선례 = 「NC 변형 적용 증명에 바이너리 md5를 쓸 수 없다」). 위치 = 「5/17 종료 시점 액션」 **앞**(기존 신설 절들과 같은 위치).
- **카테고리 21 프로세스 위생 절 append** — `ps` 21건은 기존 절의 「계통 원인의 신호」 경계 표기와 **같은 자리**라 신설하지 않았다.
- **30.9 감시 기준점 블록 append** — 09-17 블록 「한계」 줄 뒤에 **09-18 갱신 불릿**. 「PR #62 이후 감시 기준점 = 0」 문장은 **취소선 없이 유지**(09-18의 45초는 「0 외 추가분 = 우리가 낸 호출만」의 실증이지 기준점 변경이 아니다).

### 편집 건별 (전건 `str.count` == 1 assert 통과, 라인 번호 0건 사용, 물리 편집 지점 23곳)

| E# | 앵커 (실물 문구, 앞부분) | 방식 | 반영 위치 | 근거유형 |
|---|---|---|---|---|
| E1 | 「🔴 **잔존** = `00_source_raw` **미감사**(41,450파일) / …」 | **부분 해소 2 append**(취소선 없음 — 전수 미감사 · 파형 · 부분 중복 잔존) | 33.7(f) | 실측 + 미확정 |
| E2 | 「🔴 **`01_clips` 이전(수집·원천) 어디에서 발생했는지는 여전히 미규명**이다 …」 | **취소선 + ✅ 해소** | 33.7(f) | 실측 + 논증 |
| E3 | 「⇒ **발생 지점이 `00_source_raw` 원천 데이터셋일 가능성**이 있으나 …」 | **취소선(미확정 절) + 입력 실측 확인** — 논증 승격 아님을 명시 | 33.7(g) | 실측(입력) |
| E4 | 「**관련**: 33.2(test 성적 0.887 …」 앞 | **(h)(i) 신설** + 「관련」 줄 5.2(a)(f) 포인터 append | 33.7 | 항목별 병기 |
| E5 | 「**(f) 🔵 [논증 — 미실증] `01_extracted` 짝 없는 …」 + 「🔴 **같은 파일인지는 미확인**이다 …」 | **취소선 2쌍 + ✅ 해소**(헤더 라벨 + 논증 절) | 5.2(f) | 실측 |
| E6 | 「- **한계**: 09-17은 **하루가 끝나지 않은 값** / …」 | **09-18 기준점 불릿 append** | 30.9 | 실측 툴팁 |
| E7 | 「⚠️ **본 Set은 코드 무접촉**이므로 **정정 대상이라는 사실만 기록**하며 …」 | **취소선 + ✅ 해소(PR #65 세트 A)** | 6.2 | 실측 `git show` |
| E8 | 「⚠️ **코드 무접촉이라 주석 수정은 별도 PR 소관**이고 …」 | **취소선 + ✅ 해소(PR #65 세트 B)** | 27.8(m)② | 실측 |
| E9 | 「⇒ 그 문장을 그대로 인용하면 오독이 생긴다. **코드 무접촉이라 사실만 기록**한다.」 | **취소선 + ✅ 해소(PR #65 세트 C)** | 27.8(m)④ | 실측 |
| E10 | 「- ⚠️ **본 (m)은 (d)의 「인용 오기 누적」 숫자에 합산하지 않는다** …」 | **PR #65 요약 불릿 append**(4세트 · 커밋 2개 1PR · 검증) | 27.8(m) 말미 | 실측 |
| E11 | 「⚠️ **mic_common.h 주석 정정은 본 Set 범위 밖 …」 | **취소선 + ✅ 해소 + 드리프트 재발 실증**(`a918173` + `8200669`) | 6.3(k-2) | 실측 grep |
| E12 | 「**해소책은 등재하지 않는다**(코드 변경 소관).」 | **컴파일 명령 부재 보강 append** | 6.3(o) | 실측 + 논증 |
| E13 | 「- **fire_alarm 카피 무변경**: `fire_alarm_bypass`로 ToF를 **우회**하므로 …」 | **근거 문구 경계 병기**(취소선 없음 — 이력 판정) | 8.7(a) | 실측 코드 대조 |
| E14 | 「- ⚠️ **미수정으로 남긴 것 (사용자 판단 대기)**: 같은 파일 FAQ 1번 …」 | **취소선 + ✅ 해소(PR #66)** + ssr-nc 도달 불가 판정 + 🟡 신규 미결 2건 + 인접 판정 1건 | 8.7(a) | 실측 + 사용자 결정 |
| E15 | 「⚠️ **8.7(c) `twMerge` 토큰 소실도 본 하네스로는 도달 불가**다 …」 | `HelpPage` 도달 불가 **포인터 append** | 8.4(f)④ | 실측 |
| E16 | 「⚠️ **[정정 2026-09-18 …(54)…] 위 문구는 …」 + 「⇒ **미결 상태는 무변경**이며 본 정정은 클래스 수 오기만 되돌린다.」 | **회차 라벨 교체(54 → 53) + PR #65 커밋 병기 + 정정 경위 append**(클래스 수 본문 무변경) | 33.9 | 실측 |
| E17 | 「⇒ **(d)의 「누적 21건」은 (i) · (j) · (k) · (l) · (m) · 본 (n) …」 | **(o) 신설 — 5건** | 27.8 | 실측 + 실물 대조 |
| E18 | 「## 카테고리 18: 채팅방 운영 구조」 앞 | **17.2 신설** | 카테고리 17 | 실측 전건 |
| E19 | 「### 5/17 종료 시점 액션」 앞 | **무번호 절 신설**(관측만, 판정 없음) | 카테고리 20 | 실측 |
| E20 | 「발견 = 2026-08-08 PoC-(35) 세션 / 문서 반영 = 2026-08-12 PoC-(36) …」 | **`ps` 21건 점검 기록 append** + 경계 | 카테고리 21 | 실측 `ps` |

### 등재 내용 요약

- **33.7(h) 신설** — 대상 특정(15그룹 → `source_key` 역추적 → 원천 178키, **180파일 / 71,417,755 B만 md5**, 39G 전수 미수행) / (a)(g) **전건 재현** / 원천 귀속 **[실측]** G01~G09 171키 = `aihub_alarm` mp3 171 / 171 · G10 5키 = `audioset` wav 5 / 5 · G11~G15 2키 = `audioset` 양쪽 폴더 · **귀속 불가 0** · **`00` ↔ `01_clips` 매핑 테이블 없음** / 🔴 **핵심 판정 [실측 + 논증]** = **15 / 15 그룹의 동일성이 `00_source_raw` 바이트에 이미 존재, 우리 단계가 새로 만든 동일성 0 / 15**(표: 전체 동일 5그룹 / 4파일 · 선두 구간 동일 9그룹 / 171파일 · 창 무음 1그룹 / 5파일, 계수 단위 = 그룹 수 / 원천 파일 수) — 「동일 입력 → 동일 출력」은 **디코딩 결정성에 기댄 논증**임을 분리 표기 / **선두 구간 공통 [바이트 = 실측 / 초 = 산술]** G01 160,083~160,085 B(≈10.0초) · G04 142,944~142,948 B(≈8.95초) · G07 154,646~155,386 B(≈9.7초) · 교차 3대표 21,201 B(≈1.3초), **초 환산 = MPEG-1 Layer III · 128 kbps · 44.1 kHz · 417 B/프레임 헤더 산술** / 🔴 **(g) 논증이 실측으로 승격된 것이 아니라 그 논증의 입력이 원천에서 확인된 것** / G10 무음 = 원천 창이 이미 전 바이트 0(5 / 5), 파일 전체는 무음 아님 / G11~G15 = 양쪽 폴더 md5 동일 + csv 교집합 정확히 2세그먼트(대조군 = 나머지 csv 쌍 0), **원본 다중 라벨 vs 우리 필터링 미확정** / 한계 8건 전건 / 🔴 **(e) 대응 방향 사용자 판단 대기 유지**.
- **33.7(f) 부분 해소 2 + (g) 입력 실측** — 잔존 4항 중 「`00_source_raw` 미감사」는 **대상 180파일만**으로 좁혀지고(전수 미감사 유지) 「두 클래스 동시 배치 경위」는 **dump 내 경위만** 닫혔다(취소선 없음). 두 번째 잔존 「`01_clips` 이전 어디서 발생했는지 미규명」은 **취소선 + ✅ 해소**(발생 지점 = 원천, AI Hub 측 경위는 미확정). (g)의 「`00_source_raw` 미감사라 미확정 · 확정하지 말 것」 절만 취소선.
- **33.7(i) 🔴 신규 미결** — 조각내기 코드 repo 부재(대조군 = `ml/pipeline/` 5스테이지 실존 + README·config 「01_clips가 입력」 명시) / **「없다」≠「잃었다」**(타 폴더 미확인) / 파급 = 재발 방지 지점 코드 특정 불가 + **(e) 대응 ①을 고르면 `01_clips` 수작업뿐** / 5.2(a)와 같은 축 / **처리 방침 미확정**.
- **5.2(f) ✅ 해소** — `01_extracted` 0바이트 30개(doorbell 10 / knock 20) = `01_clips` 짝 없는 30개, **양방향 차집합 0 / 0** / 귀속 실측은 범위 밖(이름만) / 🆕 mp3 171 이름 집합 양방향 차집합 0, **바이트 동일성 미판정**.
- **30.9 09-18 기준점** — **3 / 0 / 45**(툴팁, 18:35 KST) = 예상값 정확히 일치(PR #64 ④런타임 #2·#3·#4 × 15초, #1은 `gate=skip`) / `usage = success × 15` / **설명 필요 신호 0건** / 한계 3건 + 09-11~09-17 그래프 판독값은 **근거 등급이 다르다**고 병기.
- **PR #65 등재(6.2 · 27.8(m)②④ · 6.3(k-2) · 33.9)** — 세트 A~D + 커밋 2개 1PR(`4e9e942` 📝 / `fe2b9f9` 🗃️, 분리 근거 = `git diff c48779a 4e9e942 -- server/ firmware/ ml/` 빈 출력) + AST 해시 · 전처리 md5 · 회귀 106 · `mic_dummy` SUCCESS. **세트 A**는 750ms 미기재(PR-C 소관) 명기. **6.3(k-2)** = `a918173`(`mic_common.h`) + **`8200669`(`mic_test.cpp` 잔여 1건 — 「한쪽만 고쳐 드리프트를 남겼다」 실증, 27.8(m)⑤ 재발)**, 전처리 md5 `1ccfe3f8…` + 대조군 갈림 `03e08322…`, **바이너리 md5 미사용**. **33.9**는 클래스 수 본문 **무변경**(재정정 금지 준수), 회차 라벨만 (54) → (53).
- **6.3(o) 보강** — `jsonpeek_test.cpp`만 파일 머리에 **컴파일 명령 없음**(대조군 = 나머지 5종 verbatim 보유) / 베이스라인 명령 `c++ -std=c++17 -Wall -o /tmp/jpt …`는 **유도값(논증)** / 「본문 복사 → NC 도달 불가」 축에 **「컴파일 명령 부재」 추가** / 해소책 미등재.
- **17.2 신설** — 실패 0 / SSoT 불일치 0 / 데이터셋 무변경. 서버 `Ran 106 tests in 0.302s` + 소켓 가드 3개 교체 · `NetworkGuardSelfTest` 2건 / 대시보드 4종 첫 기록(lint 0/1.63s · build 0/1.87s/2460 모듈 · ssr-nc 0/1.71s NC 4종, **tsc 단독 키 없음 — scripts 5키**) / 호스트 6종 첫 동시 실행 90·97·46·61·196 + `jsonpeek` 5 groups(assert 13 — **단위 다름**) / firmware **13 / 13 SUCCESS** RAM%·Flash% 13건 전기 / ml fail-fast `ValueError` + dtw 19 + export smoke + pipeline 5/5 + training 1/1(**§9 정지 → 사용자 확정 후 실행**, 판정 근거 = 명시 인자 > env) / 🔴 데이터셋 mtime **Jul 1 그대로**(files 2793 / 9656 / 12447 — `02`·`03`만 `.DS_Store` +1, `05` 일치) / 🔴 **경계 2건**(빌드 SUCCESS ≠ 동작 · 증분 빌드만) / 한계 9건 전건 / **학습 21 「첫 기록 ≠ 처음 돌림」**.
- **8.7(a) ✅ 해소 + 🟡 신규 미결 2건** — FAQ 1번 「알림이 갈 때는 …」(57자 → 57자, 최소 개입안, 3근거) / **ssr-nc 도달 불가 정식 판정**(`entry.tsx` 3종뿐 · `HelpPage` 0건 · `App.tsx` 미경유 = 8.4(f) 한계 ④, 하네스 무수정) / 🟡 **FAQ 2번**(`fire_alarm` 2차 없음) · **`SystemHealthSummaryCard`** 무조건 단언 — **처리 방침 미확정** / 인접 판정 `NotificationList.tsx` = 정정 대상 아님(단 목록 API skip 필터 미확인) / 커밋 Type 🐛 Fix 근거 / ⚠️ fire_alarm 「무조건」 근거 문구 **ToF 문맥 한정** 경계 병기.
- **카테고리 20 · 21 관측** — MCP 격리 워크트리 + 심볼릭 링크 방식 **최초 관측**(결과 정상), 잔재 = lock으로 `worktree remove` · `branch -D` 거부 + `?? .claude/`, 파급 = ssr-nc clean 요구, **좋다/나쁘다 판정 없음**, `.gitignore` 추가 여부 **사용자 판단 대기** / `ps` 21건 내역 = 데스크톱 8 + 부속 3 + 세션 9 + 미상 1, **좀비 아님**, 「전부 죽었다는 계통 원인 신호」는 **장애 시 항목이지 상시 점검값 아님**.
- **27.8(o) 신설 — 5건** — ① 회차 라벨 (54) 오기(인계 제목 → 위임·보고서 → SSoT 1건, C-0 동형) / ② 위임 3분법 불일치 → 제4범주(학습 16) / ③ 「단일 파일」 지정 → `mic_test.cpp` 드리프트 잔존 / ④ 「`run_all` 금지」 = 수단을 불변식으로(과잉 §9 정지) / ⑤ 「`fire_alarm` 무조건 도착 정확」 근거 범위 과대(**(m)④ · (n)① 계열 3회차**). **(d) 누적 숫자 무변경**.

### 🔴 사용자 판단 대기 항목 — 등재하되 방향 미확정 (본 엔트리 기준 전수 재확인)

| 항목 | 등재 위치 | 방향 확정 여부 |
|---|---|---|
| 33.7(e) 누수 대응 ①②③ | 33.7(e)(h)(i) | 🔄 **입력 추가 · 미확정 유지** — (h)는 입력만, (i)는 ①의 실행 제약을 적었을 뿐 |
| 🆕 조각내기 코드 부재 처리 | 33.7(i) | ⛔ **신규 미결 · 방침 미확정** — 해소책 0 |
| 🆕 FAQ 2번 · `SystemHealthSummaryCard` 무조건 단언 | 8.7(a) | ⛔ **신규 미결 · 방침 미확정** |
| 🆕 `.claude/` `.gitignore` 추가 · 워크트리 정리 | 카테고리 20 | ⛔ **관측만 · 방침 미확정** |
| G29 750ms 재계산 · PR-C(타임아웃·녹음 길이·해상도) | 6.2 · 7.7(l) | ⛔ **무접촉** — PR #65 세트 A가 상수값을 건드리지 않았음을 명기 |
| 5.2(g) · 카테고리 5 `SAMPLE_WEIGHT_RANGE` · 33.6(a)(f)(g) OOD · 33.6(e) round · 8.5(i) · 8.7(c) twMerge · 카테고리 10 이모지 2종 · 카테고리 21 `.env` 중복 · 27.8(d)(i) 누적 합산 | 각 절 | ⛔ **전건 무접촉** |

**SSoT 정합 검증 (문서 전용이라 코드 3단계 ①②는 무관)**

- **V1 취소선·볼드 파리티** — `decisions.md` 착수 전 **90줄 / 228개** → 종료 **98줄 / 246개**(계수 단위 = `grep -c` **줄 수** / `grep -o | wc -l` **출현 수**). 증가분 **+8줄 / +18개 = 신규 9쌍**이고 설계와 정확히 일치 — E2 · E3 · E5(2쌍) · E7 · E8 · E9 · E11 · E14. 줄 수가 8만 는 이유 = E2의 줄은 **이미 취소선이 있던 줄**(33.7(f) 「원인 미규명」 부분 해소 줄)이다. 신설 절(33.7(h)(i) · 17.2 · 27.8(o) · 카테고리 20 절)과 append 블록은 취소선 **0개**. **246 = 짝수 = 마크다운 무파손.** 볼드 마커 `**` = 10,008 → **10,674개(짝수)**. `decisions-log.md`는 취소선 마커 **0개 추가**(마커를 문자로 적지 않았다 — 2026-09-16 후속 세션 catch 준수).
- **V2 삭제 0줄 증명** — `git diff --numstat` = **+95 / −15**, 제거 15줄은 **전부 같은 줄의 치환 결과**. 제거 줄이 추가 줄에 부분 문자열로 남는지 기계 대조 = **5 / 15 통과, 10 / 15 미통과** — 미통과 10건은 전부 **취소선 마커 삽입으로 문자열이 갈라진 줄**(E2 · E3 · E5×2 · E7 · E8 · E9 · E11 · E14) + **라벨 교체 1건**(E16 33.9). 토큰 보존 = **정규화(마커 제거) 기준 손실 2토큰**(`일치한다**` → 5.2(f) 헤더에 「— 같은 집합으로 확정」을 볼드 안에 이어 붙임 / (54) 라벨 토큰 → 의도적 라벨 교체) · **원시 기준 손실 10토큰**(전부 취소선 마커 삽입으로 갈라진 토큰). ⇒ **의도치 않은 삭제 0**.
- **V3 편집 지점 실물 되읽기** — **물리 편집 지점 23곳** 전건 `count == 1` assert 통과(스크립트가 불일치 시 abort). **라인 번호 사용 0건**(편집 앵커 · 본 엔트리 본문 양쪽).
- **V4 🔴 한글 전수 검증** — 착수 전 파일에 **없던 음절 5개**(U+AC77 걷 / U+B141 녁 / U+B429 됩 / U+B72C 뜬 / U+B7AD 랭)를 코드포인트로 뽑아 문맥 확인 = 「걷어낸」 · 「저녁」 · 「표시됩니다」 · 「뜬다」 · 「블랭킹」 — **전건 정상 단어**. **신규 한글 단어 261개** 전건 문맥 출력 · 육안 통독 = 모지바케 0. 바이트 보강 = UTF-8 strict decode **통과** · `U+FFFD` **0건** · 제어문자 **0건** · NFC 정규화 동일 · 자모(U+1100~11FF · U+3130~318F) **1건 = 기존 8.4 「ㄴ안」(before/after 동일 위치, 본 Set 무관)**.
- **V5 수치 1:1 대조** — 33.7(h)(i) · 5.2(f) 수치는 `source_raw_provenance_audit.log`, 30.9 수치는 `ncp_console_0918_check.log`, 17.2 수치는 `baseline_601e7a2.log`와 **1:1 추적**. PR #65 · #66 수치는 `git show --stat 601e7a2 / 765624b`(5파일 +19 −9 / 1파일 +1 −1) · 내부 커밋 `git log -1`과 대조. **로그에 없는 수치 0건 기재.**
- **V6 git status** — `docs/` **2파일만**(`git status --short` = `M docs/decisions.md` · `M docs/decisions-log.md` · `?? .claude/` 미스테이징). 「PoC-」 접두 (54) 라벨 **0 / 0**(두 파일). 시크릿 스캔(SSID · 토큰 · 자격증명 · IP 패턴) **0건**.

**비범위**: **코드 0 수정** — `server/` · `dashboard/` · `ml/` · `firmware/` 전부 **읽기만**(`utils.py` 게이트 분기 실독 1회 — 산출 파일 0). **노션 무접촉**(Set 3 소관) · **프로젝트 지침 파일 무접촉**(Set 2 소관). 🔴 **미결 상태 전환은 위임 지정분만** — 5.2(f) · 6.2 G29 stale · 6.3(k-2) · 27.8(m)②④ · 8.7(a) FAQ 1번 = ✅ / 33.7(f) 2항 = 부분 · ✅ / 신규 미결 = 33.7(i) · 8.7(a) 2건 · 카테고리 20 관측(위임 지정). **33.9 클래스 수 · 카테고리 1 머리 재정정 0**(PR #65 기록만). **회귀 케이스 106 무변경**(서버 무접촉). **27.8(d) 누적 숫자 무변경.** `.claude/` 워크트리 **정리하지 않음**(등재 대상). **미착수** = 조각내기 코드 소재 확인 · `00_source_raw` 전수 md5 · FAQ 2번 · `SystemHealthSummaryCard` · PR-C · clean 빌드 관통 · 실모델 스윕.

**학습 적용**: **학습 13**(인용 전 grep — 편집 앵커 전건 `count == 1`, **라인 번호 0건**. 부재 판정은 마커를 걷어낸 부분 문자열로 재조회) / **학습 14 · 27**(위임이 「위임 작성자 오류」로만 적은 fire_alarm 문구를 **SSoT 8.7(a)와 `utils.py`로 직접** 확인 — 같은 문구가 SSoT에도 있었다) / **학습 17**(🔴 **위임 본문 catch 2건 + 로그 요약줄 catch 1건** — (54) SSoT 기유입 · 8.7(a) 동일 문구 · `05` 12447 일치. 전부 실물 채택 후 보고) / **학습 19**(위임의 3분법 · `run_all` 금지 · 단일 파일 지정을 그대로 등재하지 않고 **실물이 요구한 형태(제4범주 · 불변식 · 두 파일)를 27.8(o)에 기록**) / **학습 21**(🔴 **역방향 경계** — 33.9 · 카테고리 1 머리 · 33.7(f) 부분 해소 1이 **이미 처리돼 있음을 먼저 grep**해 재정정·재미결화하지 않았고, 반대로 33.7(f) 잔존 「`00_source_raw` 미감사」를 **180파일만 봤으므로 전수 해소로 날조하지 않았다**) / **학습 16 ★판정 규칙**(원천 귀속 3분법 → 제4범주 = 불변식 유지 + 메커니즘 등가 치환) / **「발견일 ≠ 반영일」**(①~⑥ = 2026-09-18 발견 · 2026-09-19 반영 / ⑦⑧ = PR #65 2026-09-19 머지(내부 커밋 09-18 21:23~09-19 11:07) / ⑨~⑫ = 2026-09-19 발견·반영) / **「실측 / 논증 / 문서 인용 3분」**(바이트 접두 · md5 · 툴팁 · 테스트 카운트 · mtime = **실측** / 초 환산 · 디코딩 결정성 · 산술 정합 · 「테스트 OK ≠ 외부 호출 0」 · `jsonpeek` 컴파일 명령 = **논증**(입력 실측 분리 표기) / FAQ 1번 채택안 = **사용자 결정**. **한 문장에 섞지 않았다** — (g) 논증은 「승격 아님 · 입력만 확인」으로 못박았다).

**관련 카테고리**: **33.7(h) (원천 귀속 감사 — 신설)** / **33.7(i) (🔴 조각내기 코드 부재 — 신규 미결)** / 33.7(f)(g) (부분 해소 2 · 논증 입력 실측) / 33.7(e) (대응 방향 — 사용자 판단 대기 유지) / **5.2(f) (✅ 해소)** / 5.2(a) (파이프라인 입구 repo 밖 — (i)와 같은 축) / **30.9 (감시 기준점 09-18 = 3 / 0 / 45)** / 6.7(d)(e) (④런타임 4이벤트 · `stt` 실존 — 45초 산출 입력) / **6.2 (G29 stale ✅ — PR #65 세트 A · 750ms 미기재)** / **6.3(k-2) (헤드룸 주석 ✅ — `a918173` + `8200669` 드리프트 실증)** / **6.3(o) (`jsonpeek` 컴파일 명령 부재)** / **27.8(m) (②④ ✅ · PR #65 요약)** / **27.8(o) (2일차 오류 5건 — 신설)** / **33.9 (회차 라벨 (54) → (53) · PR #65 세트 D 기록)** / 카테고리 1 머리 (PR #65 세트 C — 재정정 0) / **17.2 (검증 자산 베이스라인 — 신설)** / 7.5(i) · 카테고리 16 · 16.1 (기준값) / **8.7(a) (FAQ 1번 ✅ · 🟡 미정정 2건 · fire_alarm 근거 경계)** / **8.4(f) (한계 ④ `HelpPage` 포인터)** / 8.3 (5060 페르소나 — 최소 개입안 근거) / **카테고리 20 (MCP 격리 워크트리 관측 — 신설)** / **카테고리 21 (`ps` 21건 · 경계)** / 카테고리 10 (규약 이탈 2건 — 무접촉) / 학습 13 · 14 · 16 · 17 · 19 · 21.
**관련 commit**: `601e7a2`(**PR #65 squash** — 세트 A~D + 헤드룸 주석 쌍) · `765624b`(**PR #66 squash** — FAQ 1번, 착수 시점 HEAD) · 내부 커밋 `4e9e942`(📝 Docs SSoT 2건) · `fe2b9f9`(🗃️ Comment 코드 3파일) · `a918173`(🗃️ `mic_common.h` 헤드룸) · `8200669`(🗃️ `mic_test.cpp` 헤드룸 짝) · `e1df298`(🐛 FAQ 1번 — `feat/dashboard-faq-copy` 워크트리 head). 선행 = `c48779a`(2026-09-18 PoC-(53) Set 1). 문서 반영 = 본 커밋(docs-only, **PR 없음, main 직 push**).

## 2026-09-19 (토) 후속 — PoC-(54) Set 1 — 커밋 규약 이탈 2종 사용자 결정 등재 — 카테고리 10 미결 ✅ 해소(14종에 추가하지 않고 이탈로 남김) + 파생 확정 「UI 시각 변경 = `Design`」 (카테고리 10)

2026-09-19 사용자 결정으로 커밋 규약 이탈 2종(`🧪 Spike` `086c6da` PR #23 / `🎨 Style` `3544db4` PR #8)의 처리 방침이 확정됐고, 그 확정이 프로젝트 지침(하위 계층)에만 반영돼 **SSoT가 뒤처진 역방향 stale** 상태였다. 본 Set은 `docs/decisions.md` 카테고리 10 미결 1건을 해소 표기로 전환하고 본 엔트리를 남긴 것이 전부다(**문서 2파일 · 물리 편집 지점 2곳**). 확정 내용 = **규약 14종에 추가하지 않고 이탈로 남긴다** · **소급 정정 없음** · **파생 확정: UI 시각 변경의 Type은 `Design`이다**(`Style` 아님).

**Step 0 가정 대조 — 5항 전건 O(불일치 0건)**: 착수 시점 HEAD **`3247717`** · `main` · 워킹트리 **clean**(`git status --porcelain` 무출력). ① 카테고리 10에 「판단 필요 = 이 2종을 …추가할지, 이탈로 남길지」 미결 줄 **`grep -c` = 1건**(정확히 1 — 2건 이상 정지 임계 미달) ② 그 줄은 `🟡 **[신규 미결]`로 시작하고 발견·문서 반영일이 **2026-09-08 PoC-(42)**로 적혀 있다 ③ 다음 줄에 `※ 실물 14종 = …` 목록이 있고 **`🎨 Design`이 그 안에 실존**한다(근거 ②의 전제 = 유효 — 정지 트리거 해제) ④ `docs/git-convention.md`는 본 작업 수정 대상 아님(범위 OUT) ⑤ `decisions-log.md` 마지막 엔트리 헤더 날짜 = **2026-09-19**. 카테고리 10 헤더(`^## 카테고리 10`) = **1건**.

**같은 날짜 2번째 엔트리 형식 판정 (위임 §5 Step 2 ⚠️ — MCP 판정, 근거유형 = 실물 형식 인용)**: 마지막 엔트리가 이미 `## 2026-09-19 (토) — PoC-(53) Set 1 후속`이라 **새 헤더를 쓰되 「(요일) 후속 —」 형식**을 택했다. 근거 = 같은 파일에 **같은 날 2엔트리 선례 2건**이 실존한다 — `## 2026-09-16 (수) 후속 — PoC-(50) Set 1 후속` · `## 2026-09-18 (금) 후속 — PoC-(53) Set 1`. 후자는 **날짜 「후속」 + 새 회차 라벨**이라 본 건(회차가 (53) → **(54)**로 넘어감)과 형식이 정확히 같다. ⇒ 기존 엔트리에 덧붙이지 않고 신규 엔트리로 작성했다(형식 발명 0).

**find-skills**: `"changelog decision log"` **0건** / `"markdown section editing"` **0건**(후자는 2026-09-18 · 09-19 회차와 동일 결과 — 레지스트리 변동 없음). 배치 전건 0이라 위임 §1 지정대로 대조 질의 `"git"` 실행 = **1449건**(도구 생존 확인 — 0건은 레지스트리 실측이지 도구 사망이 아니다). **채택 0건 · 미채택 사유 = ③ 검색 결과 0건**(①②는 해당 없음) · install 0 · info 0.

### 편집 건별 (전건 `str.count` == 1 assert 통과, 라인 번호 0건 사용, 물리 편집 지점 2곳)

- **E1 — 미결 라벨 → 해소 마커 (`decisions.md` 카테고리 10)**: `🟡 **[신규 미결] 커밋 이력에 규약 14종 이탈 2건 (…)**` 머리를 **취소선 처리**하고 `→ **✅ 해소 (결정 2026-09-19 PoC-(54), 근거유형 = 사용자 결정)**`를 붙였다. 형식은 **같은 파일의 기존 컨벤션을 그대로 따랐다**(학습 16) — 선례 = 5.2(f) 「`~~🔵 [논증 — 미실증]~~ → ✅ 해소 (…)`」 · 6.4(f) 「`~~🟡 [신규 미결] 가짜 ToF 하드코딩 잔존 (…)~~ → ✅ 해소 (2026-09-08 PoC-(42), PR #47 `846b342`)`」. 위임이 금지한 **「줄 전체 취소선」은 쓰지 않았다** — 중간부(이탈 2건의 커밋 해시 · PR 번호 · 「소급 정정 대상은 아니다」)는 **지금도 유효한 실측 사실**이라 마커 없이 보존했다.
- **E2 — 판단 필요 문장 취소선 + 확정 append (같은 줄)**: `~~판단 필요 = 이 2종을 `git-convention.md`에 **추가할지**, 이탈로 남길지.~~` 뒤에 확정을 이어 붙였다. append 구성 = **확정 결론**(규약 14종에 추가하지 않고 이탈로 남긴다) + **결정 주체 = 사용자 · 결정일 = 2026-09-19 · 근거유형 = 사용자 결정** + **근거 3(①②③ 원문 그대로)** + **소급 정정 없음** + **파생 확정 「UI 시각 변경의 Type은 `Design`」** + **`docs/git-convention.md` 무변경 명기**. 근거 3은 위임 §7 지정대로 **윤문 0 · 원문 그대로** 옮겼다.

### 등재 내용 요약

- **확정(결정 주체 = 사용자, 2026-09-19)**: 과거 이탈 2건을 **규약 14종에 추가하지 않고 이탈로 남긴다**.
- **근거 3**: ① 14종도 이미 많아 분류 비용이 이득을 넘는다. ② `Spike`는 `Feat`으로, `Style`은 **이미 존재하는 `Design`**으로 흡수된다 — 규약 부족이 아니라 **오용**이었다. ③ 과거 이탈에 맞춰 규약을 넓히면 「이탈하면 규약이 따라온다」는 **선례**가 생긴다.
- **소급 정정 없음**: 과거 커밋 메시지 **무접촉**(이력 훼손 금지 — 기존 서술도 유지).
- **파생 확정**: **UI 시각 변경의 Type은 `Design`이다**(`Style` 아님). ※ 실물 14종에 `🎨 Design`이 이미 있음을 같은 절 다음 줄에서 확인했다.
- **`docs/git-convention.md` 무변경**: 「추가하지 않기로 한 결정」이라 규약 파일에 손댈 것이 없다 — 파일 **무접촉**(범위 OUT, `git diff` 0).

**SSoT 정합 검증 (문서 전용이라 코드 3단계 ①②는 무관)**

- **V1 취소선·볼드 파리티** — `decisions.md` 착수 전 **98줄 / 246개 / 볼드 10674개** → 종료 **99줄 / 250개 / 볼드 10692개**(계수 단위 = `grep -c` **줄 수** / `grep -o | wc -l` **출현 수**). 증가분 **+1줄 / +4개 = 신규 2쌍**(E1 · E2)이고 설계와 정확히 일치 — **짝수 = 마크다운 무파손**. 볼드 **+18개 = 9쌍**(해소 마커 1 · 확정 결론 1 · 결정 주체/결정일/근거유형 3 · `Design` 흡수 1 · 소급 정정 없음 1 · 파생 확정 1 · 무변경 1).
- **V2 삭제 0줄 증명** — `git diff --numstat docs/decisions.md` = **+1 / −1**, 제거 1줄은 **같은 줄의 치환 결과**(줄 삭제 0). 토큰 보존 = **원시 기준 미포함 · 정규화(마커 제거) 기준도 미포함**이며 원인은 전부 **마커·화살표 삽입으로 문자열이 갈라진 것**이다. 정규화 토큰 손실 = **1토큰** — 「`git log` 뒤에 닫는 볼드·콜론이 붙은 토큰」이 해소 마커 삽입으로 갈라진 것이라 **마크업 경계 토큰 1개뿐이고 내용어 손실 0**이다. 원문 실질 문구(`🧪 Spike`(`086c6da`, PR #23) · `🎨 Style`(`3544db4`, PR #8) · 「14종에 없는 타입」 · 「소급 정정 대상은 아니다」 · 판단 필요 문장 전문)는 **전건 잔존**(취소선 안에 보존).
- **V3 편집 지점 실물 되읽기** — 물리 편집 지점 **2곳** 전건 `count == 1` assert 통과(스크립트가 불일치 시 abort). **라인 번호 사용 0건**(편집 앵커 · 본 엔트리 본문 양쪽 — 앵커는 패턴 문자열로만 잡았다). 편집 후 두 파일 실물 재조회로 해당 구간 출력 확인.
- **V4 🔴 한글 전수 검증** — 착수 전 파일에 **없던 음절 0개**(`decisions.md` 추가분, 코드포인트 기준 집합 차분). 「신규 음절 0」이 충분조건이 아니므로 **신규 한글 단어 18개를 전수 추출해 문맥 출력·육안 통독** = 결정이므로 / 결정일 / 규약을 / 규약이 / 넓히면 / 따라온다 / 많아 / 변경의 / 부족이 / 선례가 / 오용이었다 / 이득을 / 이탈에 / 이탈하면 / 존재하는 / 종도 / 추가하지 / 흡수된다 — **전건 정상 단어 · 모지바케 0**. 바이트 보강 = UTF-8 strict decode **통과** · `U+FFFD` **0건** · NFC 정규화 동일.
- **V5 대조 원본** — 등재 수치·식별자는 전부 **실물 대조**다: 커밋 해시 2건(`086c6da` · `3544db4`) · PR 번호 2건(#23 · #8)은 **기존 SSoT 줄의 문구를 그대로 보존**한 것이고(신규 주장 0), 14종 목록의 `🎨 Design` 실존은 **같은 절 다음 줄 실측 grep**, 착수 HEAD `3247717` · 워킹트리 clean은 `git log -1` · `git status --porcelain` 실측. **로그·실측 없는 수치 0건 기재.**
- **V6 git status** — `docs/` **2파일만**(`git status --short` = `M docs/decisions.md` · `M docs/decisions-log.md`). `git add`는 두 파일만 명시 스테이징. 시크릿 스캔(SSID · 토큰 · 자격증명 · IP 패턴) **0건**.

**비범위**: **코드 0 수정 · 0 읽기** — `server/` · `dashboard/` · `ml/` · `firmware/` **전부 무접촉**. 🔴 **`docs/git-convention.md` 무접촉**(범위 OUT — 「추가하지 않기로 한 결정」이라 규약 파일에 반영할 변경 자체가 없다. 수정 필요 판단이 들면 정지하라는 §9 트리거에 해당하는 상황은 발생하지 않았다). **과거 커밋 메시지 무접촉**(소급 정정 없음) · **노션 무접촉**(Set 3 소관) · **프로젝트 지침 파일 무접촉**(Set 2 소관 — 이미 반영돼 있던 하위 계층이 본 Set의 입력이었다). 🔴 **미결 상태 전환은 카테고리 10 1건뿐** — 신규 미결 **0건 신설**. 카테고리 10의 나머지 미결(**이모지 2종**)은 **무접촉**. 이전 회차 미결(33.7(e)(i) · 8.7(a) FAQ 2번 · 7.7(l) · 6.2 G29 750ms · 카테고리 20 `.claude/` 방침 등)도 **전건 무접촉**. 브랜치·PR **생성 0**(문서 전용 = `main` 직 push).

**학습 적용**: **학습 13**(출처 catch — 인용 전 grep. 편집 앵커 2곳 전건 `count == 1` assert, **라인 번호 0건**. 미결 줄 · 카테고리 10 헤더 · 14종 목록 `Design` 실존을 **착수 전 실물로 확인**했고, 미러 참조 후보 `086c6da` · `3544db4` grep으로 **`decisions.md` 내 중복 미결 줄 0건**임을 확인해 한 곳만 고쳤다) / **학습 16**(기존 컨벤션 우선 — 해소 표기 형식을 발명하지 않고 **같은 파일의 5.2(f) · 6.4(f) 선례 형식**을 그대로 따랐고, 같은 날 2번째 엔트리 헤더도 **09-16 · 09-18 선례**를 근거로 정했다) / **학습 21**(미결 자체가 유령일 수 있음 — **역방향 경계**. 착수 전 「판단 필요 = 이 2종을」 `grep -c` = **1건**으로 미결의 실존을 먼저 확인했고(부재면 취소선 = 날조 → §9 정지), 반대로 **`decisions-log.md` 1575행의 같은 미결 서술은 과거 엔트리라 소급 수정하지 않았다** — 로그는 시점 기록이라 정정 대상이 아니다).

**관련 카테고리**: **카테고리 10 (Git Convention — 규약 이탈 2건 ✅ 해소 · 파생 확정 「UI 시각 변경 = `Design`」)** / 카테고리 10 이모지 2종 미결 (무접촉) / 학습 13 · 16 · 21.

**관련 commit**: `3247717`(2026-09-19 PoC-(53) Set 1 후속 문서 커밋, **착수 시점 HEAD**) · 대상 이탈 커밋 2건 = `086c6da`(PR #23 `🧪 Spike`) · `3544db4`(PR #8 `🎨 Style`) — **둘 다 무접촉**(소급 정정 없음, 인용만). 문서 반영 = 본 커밋(docs-only, **PR 없음, main 직 push**).

## 2026-09-20 (일) — PoC-(54) Set 1 후속 — 파이프라인 입구 코드 소재 규명(33.10 신설) + 33.7(h) csv 출처 부분 해소 · (i) ✅ 소재 확인(미결 유지) + 5.2(a) · 33.6(a) 판단 입력 append + 🆕 4번째 `TARGET` `smoke_detector` 349세그먼트 실측(오디오 0) (카테고리 5.2/16/33.6/33.7/33.10)

2026-09-19 회차 33.7(i)가 *"`00_source_raw` · `01_extracted` → `01_clips` 조각내기 코드가 repo에 없다"*를 신규 미결로 등재하면서 **소재를 미확인으로 남겼다**. 2026-09-20 학부생 로컬 셸 조사에서 그 코드가 **별개 로컬 repo**(`~/Desktop/xorms/프로젝트/ddingdong/pretest/scripts/` — 33.5가 인용한 `park-taegeun/ddingdong-pretest`의 클론, HEAD `7d93743`, mtime **2026-04-28**)에 **스크립트 7개로 실재함**이 확인됐고, 그 코드가 **33.7(h)의 미확정 2건 중 csv 출처 축**을 닫았다. 추가로 **4번째 클래스 `smoke_detector`의 흔적**(csv 349세그먼트 · 오디오 0)이 실측됐다. 본 Set은 `docs/decisions.md`에 **33.10을 신설**하고 기존 **4곳(33.7(i) · 33.7(h) 2곳 · 5.2(a) · 33.6(a))**을 갱신한 뒤 본 엔트리를 남긴 것이 전부다(**문서 2파일 · 물리 편집 지점 6곳**). 🔴 **대응 방향 · 임계 · 정책 · 수치 신설 0건**이며 **33.7(e) · 33.6(a)는 사용자 판단 대기 그대로**다.

**Step 0 가정 대조 — 7항 전건 O(불일치 0건)**: 착수 시점 HEAD **`24c8630`** · `main` · 워킹트리 **clean**(`git status --short` 무출력). ① 33.7(i)에 「조각내기 코드가 repo에 없다」 미결 **`grep -c` = 1**이고 「스크래치패드 · 타 폴더 소재 **미확인**」으로 남아 있다 ② 33.7(h)에 「csv 8개는 3열」 포함 미확정 줄 **`grep -c` = 1** ③ 33.7(h)에 「디코딩 결정성에 기댄」 **`grep -c` = 1** ④ 5.2(a) 「유입 경로 — `04_direct_recording` → `01_clips` 코드가 없다」 실존(`04_direct_recording` grep 5줄 중 5.2(a) 헤더 실물 확인) ⑤ 카테고리 33 최대 절 번호 = **33.9**(`^### 33\.[0-9]+` tail = 33.7 · 33.8 · 33.9) ⑥ `decisions-log.md` 마지막 엔트리 = **2026-09-19 (토) 후속 — PoC-(54) Set 1** ⑦ 33.6(a)에 「임계 재검토 / **4번째 클래스** / 재학습 / 시연 각본 조정」 사용자 판단 대기 서술 실존. ⇒ **§9 정지 트리거(가정 1·2·3 중 X) 미발동**, 스코프 축소 없음.

**신설 절 번호 결정 (위임 미지정 — MCP 판정, 근거유형 = 실측 grep)**: **33.10**. 근거 = 카테고리 33의 현재 최대 절이 **33.9**이고 본 절이 **33.7(h)(i)의 동성격 후속**(같은 원천 귀속 축)이라 카테고리 33 말미가 자연스럽다. 충돌 검사 = `^### 33\.10` 사전 grep **0건**. 파일 말미(33.9의 `---` 다음)에 append했고 **기존 절 순서 무변경**이다.

**33.7(i) 미결을 🟢로 닫지 않은 판정 (위임 §5 Step 2 ⚠️ — MCP 판정 + 근거 보고)**: **미결 유지**. 근거 3 — ① 미결 제목 「**repo에 없다**」는 **지금도 사실**이다(pretest는 **별개 repo**이지 본 repo가 아니다) — 위임 §5도 「사실이므로 취소선 대상이 아니다」로 지정했다 ② 그 절이 스스로 적은 열린 축은 「🔴 **처리 방침 미확정 = 사용자 판단 대기**」인데 본 Set은 **처리 방침을 확정하지 않았다**(§7 불변식) ③ 「**소재 확인**」과 「**대응 방향 확정**」은 다른 축이다. ⇒ 미결 제목 · 상태 마커 **무변경**, 하위에 **✅ 소재 확인** 서술만 append했다. ⇒ §9의 「판단이 갈리면 정지」 상황은 **발생하지 않았다**(세 근거가 한 방향).

**같은 회차 라벨 중복 회피 판정 (위임 §5 Step 3 ⚠️ — MCP 판정, 근거유형 = 실물 형식 인용)**: 마지막 엔트리가 이미 `## 2026-09-19 (토) 후속 — PoC-(54) Set 1`이라 **날짜만 바꾸면 「PoC-(54) Set 1」이 두 번** 나온다. 같은 파일 선례 = `## 2026-09-18 (금) 후속 — PoC-(53) Set 1` 다음 날의 `## 2026-09-19 (토) — PoC-(53) Set 1 후속` — **같은 회차 · 같은 Set이 새 날짜로 이어질 때 「Set N 후속」을 붙인다**. ⇒ 본 엔트리는 **`## 2026-09-20 (일) — PoC-(54) Set 1 후속`** 형식을 택했다(형식 발명 0).

**find-skills**: `"dataset provenance"` **0건** / `"audio dataset pipeline"` **0건**. 배치 전건 0이라 위임 §1 지정대로 대조 질의 `"git"` 실행 = **1449건**(도구 생존 확인 — 0건은 레지스트리 실측이지 도구 사망이 아니다). **채택 0건 · 미채택 사유 = ③ 검색 결과 0건**(① 설치 수·출처 미달 · ② 워크플로 불일치는 해당 없음) · install 0 · info 0.

### 편집 건별 (전건 `str.count` == 1 assert 통과 후 치환, 불일치 시 스크립트 abort, 라인 번호 앵커 0건, 물리 편집 지점 6곳)

- **E1 — 33.7(i) ✅ 소재 확인 append**: 「`「repo에 없다」이지 「잃었다」가 아니다` — 소재 **미확인**」 줄 **아래에 하위 bullet 신설**. 내용 = 소재 절대경로 · pretest repo 클론 관계(33.5) · HEAD `7d93743` · mtime 2026-04-28 · 스크립트 7개 · **미결 제목 취소선 아님 명기** · [논증] 카테고리 16(2026-05-07)보다 9일 앞서 **0줄 = 결함 아니라 이력** · 「발생 지점 지목 수단」 축이 33.10(c)(e)로 **입력을 얻었다** · 🔴 **소재 확인 ≠ 처리 방침 확정**. **취소선 0개 사용.**
- **E2 — 33.7(h) csv 생성 코드 부재 → 부분 해소 (취소선 1쌍)**: 🔴 **위임은 「csv 미확정 문장에 취소선」이라 했으나 MCP가 취소선 범위를 좁혔다** — 문장 전체가 아니라 **`**생성 코드가 repo에 없어**` 클로즈 1개에만** 취소선을 걸었다. 근거 = ① 「csv 8개는 3열(`ytid,start,end`) 가공본」은 **지금도 사실**이고 ② 「이중 등재가 원본 라벨인지 우리 필터링인지 **미확정**」도 **지금도 유효**하다((e)로 **기울 뿐** 확정 아님) ③ 무효가 된 것은 **미확정의 사유 절(「생성 코드가 repo에 없어**서**」)** 하나뿐이다. **사실인 서술에 취소선 = 날조**라는 같은 파일 컨벤션(33.7(d) · 33.7(i) · 33.6(b)의 「틀린 것이 아니라 범위가 좁았다 — 취소선 대상이 아니다」 선례)을 따랐다. append 구성 = [실측] 생성 코드 = `audioset_filter.py` · csv 8개 = `data/filtered/` 복사본 · 8쌍 행 수 동일 + [논증] `break` 부재 → 원본 다중 라벨 쪽으로 기움 + 🔴 `label_list` 미확인이라 **미확정 그대로** + 「3열 가공본」은 취소선 대상 아님 명기.
- **E3 — 33.7(h) 「디코딩 결정성에 기댄 논증」 줄 (취소선 0개)**: 위임 지정대로 **취소선 없이** append만 했다 — 결정성 주체 = **ffmpeg + 인자 전문** 특정 · 선두 ≈10.0초 → 3조각 / ≈8.95초 → 2조각 산술 정합 · 🔴 **ffmpeg 버전 미기록이라 논증 등급 무변경**.
- **E4 — 5.2(a) 포인터 append (신규 bullet)**: (a) 마지막 bullet(16kHz mono 가드) 아래에 신설. 내용 = 입구 코드 소재 확인 + 33.10 포인터 + 🔴 **「`04_direct_recording` → `01_clips` 유입 경로 0건」 무변경**(세 전처리는 `00_source_raw` · FSD50K dump에서만 읽으므로 pretest 스크립트도 그 경로를 만들지 않는다) + **「수동 복사 단계」 판정도 무변경**. **취소선 0개.**
- **E5 — 33.6(a) 판단 입력 append**: 「대응 방향 = 사용자 판단 대기」 줄 **끝에** 이어 붙였다(2026-09-15 · 09-17 append 선례와 같은 자리·같은 형식). 내용 = `TARGET` 4번째 = `smoke_detector`(`/m/01g50p`) · 349세그먼트(183 / 166) · 🔴 **오디오 0** · YouTube 링크 생존률 미확인 · ⚠️ **`fire_alarm`과 같은 계열이라 OOD 축 완화 여부 미실증 — 두 축을 섞지 말 것** · 🔴 **대응 방향 여전히 미확정**. **선택지 평가 · 권고 0건.**
- **E6 — 33.10 신설 (파일 말미 append, 73줄)**: 제목에 **신설일 · 발견일 = 반영일 = 2026-09-20 · 근거유형 3분 · 계수 단위**를 병기했다(형식은 **33.7(h) · 33.8 · 33.9 실물을 읽고** 따랐다 — 발명 0). (a) 소재 / (b) 조각내기 규칙 / (c) 선두 구간 논증 / (d) `PIECE_SUFFIX_PATTERN` 발원 / (e) 클래스 교차 / (f) csv 출처 / (g) `smoke_detector` / (h) 한계 / (i) 파급 **9항**. 항목마다 **근거유형(실측 / 논증 / 문서 인용) 개별 병기**했고 **한 문장에 섞지 않았다**.

### 등재 내용 요약 (실측 9건 — 전건 2026-09-20 학부생 로컬 셸, 발견일 = 반영일 = 2026-09-20)

- **소재 (실측)**: `~/Desktop/xorms/프로젝트/ddingdong/pretest/scripts/` — **본 repo와 다른 별개 로컬 repo**, 33.5 인용 pretest repo의 클론. HEAD `7d93743` *"feat: add preprocessing pipeline and train/val/test split"*, mtime **2026-04-28**. 스크립트 **7개**(`aihub_preprocess.py` · `audioset_download.py` · `audioset_filter.py` · `audioset_preprocess.py` · `dataset_split.py` · `fsd50k_download.sh` · `fsd50k_preprocess.py`).
- **[논증] 0줄 = 이력**: mtime이 본 repo 최초 셋업(**2026-05-07**, 카테고리 16)보다 **9일 앞선다** ⇒ 본 repo에 조각내기 코드가 없는 것은 **결함이 아니라 이력**.
- **조각내기 규칙 (실측 코드 대조)**: 3개 전처리가 **완전 동형** — `CLIP_SEC = 3.0` / `start = i * CLIP_SEC` / `{stem}_{start_ms:07d}.wav` / ffmpeg `-ss {start} -t 3.0 -ar 16000 -ac 1 -sample_fmt s16` / `int(duration // CLIP_SEC)`(마지막 불완전 조각 버림) / `if out_path.exists(): skipped`(멱등). 담당 = `aihub_preprocess` → `fire_alarm` / `audioset_preprocess` → `01_clips/{class}` / `fsd50k_preprocess` → `doorbell` · `knock`. 유일한 분기 = `fsd50k_preprocess`의 `duration < CLIP_SEC` 단일 클립.
- **[논증] 선두 구간 동일의 기전**: ffmpeg가 선두부터 고정 간격으로 자르므로 앞 ≈10.0초 공통 → **3조각 동일**, ≈8.95초 → **2조각 동일** — 33.7(h)의 **G01 · G04 구조와 산술 정합**. 🔴 **ffmpeg 버전 미기록 → 재실행 결정성은 논증 유지**.
- **[문서 인용 + 논증] `PIECE_SUFFIX_PATTERN` 발원**: `dataset_split.py` 머리 주석의 「`{source_stem}_{start_ms:07d}.wav` → source_stem이 그룹키」 · 「마지막 `_XXXXXXX` 제거」가 본 repo `ml/pipeline/config.py`의 `r"_\d{7}$"`와 동일 규칙 ⇒ **33.7(d)의 「파일명 기준이라 내용 동일 파일은 범위 밖」 경계도 상속**된 것(본 repo가 만든 한계가 아님).
- **[논증] 클래스 교차**: `audioset_filter.py`의 `TARGET` 순회에 **`break` 없음** ⇒ 두 mid를 가진 세그먼트는 두 리스트에 각각 들어간다 ⇒ 33.7(h) 미확정이 **원본 다중 라벨 쪽으로 기운다**. 🔴 **2세그먼트 실제 `label_list` 미확인**(`balanced_train_segments.csv` 미열람) — **확정 금지**.
- **[실측] csv 출처 ✅**: `00_source_raw/audioset/` csv **8개** = pretest `data/filtered/` **복사본**, **8쌍 전건 행 수 동일**(`wc -l`) ⇒ **생성 코드 = `audioset_filter.py` 확정**, 산출 경로 `data/filtered/{split}_{name}.csv`도 코드와 일치.
- **[실측] 🆕 `smoke_detector`**: `TARGET` **4개** = `doorbell`(`/m/03wwcy`) · `knock`(`/m/0dxrf`) · **`smoke_detector`(`/m/01g50p`)** · `fire_alarm`(`/m/07pp_mv`). 최종 `CLASSES`는 3종이라 **어디에도 없다**. 세그먼트 수(헤더 제외) = doorbell 60 / 60 · knock 60 / 60 · fire_alarm 60 / **61** · **smoke_detector 183 / 166 = 349**. 🔴 **오디오 0**(`00_source_raw/` 직하 3폴더뿐 · 데이터셋 전역 `find -iname "*smoke*"` = 위 csv 2건뿐 · 본 repo `docs/` · `ml/` · `server/` grep 0건) ⇒ **긁기는 실행됐고 다운로드는 안 됐다**. ⚠️ `fire_alarm` eval만 **+1**인 점은 33.7(h)의 2세그먼트 교집합과 **맞물릴 수 있으나 수가 정확히 대응하지 않는다 — 확정 금지**. **[논증]** 3클래스 60 / 60은 상한 코드 부재(전수 순회)이므로 **AudioSet 원본 분포**로 읽는다.
- **한계 6종 전건 등재**: ffmpeg 버전 미기록 / `label_list` 미확인 / 🔴 **경로 stale 3건**(세 전처리의 `SOURCE_DIR` · `OUTPUT_BASE`가 **Desktop 옛 경로** — 카테고리 5.1의 TCC(EPERM) 이동 확정과 어긋나 **이대로 재실행하면 다른 위치에 쓴다**. **PR #60이 본 repo `DEFAULT_DATA_ROOT`에서 고친 것과 같은 종류**, 33.8) / `smoke_detector` 오디오 0 · YouTube 링크 생존률 미확인 / 🔴 **`smoke_detector` = `fire_alarm`과 같은 계열 경보음이라 33.6(a) OOD 축 완화 여부 미실증 — 두 축 혼독 금지** / **읽기 전용 조사**(pretest repo · 데이터셋 무접촉).
- **파급 (사실까지만)**: 33.7(i)의 「`01_clips`를 손으로 고치는 것 말고 방법이 없다」에 대해 **재생성 경로가 생겼다**는 **사실만** 적었다. 🔴 **33.7(e) · 33.6(a) 대응 방향은 사용자 판단 대기 그대로** — 선택지 **평가 0 · 권고 0**.

**SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 위임 §6 8항 예외 적용)**

- **V1 취소선·볼드 파리티** — `decisions.md` 착수 전 **99줄 / 250개 / 볼드 10692개** → 종료 **100줄 / 252개 / 볼드 11022개**(계수 단위 = `grep -c` **줄 수** / `grep -o | wc -l` **출현 수**). 취소선 **+2 = 신규 1쌍**(E2 하나뿐 — E1 · E3 · E4 · E5 · E6은 취소선 0개)이고 **짝수**. 볼드 **+330 = 165쌍**, **짝수**. ⇒ 마크다운 무파손.
- **V2 삭제 0줄 증명** — `git diff --numstat docs/decisions.md` = **+78 / −3**. 제거 3줄은 **같은 줄의 치환 결과**(E2 · E3 · E5 — in-place append)이고, E1 · E4 · E6은 **순수 삽입**이라 제거가 없다(`git diff -U0` 훅 헤더 `-3420 +3421` · `-3662 +3663` · `-3668 +3669` = 1:1 치환, `-136,0 +137` · `-3676,0 +3678` · `-3721,0 +3724,73` = 삽입). **줄 삭제 0.** 토큰 보존 = **원시 기준 손실 1토큰**(「없어」 뒤에 닫는 볼드와 쉼표가 붙은 토큰) · **정규화(취소선 · 볼드 · 백틱 마커 제거) 기준 손실 0토큰**. 원시 1토큰은 **E2의 취소선 마커가 「없어」의 닫는 볼드와 쉼표를 갈라놓은 마크업 경계 토큰**이며 **내용어 손실 0**이다.
- **V3 앵커 `count == 1` + 실물 되읽기** — 물리 편집 지점 **6곳 전건 `str.count` == 1 assert 통과**(불일치 시 `sys.exit` abort 설계, 실제 abort 0회). **라인 번호 앵커 사용 0건** — 전부 패턴 문자열로 잡았다(본 엔트리 본문의 라인 번호 인용도 0건, 위 `git diff -U0` 훅 헤더는 diff 출력 인용이지 앵커가 아니다). 신설 절 append는 파일 말미 문자열(`26.7(발표 스크립트 출처)\n\n---\n`) **endswith assert**로 고정했다. 편집 후 **6곳 전건 실물 재조회**로 구간 출력 확인.
- **V4 🔴 한글 전수 검증** — 추가분에서 **착수 전 파일에 없던 음절 = 1개**(`긁` **U+AE01**, 「긁기는」 — 정상 음절). 「신규 음절 0」이 충분조건이 아니므로 **신규 한글 단어 100개를 전수 추출해 문맥 출력·육안 통독** = 가공본 / 가리킨다 / 개도 / 건이다 / 결정성은 / 결정성의 / 결정성인가 / 경계도 / 경보음 / 고르게 / 공통이 / 공통이면 / 교차를 / 교차의 / 권고하지도 / 규칙이며 / 그룹키 / 긁기는 / 기준이라 / 다운로드는 / 닫힌다 / 대응하지 / 도구의 / 동일로 / 동형이다 / 뒤집지 / 등급은 / 때문이므로 / 리스트에 / 맞는다 / 맞물릴 / 목록만으로 / 묶어 / 미상이던 / 미확인에서 / 미확정은 / 미확정이 / 버림 / 버전이 / 분담 / 분포로 / 불완전 / 사실이고 / 산술이 / 생존률 / 생존률도 / 선두부터 / 선택은 / 선택지를 / 선택지의 / 설명된다 / 세그먼트의 / 소재가 / 순회에 / 스크립트의 / 승격되지 / 시작했다 / 실린 / 실재한다 / 실측분 / 실행됐고 / 쓰였고 / 앞서므로 / 앞선다 / 오디오는 / 옮겨간 / 완화되는지는 / 원본이다 / 위치에 / 이대로 / 이동을 / 이어지는 / 자르므로 / 잡음이 / 재계측은 / 재생성 / 재현이나 / 조사는 / 종류 / 종류의 / 줄인 / 지금도 / 직하는 / 차단으로 / 채우고 / 채움 / 채워졌다 / 축뿐 / 축이며 / 출력명 / 클론 / 평가하지도 / 폴더뿐 / 필터링 / 필터링인지 / 한쪽으로 / 항으로 / 확보됐으므로 / 확인으로 / 확정됐고 — **전건 정상 단어 · 모지바케 0**. 바이트 보강 = UTF-8 **strict decode 통과** · **`U+FFFD` 0건** · **제어문자 0건**(`\n` · `\t` 제외) · **NFC 정규화 동일**.
- **V5 대조 원본** — 등재 수치·식별자는 전부 **학부생 로컬 셸 실측분**이며 **위임 본문이 원본**이다(§3 지정 — MCP는 repo 밖을 **읽지도 재실행하지도 않았다**). 본 repo 측 대조만 MCP가 실측했다: 착수 HEAD `24c8630` · 워킹트리 clean(`git log -1` · `git status --short`) · §4 grep 3건 **각각 1** · 절 번호 `^### 33\.10` 사전 grep **0건** · 「관련」 줄 절 번호 **전건 grep 실존 확인**(33.5 · 33.6(a) · 33.7(h)(i) · 33.8 · 33.9 · 5.1 · 5.2(a) · 카테고리 16 · 카테고리 20 · 학습 21). **로그·실측 없는 수치 0건 기재.**
- **V6 git status** — `docs/` **2파일만**(`M docs/decisions.md` · `M docs/decisions-log.md`). `git add`는 두 파일만 명시 스테이징. 시크릿 스캔(토큰 · 자격증명 · SSID · IP 패턴) **0건**.

**비범위**: 🔴 **repo 밖 무접촉** — `~/Desktop/xorms/프로젝트/ddingdong/pretest/` 및 `~/ML 학습 데이터/`를 **읽지도 쓰지도 않았다**(§3 · §7 지정, MCP 재실행 0회). **본 repo 코드 0 수정 · 0 읽기** — `firmware/` · `server/` · `dashboard/` · `ml/` **전부 무접촉**(`ml/pipeline/config.py`의 `PIECE_SUFFIX_PATTERN` 인용은 **기존 SSoT 줄 5.2(b)의 문구를 그대로 쓴 것**이며 코드 파일 미열람). `docs/git-convention.md` **무접촉**. **노션 무접촉**. 🔴 **33.7(e) · 33.6(a) 대응 방향 미확정 유지** — 확정 · 권고 · 평가 **0건**(§7 불변식). **33.7(i) 미결 🟢 전환 0건**(소재 확인만 append). 신규 미결 **0건 신설**. 이전 회차 미결(33.7(e) · 8.7(a) FAQ 2번 · 7.7(l) · 6.2 G29 750ms · 카테고리 10 이모지 2종 · 카테고리 20 `.claude/` 방침 등)도 **전건 무접촉**. 브랜치 · PR **생성 0**(문서 전용 = `main` 직 push).

**학습 적용**: **학습 13**(출처 catch — 인용 전 grep. §4 grep 6건을 착수 전 실행해 3건이 **각각 정확히 1**임을 확인했고, 5.2(a) 갱신 지점은 위임이 문구를 인용하지 않아 **`04_direct_recording` grep으로 실물을 직접 찾았다**. 「관련」 줄 절 번호도 **기재 전 전건 grep 실존 확인**. 편집 앵커 6곳 전건 `count == 1` assert, **라인 번호 앵커 0건**) / **학습 14**(repo 구조 가정 — 「pretest」를 **본 repo의 하위 폴더로 가정하지 않았다**. 실물은 **별개 로컬 repo**이고 33.5가 인용한 GitHub repo의 클론이며, 본 repo 최초 셋업보다 **9일 앞선 mtime**이라 **「같은 프로젝트 = 같은 repo」 가정이 틀렸을 경우의 서술**을 (a)에 명시했다. 또한 **본 repo 코드 파일을 열지 않고** `PIECE_SUFFIX_PATTERN`은 기존 SSoT 줄 인용으로만 다뤘다) / **학습 19**(근본원인 재검증 — 33.7(i)가 등재한 근본원인은 「코드가 **없다**」였는데 실물은 「**다른 repo에 있다**」였다. 그러나 **지정 수정이 no-op은 아니었다** — 미결 제목 「**본 repo에 없다**」는 지금도 사실이라 **취소선을 긋지 않고 소재 확인만 append**했고, 「0줄 = 결함」이라는 **암묵 해석만 「이력」으로 정정**했다. 근본원인의 층을 갈라 적은 것이지 pivot이 아니다) / **학습 21**(미결·차단 사유가 유령일 수 있음 — **두 방향 모두 적용**. ① 33.7(h)의 미확정 **사유**(「생성 코드가 repo에 없어」)가 **유령이 됐으므로 그 사유 절에만 취소선**을 걸고 미확정 자체는 남겼다 ② 33.7(i) 미결은 **유령이 아니었다** — 실존 확인 후 **닫지 않았다**. 「부재 판정」이 **「본 repo 범위」라는 경계 위에서만 유효**했다는 점을 (a)와 「관련」 줄에 명기했다).

**관련 카테고리**: **33.10 (신설 — 파이프라인 입구 코드 소재 · 조각내기 규칙 · `smoke_detector`)** / 33.7(h) (csv 출처 부분 해소 · 디코딩 결정성 도구 특정) / 33.7(i) (✅ 소재 확인 · 미결 유지) / 33.6(a) (「4번째 클래스」 판단 입력 · 대응 방향 미확정 유지) / 5.2(a) (파이프라인 입구 포인터 · 유입 경로 부재 무변경) / 33.5 (pretest repo 인용 선례) / 5.1 (TCC 차단 → 데이터 루트 이동 — 경로 stale 대조 기준) / 33.8 (PR #60 = 같은 종류 stale 선례) / 카테고리 16 (본 repo 최초 셋업 2026-05-07) / 카테고리 20 / 학습 13 · 14 · 19 · 21.

**관련 commit**: `24c8630`(2026-09-19 PoC-(54) Set 1 커밋 규약 문서 커밋, **착수 시점 HEAD**) · 인용만 한 외부 커밋 = pretest repo HEAD **`7d93743`**(**별개 repo · 무접촉 · 읽기도 MCP 미수행**, 학부생 실측 보고분) · 본 repo 선례 `00f43f8`(PR #60 `DEFAULT_DATA_ROOT` stale 정정 — **인용만, 무접촉**). 문서 반영 = 본 커밋(docs-only, **PR 없음, `main` 직 push**).

## 2026-09-20 (일) 후속 — PoC-(54) Set 1 후속 2 — 열린 미결의 차단 사유 실물 검증 전수 발굴(33.11 신설) + 9.1 Stage B 차단 사유 stale 정정 · 7.7(l) 재판정 트리거 충족 append(양건 미결 유지) + 카테고리 21 `.env` 키 중복 현재값 2 → 3 + PR #67 등재 · `enrich_status="skipped"` 3경로 · 미정정 2건 → 1건(8.7(a)) (카테고리 7.7/8.7/9.1/21/33.11)

2026-09-20 열린 미결의 **차단 사유 실물 검증**(읽기 전용 조사)에서 **차단 사유 문장이 실물보다 낡은 건 2개**와 **부수 실측 2건**이 나왔고, 같은 날 **PR #67**이 머지됐다. 전부 SSoT 미등재였다. 본 Set은 `docs/decisions.md`에 **33.11을 신설**하고 기존 **4곳(9.1(h) · 7.7(l) · 카테고리 21 · 8.7(a))**을 갱신한 뒤 본 엔트리를 남긴 것이 전부다(**문서 2파일 · 물리 편집 지점 6곳**). 🔴 **어떤 미결도 닫지 않았다** — 등재한 것은 **차단 사유가 낡았다는 사실**이지 **해소**가 아니다. **대응 방향 · 해소책 · 권고 · 임계 · 수치 신설 0건**이다.

**Step 0 가정 대조 — 8항 전건 O(불일치 0건)**: 착수 시점 HEAD **`5868c71`** · `main` · 워킹트리 **clean**(`git status --porcelain` 무출력). ① 9.1(h)에 Stage B 차단 사유 문장 **`grep -c` = 1**이고 **같은 절 아래**에 2026-08-12 PoC-(36) B-1 ④런타임 완료 서술 실존 ② 7.7(l)에 「작성 시점으로 넘긴다」 **`grep -c` = 1**이고 같은 절에 2026-09-17 PoC-(51) 「값은 맞고 주소가 좁다」 단서 실존 ③ 카테고리 21에 「여전히 2」 **`grep -c` = 1** ④ 8.7(a)에 「같은 계열 미정정 2건」 **`grep -c` = 1**이고 그 서술이 `fire_alarm` 축만 적음 ⑤ `server/app/utils.py` `_apply_prediction_policy()`에 `enrich_status="skipped"` 경로 **3개** 실재(`grep -c` = 3, 리포 전역 5건 중 2건은 `server/seed.py` 시드 픽스처) ⑥ 카테고리 33 최대 절 = **33.10** ⑦ `decisions-log.md` 마지막 엔트리 = **2026-09-20 (일) — PoC-(54) Set 1 후속** ⑧ 워킹트리 clean · HEAD `5868c71`. ⇒ **§9 정지 트리거(가정 1~4 중 X · 가정 8 X) 미발동**, 스코프 축소 없음(가정 5 O라 §2-5 등재 유지).

**신설 절 번호 결정 (근거유형 = 실측 grep)**: **33.11**. 근거 = `^### 33\.[0-9]+` tail 2 = **33.9 · 33.10**이므로 최대 절이 **33.10**이고, 본 절이 **33.10과 같은 「읽기 전용 감사」 계열**(부재 판정 · 범위 경계 방법론을 그대로 상속)이라 카테고리 33 말미가 자연스럽다. 충돌 검사 = `^### 33\.11` 사전 grep **0건**. 파일 말미(33.10의 `---` 다음)에 append했고 **기존 절 순서 무변경**이다.

**엔트리 형식 · 날짜 처리 판정 (위임 §4 6번째 — MCP 판정, 근거유형 = 실물 형식 인용)**: 오늘 이미 `## 2026-09-20 (일) — PoC-(54) Set 1 후속`이 있어 **날짜만으로는 중복**이다. 같은 파일 선례 = **같은 날짜 두 번째 엔트리는 날짜 뒤에 「후속」을 붙인다** — `## 2026-09-15 (화)` → `## 2026-09-15 (화) 후속` / `## 2026-09-16 (수)` → `## 2026-09-16 (수) 후속` / `## 2026-09-18 (금)` → `## 2026-09-18 (금) 후속` **3쌍 전건 동형**. 회차측은 선행 엔트리가 이미 「Set 1 후속」을 쓰고 있어 그대로 쓰면 또 중복이므로 **「Set 1 후속 2」**로 이어 붙였다(위임 머리말의 자칭 「2026-09-20 후속 2」와 동형). ⇒ **날짜측 「후속」 = 실물 선례 3쌍 인용, 회차측 「후속 2」 = 중복 회피 최소 증분**이며 **형식 발명 0**이다.

**find-skills**: `"blocker analysis"` **0건** / `"stale documentation detection"` **0건**. 배치 전건 0이라 위임 §1 지정대로 대조 질의 `"git"` 실행 = **1449건**(도구 생존 확인 — 0건은 레지스트리 실측이지 도구 사망이 아니다). **채택 0건 · 미채택 사유 = ③ 검색 결과 0건**(① 설치 수·출처 미달 · ② 워크플로 불일치는 해당 없음) · install 0 · info 0.

### 편집 건별 (전건 `str.count` == 1 assert 통과 후 치환, 불일치 시 스크립트 abort, 라인 번호 앵커 0건, 물리 편집 지점 6곳)

- **E1 — 9.1(h) Stage B 차단 사유 취소선 (취소선 1쌍)**: 🔴 **취소선 범위를 문장 1개로 좁혔다** — 머리의 「Stage B 미결 유지」 표지에는 **걸지 않았다**(미결은 지금도 유지되므로 그 표지는 사실이다). 취소선 대상은 사유 문장 **「Motion Indicator(…) 실구현·④런타임 미착수.」** 하나뿐이다. **사실인 서술에 취소선 = 날조**라는 같은 파일 컨벤션(33.7(d) · 33.6(b) 선례) 준수.
- **E2 — 9.1(h) 정정 append (취소선 0개)**: `✅ 실구현 착수 (2026-08-12 PoC-(36))` 줄 아래에 하위 bullet 6개 신설. 내용 = [실측] `initToFMotionIndicator()` 완전 구현(호출 2종 · status 분기 · 실패 시 Stage A 계속) · [실측] `+<tof_common.cpp>` 편입 env **5개** 전건 명시 · [실측] **대조군**(라이브러리 원본 · 호스트 스텁을 별개로 잡음) · 🔴 **절 내부 드리프트 명기**(2026-08-12에 반증된 문장이 남음, 27.8(m)⑤의 절 내부 판) · 🔴 **위임 전제 오류 catch**(아래) · [논증] **실제 잔존 축 = 9.4(e) · 9.3(G)**. 🔴 **미결 자체 무변경**.
- **E3 — 7.7(l) 재판정 트리거 충족 append (취소선 0개)**: 위임 지정대로 **취소선 없이** (l) 말미에 top-level bullet 1개 + 하위 4개. 내용 = [실측] `uplink_common.cpp`의 `/api/v1/enrich` URL 구성 · [실측] `env:enrich_uplink` 편입(PR #64, 6.7) · ⚠️ **상수 주소 정정은 신규가 아님**(2026-09-17 PoC-(51) 단서가 이미 등재) · 🔴 **트리거 충족 ≠ 재판정 수행**이라 미결 유지 · **값 · 배율 · 방식 확정 0건**.
- **E4 — 카테고리 21 현재값 갱신 (취소선 0개)**: 🔴 **위임은 「여전히 2」에 취소선을 지정했으나 MCP가 걸지 않았다 — 의도적 이탈이며 근거를 문서 본문에도 명기**했다. 근거 = 그 줄은 **날짜가 박힌 재확인 단서**(2026-09-12 PoC-(46))이고 **그 시점에는 참**이었다. 이 파일은 **사실인 서술에 취소선을 걸지 않는다**(33.7(d) · 33.6(b) 선례, **학습 16 = 기존 컨벤션 우선**). 대신 **현재값 갱신 bullet**을 신설해 2 → **3**을 등재하고 「2를 현재값으로 읽지 말 것」을 명기했다. 동반 등재 = **대조군**(나머지 키 전부 1) · **값 미출력 원칙** · 🔴 **해소 방침 미확정 유지** · 🔴 **MCP 재실행 0회**(학부생 로컬 셸 실측분, 33.11이 원본).
- **E5 — 8.7(a) 미정정 건수 갱신 + PR #67 등재 (취소선 1쌍)**: 미결 제목의 **「같은 계열 미정정 2건」에만** 취소선 + **→ 1건**. 🔴 **E4와 판정이 갈린 이유** = 미결 제목은 **현재 상태 서술**(8.5(i)가 「4.5필드」에 취소선을 긋고 「3.5필드」로 이은 선례와 동형)이고 E4의 대상은 **날짜 박힌 실측 단서**다 — **두 축은 다르다**. append 구성 = ✅ **① 해소**(PR #67 `5868c71` · 내부 커밋 `9ecdff3` · 52자 → 64자 · 1파일 +1/−1 · Type `🐛 Fix` · 게이트 비특정 · 결정 근거 3 · 검증 2축 + NC 3단 + lint·build·ssr-nc · ★ NC 2축이 위임보다 정밀했던 경위 · 🔴 `ssr-nc` 도달 불가 = 8.4(f) 한계 ④) / 🔴 **② 미해소**(`SystemHealthSummaryCard` 사용자 결정 — 8.5(i) heartbeat 통합과 **동시 처리**, 한 번에 한 변수) / 🔴 **`enrich_status="skipped"` 3경로 표**(아래). **미결 자체는 유지**(①만 닫히고 ②가 남아 1건).
- **E6 — 33.11 신설 (파일 말미 append, 69줄)**: 제목에 **신설일 · 발견일 = 반영일 = 2026-09-20 · 근거유형 3분 · 계수 단위**를 병기했다(형식은 **33.10 · 33.7 실물을 읽고** 따랐다 — 발명 0). (a) 조사 방법 / (b) 분류 결과 / (c) B 6건 성격 분해 / (d) C 3건 / (e) 한계 / (f) 방법론 자산 **6항**. 항목마다 **근거유형(실측 / 논증 / 문서 인용) 개별 병기**했고 **한 문장에 섞지 않았다**.

### 등재 내용 요약 (사실 6건)

- **[실측 코드 대조] 1 · 9.1 Stage B 차단 사유가 stale이다**: `initToFMotionIndicator()`가 `firmware/src/tof_common.cpp`에 **완전 구현**(`vl53l5cx_motion_indicator_init(tofImager.Dev, &motionConfig, VL53L5CX_RESOLUTION_8X8)` + `vl53l5cx_motion_indicator_set_distance_motion(…)` + status 분기 + 실패 시 Stage A 계속)돼 있고 `platformio.ini`의 **env 5개**(`tof_dummy` · `mic_uplink` · `mic_noiseprobe` · `camera_probe` · `enrich_uplink`)에 편입돼 있다. **대조군** = 같은 grep이 라이브러리 원본(`firmware/.pio/libdeps/…`)과 호스트 스텁(`firmware/tools/host_stubs/…`)을 **별개로** 잡았다 ⇒ 도구 생존 · 혼동 0. 🔴 **같은 절이 2026-08-12에 이미 반증**하고 있었다(절 내부 드리프트). 🔴 **미결 유지.**
- 🔴 **[위임 전제 오류 catch — 학습 19] 「실제 잔존 축 = B-2 판정 융합·임계값 확정」은 실물과 어긋난다**: **9.4**가 **2026-09-02 PoC-(38) PR #39 `a0953fb`**로 B-2 판정 계층을 실코드화하며 `TOF_MOTION_NDET_MIN = 1` · `TOF_MOTION_LATCH_FRAMES = 75`를 **채택 확정**했고 **④런타임 실측(9.4(d))**까지 마쳤다. ⇒ **[논증] 실제 잔존 축은 검증 축** — **9.4(e) 프로토콜 ②(정지 사물 대조) 미수행** · **9.3(G) 벽면 실사용 정확도 미측정**. **위임 문구를 그대로 옮겼으면 날조가 됐을 자리**다.
- **[실측 + 논증] 2 · 7.7(l) 재판정 트리거가 충족됐다**: `firmware/src/uplink_common.cpp`가 `/api/v1/enrich` URL을 구성하고 `env:enrich_uplink`가 **PR #64로 편입**됐다(6.7) ⇒ *「2차 클라이언트 작성 시점」*이 **이미 왔다**. ⚠️ **상수 주소 정정은 신규가 아니다**(2026-09-17 PoC-(51) 단서 기등재). 🔴 **타임아웃 값 · 해결책 신설 0 · 미결 유지**(PR-C 소관).
- **[실측 — 학부생 로컬 셸, 계수 단위 = 줄 수] 3 · `.env` 키 중복이 2 → 3으로 늘었다**: `grep -c '^DDINGDONG_CAPTURE_URL_BASE' server/.env` = **3**. 나머지 키(`NCP_CLIENT_SECRET` · `NCP_CLIENT_ID` · `KAKAO_REST_API_KEY` · `KAKAO_REFRESH_TOKEN` · `KAKAO_CLIENT_SECRET` 등)는 **전부 1**(대조군). 🔴 **값 미출력 · 키 이름만 집계.** ⇒ **미결이 정지가 아니라 증가 중**이며 **해소 방침은 미확정 그대로**다.
- **[실측 + 논증, 계수 단위 = 선별 건수] 4 · C 분류 3건이 전부 「마지막 ④런타임 단계」만 하드웨어다(3/3)**: **26.10(d)** 실모델 산출물(`ml/models/yamnet/best.keras` · `ml/models/yamnet/inference_savedmodel/` · `server/venv_real` · `server/app/model_serving.py` · `server/tools/gate_axis_sweep.py`)이 **워킹트리에 전부 실재**하고 `.gitignore`(`ml/models/` · `*.keras`) 때문에 **`git ls-files ml/models` = 0**이라 「repo에 없다」로 오판하기 쉬운 자리다 / **8.5** `WiFi.RSSI()` 취득은 펌웨어 **5파일 · 9줄**에 이미 있고 부재한 것은 **우리 서버와의 전송·수신 계약**(`server/app/` 전역 `rssi` grep **0건**, `routes.py`는 `"online"` · `"strong"` 리터럴) / **9.1** B-2는 확정·실측 완료라 잔존은 검증뿐.
  - ⚠️ **[학습 19 catch 2건째] 위임의 「시리얼 로그 한정」은 실물보다 좁다** — `main.cpp`의 `buildJsonPayload()`가 `doc["rssi"] = (int)WiFi.RSSI();`로 **JSON 본문에 실어 POST까지 한다**. 🔴 **다만 대상이 `HTTP_TARGET_URL = "https://httpbin.org/post"` = 에코 스파이크이지 우리 서버가 아니므로 결론(계약 부재)은 무변경**이다 — 27.8(m)④ 「결론은 옳고 근거 문구가 어긋남」 계열.
- **[실측 코드 대조, 계수 단위 = 판정 분기 수] 5 · 2차 알림 부재 경로는 3개다**: `server/app/utils.py` `_apply_prediction_policy()`에서 `enrich_status="skipped"`로 가는 분기 = `confidence < CONFIDENCE_THRESHOLD`(`primary_sent` **False** / `"low_confidence"`) · `predicted_class == "fire_alarm"`(**True** / `None`) · `tof["applied"] and not tof["passed"]`(**False** / `"tof_rejected"`). ⚠️ **8.7(a)는 `fire_alarm` 축만 적고 있었다.** ⚠️ **`fire_alarm`만 1차가 간다 — 나머지 둘은 1차도 안 간다. 섞어 읽지 말 것.** ★ **이 사실이 PR #67의 「게이트 비특정」 제약을 코드 쪽에서 뒷받침한다.**
- **[실측 `git show` + 사용자 결정] 6 · PR #67 등재**: 머지 **`5868c71`**(2026-09-20), 내부 커밋 **`9ecdff3`**, Type = **`🐛 Fix`**, **1파일 +1 / −1**. `dashboard/src/pages/HelpPage.tsx` FAQ 2번 `a` 앞머리에 조건절 **「2차 알림이 갈 때는」** 추가(**52자 → 64자**), **게이트 종류 비특정**. 결정 = **최소 개입안**(PR #66과 동형, 결정 주체 = 사용자, 결정일 2026-09-20) — 근거 ① 화면 내부 자기모순 ② 사용자 대면 약속 ③ 5060 페르소나(8.3) 가독성. 🔴 **`SystemHealthSummaryCard.tsx` 무접촉 = 사용자 결정** — 8.5(i) heartbeat 통합과 **동시 처리**(카피만 먼저 고치면 8.3 안심 카드 전제 재결정을 앞당겨 버린다).

**SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 위임 §6 8항 예외 적용)**

- **V1 취소선·볼드 파리티** — `decisions.md` 착수 전 **취소선 포함 줄 100 / 마커 출현 252 / 볼드 마커 출현 11,022** → 종료 **101 / 256 / 11,506**(계수 단위 = 줄 수 / 마커 출현 수). 취소선 **+4 = 신규 2쌍**(E1 하나 · E5 하나 — E2 · E3 · E4 · E6은 취소선 0개)이고 **짝수**. 볼드 **+484 = 242쌍**, **짝수**. ⚠️ 본 엔트리 본문은 **취소선·볼드 마커를 리터럴로 쓰지 않고 서술로 풀어 썼다**(2세션 연속 계수 오염 재발 방지). ⇒ 마크다운 무파손.
- **V2 삭제 0줄 증명** — `git diff --numstat docs/decisions.md` = **+106 / −2**. 제거 2줄은 **같은 줄의 치환 결과**(E1 · E5 — in-place)이고 E2 · E3 · E4 · E6은 **순수 삽입**이라 제거가 없다(`git diff -U0` 훅 헤더 4개가 `,0 +` 형태의 삽입, 2개가 1:1 치환). **줄 삭제 0.** 토큰 보존 = **정규화 기준(취소선 · 볼드 · 백틱 마커 제거) 손실 0토큰** / **원시 기준 손실 2토큰**(`2건` · `미착수.`) — 둘 다 **취소선 마커가 토큰 경계를 갈라놓은 마크업 산물**이며 **내용어 손실 0**이다.
- **V3 앵커 `count == 1` + 실물 되읽기** — 물리 편집 지점 **6곳 전건 `str.count` == 1 assert 통과**(불일치 시 `sys.exit` abort 설계, 실제 abort 0회). **라인 번호 앵커 사용 0건** — 전부 패턴 문자열로 잡았다(본 엔트리 본문의 라인 번호 인용도 0건). 신설 절 append는 파일 말미 문자열 **endswith assert**로 고정했다. 편집 후 **6곳 전건 diff 실물 재조회**로 구간 확인했고, 그 되읽기에서 **중첩 볼드 1건**(미결 제목 안의 `→ 1건`)을 발견해 **8.5(i)의 건수 갱신 선례(취소선 뒤 화살표로 새 값을 잇되 내부 볼드를 쓰지 않는다)대로 내부 볼드를 제거**했다.
- **V4 🔴 한글 전수 검증** — 추가분에서 **착수 전 파일에 없던 음절 = 3개**(`낡` **U+B0A1** · `봐` **U+BD10** · `쉬` **U+C26C** — 전건 정상 음절, 문맥 출력 확인). 「신규 음절 소수」가 충분조건이 아니므로 **신규 한글 단어 176개를 전수 추출해 문맥 육안 통독** = 가고 / 가는 / 가려진다 / 갱신하며 / 건뿐이다 / 걸린 / 걸지 / 검증했다 / 검출력을 / 경로만 / 계획으로 / 고치라고 / 고치면 / 고칠지 / 구조본 / 구현돼 / 권고가 / 규범은 / 글자로 / 날짜가 / 낡았다 / 낡은 / 낡지 / 남았다 / 내부다 / 높으나 / 단계다 / 단언이나 / 대상이라 / 대조도 / 대조뿐 / 대조했다 / 덩어리가 / 데이터만 / 뒷받침한다 / 들어간 / 로깅용 / 리터럴이 / 말을 / 말지 / 맡기되 / 매치했다 / 명기했듯 / 명시하는데 / 문안도 / 문장과 / 문장만 / 문형 / 뭉뚱그려진 / 바꾼 / 박힌 / 반증하고 / 반증하면서도 / 반환값은 / 반환했다 / 발견한 / 발견해 / 방문자가 / 방향도 / 벌일 / 보여 / 봐도 / 부재한 / 부족하면 / 블랭킹만으로는 / 비추적 / 비특정 / 빠뜨렸을 / 뻔한 / 뿐이고 / 사실이다 / 사실인 / 사유의 / 사진과 / 상속되는 / 생존을 / 생존이며 / 서버와의 / 설치본이 / 소재를 / 수행된 / 쉬운 / 스파이크이지 / 시점에는 / 신규가 / 신규로 / 신선도만 / 실물보다 / 실상속값 / 실증했다 / 실코드화하며 / 심볼이 / 아래가 / 아무리 / 않았고 / 앞당겨 / 앞머리에 / 약속하면 / 양식을 / 어긋나거나 / 에코 / 예다 / 오작동이 / 오판하기 / 워킹트리에 / 워킹트리에는 / 위에서만 / 위임보다 / 유령이 / 의존이라 / 이번에는 / 임계값도 / 잔존은 / 잡는지 / 재결정을 / 재결정하라 / 재계수하면 / 재실측도 / 재실행하지 / 재판정이 / 재현값 / 재현됐고 / 재현이 / 재현하지 / 적기 / 정밀했다 / 정책이라 / 제약을 / 조건절 / 종으로 / 죽이지 / 지시한 / 지정했으나 / 집계에서 / 쪽에서 / 쪽이다 / 찍은 / 차단의 / 차단하므로 / 착수하라는 / 채운 / 채웠다 / 충족된 / 취득은 / 카메라가 / 카피만 / 코딩 / 타는 / 통합과 / 특정했다면 / 파일이라 / 판정됐다고 / 판정이다 / 판정이지 / 펌웨어측 / 편입돼 / 편입됐다 / 편입한 / 표지하는 / 픽스처 / 하드웨어다 / 하라 / 해결책을 / 해서 / 해소책 / 허가는 / 현재값만 / 현재값으로 / 현재값이다 / 호출부가 / 호출부는 / 혼동하지 / 확장한 / 확정이며(※ 「1차도」 · 「2차만」은 숫자 뒤 어절이라 추출기가 갈라 낸 조각) — **전건 정상 단어 · 모지바케 0**. 바이트 보강 = UTF-8 **strict decode 통과** · **`U+FFFD` 0건** · **제어문자 0건**(`\n` · `\t` 제외) · **NFC 정규화 동일**.
- **V5 대조 원본** — 등재 수치·식별자 중 **`server/.env` 키 중복 3**과 **전수 발굴 집계**는 **학부생 로컬 조사분**이며 **위임 본문이 원본**이다(§3 · §7 지정 — MCP는 `server/.env`를 **읽지도 재실행하지도 않았다**). 본 repo 측 대조는 MCP가 실측했다: 착수 HEAD `5868c71` · 워킹트리 clean · §4 grep 4건 **각각 1** · 절 번호 `^### 33\.11` 사전 grep **0건** · `_apply_prediction_policy()` 3경로 · `initToFMotionIndicator()` 실물 · `+<tof_common.cpp>` env 5개 · `/api/v1/enrich` URL · `WiFi.RSSI()` 5파일 9줄 · `git ls-files ml/models` = 0 · `git show 5868c71` 1파일 +1/−1 · 내부 커밋 `9ecdff3` 실존 · 「관련」 줄 절 번호 **전건 grep 실존 확인**(9.3 · 9.4(e) · 7.7(l) · 6.5 · 6.7(c) · 7.6(i) · 8.5(i) · 8.7(a)(c) · 26.10(d) · 27.8(m) · 33.6(c) · 33.7(i) · 33.10 · 카테고리 21). **로그·실측 없는 수치 0건 기재.**
  - 🔴 **재현 불일치 1건을 숨기지 않고 등재했다** — 위임의 「문형 매치 인스턴스 **88**」을 MCP가 동일 문형 3종으로 재계수하면 **86**이다(Δ2). **매치 줄 80 · 매치 절 33은 일치**한다. **위임에 문형 정의가 미기재**라 동일 재현이 불가능하므로 **양쪽 값을 병기**하고 33.11(e)에 **한계로 명기**했다. **차이의 원인은 확정하지 않았다.**
- **V6 git status** — `docs/` **2파일만**(`M docs/decisions.md` · `M docs/decisions-log.md`). `git add`는 두 파일만 명시 스테이징. 시크릿 스캔(토큰 · 자격증명 · SSID · IP · `.env` 값 패턴) **0건** — `.env` 항은 **키 이름만** 실렸다.

**비범위**: 🔴 **코드 전체 무수정 · 무커밋** — `firmware/` · `server/` · `dashboard/` · `ml/` **전부 읽기만**(수정 0). `docs/git-convention.md` **무접촉**. **노션 무접촉**. 🔴 **repo 밖 무접촉** — `server/.env` **읽지 않았다**(§3 지정, MCP 재실행 0회). 🔴 **어떤 미결도 닫지 않았다** — 9.1(h) Stage B · 7.7(l) · 카테고리 21 `.env` 중복 · 8.7(a) ② **전건 미결 표기 유지**(8.7(a)는 ①만 닫혀 **2건 → 1건**이며 미결 자체는 존속). **대응 방향 · 해소책 · 권고 · 임계 · 타임아웃 값 · 카피 문안 신설 0건**(§7 불변식). 신규 미결 **0건 신설**. 이전 회차 미결(33.7(e) · 33.7(i) · 33.6(a) · 6.2 G29 750ms · 8.7(c) `tailwind-merge` · 카테고리 20 `.claude/` 방침 등)도 **전건 무접촉**. 브랜치 · PR **생성 0**(문서 전용 = `main` 직 push).

**학습 적용**: **학습 13**(출처 catch — 인용 전 grep. §4 grep 6건을 착수 전 실행해 앞 4건이 **각각 정확히 1**임을 확인했고, 「관련」 줄 절 번호는 **기재 전 전건 grep 실존 확인**. 편집 앵커 6곳 전건 `count == 1` assert, **라인 번호 앵커 0건**. 위임이 제시한 집계 6종도 **재현 가능한 3종만 재계수**해 일치 2 · 불일치 1을 **그대로 병기**했다) / **학습 16**(기존 컨벤션 우선 — 위임은 카테고리 21 「여전히 2」에 **취소선을 지정**했으나, 그 줄이 **날짜 박힌 실측 단서**라 이 파일의 「사실인 서술에 취소선 = 날조」 컨벤션(33.7(d) · 33.6(b))과 충돌했다. **컨벤션을 택하고 현재값 bullet으로 대체**했으며 그 판정을 문서 본문과 본 엔트리 양쪽에 명기했다. 반대로 8.7(a) 미결 제목의 건수 갱신은 **8.5(i)의 `4.5필드 → 3.5필드` 선례가 취소선을 쓰므로** 선례대로 걸었다 — **두 축을 가른 것이 본 학습의 적용**이다) / **학습 19**(근본원인 재검증 — **catch 2건**. ① 위임의 「실제 잔존 축 = B-2 판정 융합·임계값 확정」이 **9.4와 정면으로 어긋났다** — B-2는 2026-09-02에 확정·실측 완료였고 잔존은 **검증 축**(9.4(e) · 9.3(G))이었다. 위임 문구를 그대로 옮겼으면 **날조**가 됐다. ② 위임의 「`WiFi.RSSI()`는 시리얼 로그 한정」이 **실물보다 좁았다** — `main.cpp`가 JSON에 실어 POST까지 하나 **대상이 에코 스파이크**라 결론은 무변경이다. **9.4(f)의 「위임 전제 오류 2건」 선례와 동형**으로 등재했고 **정지 없이 진행**한 근거는 §9 정지 트리거(가정 1~4 X)가 **미발동**이었다는 것이다) / **학습 21**(미결·차단 사유가 유령일 수 있음 — 본 Set이 **그 학습의 문서 전역 확장판**이다. 개별 건에서 발견된 실패 양식을 **문형 전수 스캔 → 16건 실물 대조**로 넓혔고, 결과를 **A 5 / B 6 / C 3 / 판정 불가 2**로 갈랐다. 🔴 **B 6건을 한 덩어리로 쓰지 않았다** — 문장이 낡은 것은 **2건뿐**이고 나머지 4건은 **범위 밖 미확인 1 · 문서가 이미 축 전환 기록 1 · 정책 성격 1 · 같은 날 채운 것 1**로 **성격을 개별 명기**했다. 다음 세션이 4건을 「지금 가능」으로 오독하지 않게 하는 것이 본 절 (c)의 존재 이유다)

**관련 카테고리**: **33.11 (신설 — 차단 사유 실물 검증 전수 발굴 · 분류 A/B/C · 방법론 3종 · 한계)** / 9.1(h) (Stage B 차단 사유 stale 정정 · 미결 유지) / 9.3 · 9.4 (B-1 · B-2 실측 — 실제 잔존 축의 근거) / 7.7(l) (재판정 트리거 충족 · 미결 유지) / 6.5 · 6.7 (2차 클라이언트 · `env:enrich_uplink`) / 7.6(i) (네트워크 타임아웃 재현 — B-④) / 8.5(i) (`device_status`/`signal_strength` · heartbeat 트리거 — C · `SystemHealthSummaryCard` 동시 처리 근거) / 8.7(a) (PR #67 등재 · 3경로 · 미정정 2건 → 1건) / 8.7(c) (`tailwind-merge` — B-⑤ 정책 성격) / 26.10(d) (진입점 2 — C) / 27.8(m) (드리프트 계열 — 절 내부 판 · 「결론은 옳고 근거 문구가 어긋남」) / 33.6(c) (실모델 구동 이력) / 33.7(i) · 33.10 (부재 판정의 범위 경계 — B-⑥) / 카테고리 21 (`.env` 키 중복 현재값 3) / 8.3 (5060 페르소나 · 안심 카드 전제) / 8.4(f) (`ssr-nc` 한계 ④ 도달 불가) / 학습 13 · 16 · 19 · 21.

**관련 commit**: `5868c71`(PR #67 머지, **착수 시점 HEAD** — 본 Set의 등재 대상) · 내부 커밋 `9ecdff3`(같은 PR) · 인용만 한 선례 = `765624b`(PR #66, FAQ 1번 동형 정정) · `a0953fb`(PR #39, Stage B-2 판정 계층 확정) · `84f2272`(PR #36, Stage B-1 계측 계층). 문서 반영 = 본 커밋(docs-only, **PR 없음, `main` 직 push**).

## 2026-09-21 (월) — PoC-(55) Set 1 — 🔴 33.7(e) 누수 대응 방향 ① 확정 · 33.6(a) OOD 대응 방향 `other` 신설 확정 + split 정책 정비 등재(33.12 신설) · OOD 대응 방향 등재(33.13 신설) · 네거티브 후보 선별 등재(33.14 신설) + 8.7(c) tailwind-merge 해소 · 정정 5건 · 신규 미결 1건 + 5.2(g) 대기 6항목 분해 · 카테고리 5 sample_weight 부분 확정 + 27.8(p) 위임·판단 오류 7건 (카테고리 5/5.2/8.7/26.10/27.8/33.6/33.7/33.12/33.13/33.14)

2026-09-21 하루에 **PR 3건**(#68 · #69 · #70)이 머지됐고 **사용자 결정 10건**(D1~D6 · E1~E4)이 나왔다. 전부 SSoT 미등재였다. 본 Set은 `docs/decisions.md`에 **33.12 · 33.13 · 33.14를 신설**하고 **8.7(c-1)** · **27.8(p)** 두 소절을 신설한 뒤 기존 **10곳**(카테고리 5 머리 · 5.2(b)(c)(f)(g) · 8.7(c) 2곳 · 26.10(d) · 33.6(a) · 33.7(e)(g)(h)(i))을 갱신하고 본 엔트리를 남긴 것이 전부다(**문서 2파일 · 앵커 치환 15곳 + 파일 말미 append 1곳 = 물리 편집 16곳 / 등재 항목 18건**). 🔴 **오래 열려 있던 미결 2건의 「대응 방향」이 닫혔다** — 33.7(e)(2026-09-15 신설)와 33.6(a)(2026-09-12 신설)이며, **닫힌 것은 방향이고 실행은 전부 재학습 소관**이다. **합격 수치 · 임계 · 권고 신설 0건**이고, 등재한 규칙은 **사용자가 2026-09-21에 확정한 운영 규칙 1건**(파이프라인 실데이터 재실행 = 재학습과 한 세트, 33.12(e))뿐이다.

**Step 0 가정 대조 — 6항 전건 O(불일치 0건)**: 착수 시점 HEAD **`a06599d`** · `main` · 워킹트리 **clean**(`git status --porcelain` 무출력). ① 8.7(c)에 잔존 서술 문장 `str.count` = 1이고 그 문장이 잔존을 `NotificationTof` 1곳으로 적음 ② 8.7(c) 해법 줄에 사후 실측 grep 문장 `str.count` = 1 ③ 33.7(e) 말미 문장 · 33.7(i) 말미 문장 각각 1 ④ 33.6(a) 말미 문장 1 · 26.10(d) 대응 방향 대기 문장 1 ⑤ 카테고리 33 최대 절 = **33.11**(`^### 33\.[0-9]+` 실측) · 27.8 최대 소절 = **(o)** ⑥ `decisions-log.md` 마지막 엔트리 = **2026-09-20 (일) 후속 — PoC-(54) Set 1 후속 2**. ⇒ **§9 정지 트리거 미발동**(앵커 18곳 전건 1 · 정정 대상 문구 전건 실존 · 한글 깨짐 0 · PR 본문과 설명 안 되는 어긋남 0 · 워킹트리 clean).

**요일 검산**: `date -j -f "%Y-%m-%d" "2026-09-21" "+%A"` = **월요일** ⇒ 헤더 `(월)` 정합.

**신설 절 번호 결정 (근거유형 = 실측 grep)**: **33.12 · 33.13 · 33.14**. 근거 = `^### 33\.[0-9]+` 최대 절이 **33.11**이고, 세 절 모두 **카테고리 33(ML 파이프라인 · 학습 · 데이터 감사) 계열**이라 말미 append가 자연스럽다. 충돌 검사 = `^### 33\.1[2-4]` 사전 grep **0건**. **기존 절 순서 무변경**이다. 🔴 **(C)를 (B)와 같은 절의 소절로 넣지 않고 별도 절로 뗀 이유** = 위임 §4가 양쪽을 허용했고, **33.12는 코드가 머지된 구현 절**이고 **33.13은 코드 0줄의 결정 절**이라 **성격이 다르다**(33.6 「측정 결과」 vs 33.8 「수단·자산」을 가른 선례와 같은 축). 33.14는 위임 §4 지정대로 신설 절이다.

**소절 문자 결정**: 8.7은 (a)~(d)가 차 있고 본 등재가 **(c)의 해소**라 **(c) 바로 뒤 (c-1)**로 넣었다(제자리 해소 — 위임 §4 지정). 알파벳 (e)로 밀면 (c)와 물리적으로 떨어져 다음 세션이 (c)만 읽을 위험이 있다. 27.8은 최대 소절이 **(o)**라 **(p)** 이어 붙였다.

**엔트리 형식 · 날짜 처리 판정 (근거유형 = 실물 형식 인용)**: `2026-09-21` 엔트리는 **본 건이 처음**이라 「후속」 접미가 필요 없다 — 같은 파일의 첫 엔트리는 전부 `## <날짜> (<요일>) — PoC-(N) …` 형식이다. 회차는 **PoC-(55)**이며 **새 채팅방 기준**이다(직전 인계 제목의 다음 회차 예고와 혼동하지 않는다 — 27.8(o)① 회차 라벨 오기 선례를 의식한 판정이다).

**find-skills**: `"decision log"` **4건**(cxo-briefing-kit · design-decisions · game-scoring · pursuit-governance) / `"markdown section editing"` **0건** / `"korean text validation"` **0건**. 배치 전건 0이 아니므로(1건이 4건을 반환) **도구 생존이 이미 확인**돼 대조 질의는 불필요했다. **채택 0건** · install 0 · info 0. **미채택 사유** = design-decisions · pursuit-governance · cxo-briefing-kit는 **② 워크플로 불일치**(전부 「결정 로그를 새로 만들거나 임원 보고용으로 포장」하는 스킬이고, 본 Set은 **이미 있는 SSoT 2파일을 앵커 기준으로 국소 편집**한다 — 형식 발명 금지가 불변식이라 정반대다) · game-scoring은 **② 워크플로 불일치**(후보 점수·softmax 도메인) · 설치 수 · 출처 기준으로도 **① 미달**(전건 0 installs · 0 stars). 나머지 2질의는 **③ 검색 결과 0건**.

### 편집 건별 (앵커 15곳 전건 `str.count` == 1 assert 통과 후 치환, 불일치 시 스크립트 abort, 라인 번호 앵커 0건 / E16~E18은 파일 말미 append 1회로 묶임)

- **E1 — 8.7(c) 해법 줄 계수 단위 오기 정정 (취소선 1쌍)**: 「6곳(계수 단위 = 출현 줄 수)」에서 **단위 이름에만** 취소선을 걸고 「실사용 줄 수」로 이었다. 값 **6은 틀리지 않았다** — `git grep`을 PR #57 머지 시점 커밋에서 다시 돌리면 **7줄**이고 그중 1줄이 주석이다. 현재 HEAD 값(**8줄 / 실사용 6**)도 병기했다.
- **E2 — 8.7(c) 잔존 줄 정정 3건 (취소선 3쌍)**: 「1개 병합 지점」 · 「심각도 낮음」 · 「수정 여부 = 사용자 판단 대기」 **세 조각에만** 취소선. 🔴 **「3분기 전부 소실을 in-session 재현했다」는 실측 문장에는 걸지 않았다** — 그 관측은 참이고 좁았던 것은 범위다(33.7(d) · 33.6(b)의 「틀린 것이 아니라 범위가 좁았다」 컨벤션). 하위 bullet로 정정 3건(잔존 4곳 + 변종 2 / 인용 이름 오기 / 심각도 등급)을 명기했다.
- **E3 — 8.7(c-1) 신설 (취소선 0개)**: (c) 바로 아래 소절 신설. 결정 경위(A안 철회 → B안) · 소실 지점 전수 표 6행 · 구현 · 🔴 기본 그룹을 피한 이유(악화 10건 · 거짓 충돌) · 검증(차이표 28행 중 8 · 토큰 677종 · NC 2건 · 빌드 산출 바이트 동일) · 주석이 CSS를 늘린 catch · ⚠️ 브라우저 실측은 C2 1곳뿐 · 🟡 신규 미결(토큰 목록 이중 관리) · 잔존 한계 3건.
- **E4 — 카테고리 5 머리 sample_weight 부분 확정 (취소선 1쌍)**: 「구현할지 서술을 실물에 맞출지 = 사용자 판단 대기」에만 취소선. 🔴 **부분만 닫혔다** — D6이 정한 것은 「서술을 실물에 맞춘다」이고, `config.py` 주석은 **지금도 「구현할지…는 사용자 판단 대기」라 적는다**(실측). 그 경계를 문장으로 명기했다.
- **E5 — 5.2(b) 부분 해소 append (취소선 0개)**: D3(유닛 그룹 키) · D5(외부 평가 전용). 🔴 웹 수집분 명명·pitch 범위 자체는 **미확정 유지**.
- **E6 — 5.2(c) 메커니즘 해소 append (취소선 0개)**: 🔴 **위임은 (c) 정정을 지시하지 않았고 MCP도 표에 취소선을 걸지 않았다** — 그 표와 4개 관측은 **2026-09-17 시점 코드에 대한 실측 이력**이며 틀린 값이 아니다(학습 16 = 기존 컨벤션 우선, 27.8(o)·PoC-(54) E4 선례와 같은 판정). 대신 메커니즘이 사라졌다는 사실과 새 배정의 변경 규모를 append했다.
- **E7 — 5.2(g) 대기 6항목 분해표 신설 (취소선 0개)**: ✅ 3 / 🟡 1 / 🔴 2. 장치 설치 위치는 **결정 대상에 들어가지 않았다**는 사실을 따로 못박았다.
- **E8 — 5.2(f) 원인 규명 append (취소선 0개)**: 0바이트 30건이 덤프 0바이트 집합에 30 / 30 포함.
- **E9 — 33.6(a) 대응 방향 확정 (취소선 1쌍)**: 말미의 「대응 방향은 여전히 미확정이다」 **한 문장에만** 취소선. 🔴 그 앞의 2026-09-20 `smoke_detector` 단서는 **무변경**이며, 보류 판정을 함께 적었다.
- **E10 — 33.7(e) 대응 방향 ① 확정 (취소선 1쌍)**: 🔴 **취소선 범위를 「방향은 위 ①②③ 미확정」이 붙은 문장으로 좁혔다** — 같은 문장의 「누수의 존재 자체가 재학습 전 해소 대상」은 **지금도 참**이라 새 bullet에서 그대로 유지한다고 명기했다. ②③가 채택되지 않았다는 것과 수치 무변경을 함께 등재했다.
- **E11 — 33.7(g) 0바이트 30건 원인 append (취소선 0개)**.
- **E12 — 33.7(h) 한계 ⑧ 부분 해소 (취소선 1쌍)**: 「fsd50k 40,966 wav 미감사」에 취소선 + 0바이트 축만 닫혔음을 명기. 🔴 **md5 중복 감사는 여전히 미수행**.
- **E13 — 33.7(i) 부분 닫힘 판정 append (취소선 0개)**: 위임 §4가 요구한 판정이다. **2축 중 1축**만 닫혔다 — D1이 split 단계에서 걸러 `01_clips` 수작업이 불필요해졌으므로 **파급 축은 닫히고**, 「조각내기 코드를 본 repo에 둘지」라는 **처리 방침 축은 미결 유지**다. 🔴 **「본 repo에 코드가 없다」는 사실 서술 무변경**(위임 지정 준수).
- **E14 — 26.10(d) OOD 파급 줄에 포인터 (취소선 1쌍)**: 「대응 방향 = 사용자 판단 대기」에만 취소선. 「두 문제를 한 번에 건드리지 말 것」은 **무변경**.
- **E15 — 27.8(p) 신설 (취소선 0개)**: 오류 7건 ①~⑦ + (d) 누적 숫자 무접촉 고지. 🔴 **(d)의 누적 21건은 건드리지 않았다** — 합산 여부가 (i)에 사용자 판단 대기로 등재돼 있다((o) 말미와 같은 판정).
- **E16 — 33.12 신설 (파일 말미 append, 취소선 0개)**: (a) 결정 5건 · (b) 제거 내역과 **제거 순서가 규칙의 일부** · (c) 새 배정과 변경 규모 · (d) 검증과 **NC-2 함정 관통 4조합** · (e) 🔴 운영 규칙 + 33.7(i) 파급 축 닫힘 · (f) 한계 4건.
- **E17 — 33.13 신설 (취소선 0개)**: (a) D4 · E1~E4 표 · (b) 보류 3건 · (c) 착수 전 비용. 🔴 **`not_target`은 신규 어휘이며 계약 PR에서 확정**임을 못박고 8.7(a)의 3분기 표와 **별개 축**임을 명기했다.
- **E18 — 33.14 신설 (취소선 0개)**: (a) 1차 정지와 Zenodo 반입 · (b) 부모를 제외 기준으로 쓰지 않은 근거 · (c) 🔴 0바이트 발견 2건 · (d) 결과와 합계 검산 · (e) 결함 2건과 청취 검수 기호 반전 · (f) 한계 7건.

### 등재 내용 요약 (사실 · 결정 10건)

- **[사용자 결정] 1 · 33.7(e) 대응 방향 = ①(중복 제거 후 재split · 재학습)**: ②(누수 고지 병기) · ③(청정값 정본 교체)는 채택되지 않았다. 🔴 **확정은 방향이고 실행은 재학습 소관** — 33.2 · 33.6 · 33.7의 등재 수치와 청정값은 **재학습 전까지 전건 무변경**이다.
- **[사용자 결정] 2 · 33.6(a) 대응 방향 = 4번째 클래스 `other` 신설(D4)**: E1 기록·미발송, E2 게이트 맨 앞(펌웨어 변경 0), E3 로컬 FSD50K 우선, E4 지표 2종 확정·**합격 수치는 재학습 후**. 보류 3건 = `smoke_detector`(같은 계열 경보음이라 OOD 해결책 아님) · `fire_alarm` 연속 감지 확인(보드 실측 후) · 장치 설치 위치.
- **[실측, 계수 단위 = 클립 수] 3 · split 정비 시뮬레이션**: 고유 해시 **2305 / 2792**로 33.7(a) 재현, 제거 **493**(무음 5 · 교차 10 · 중복 478) → 잔존 **2299**, 새 배정 **train 1606 / val 358 / test 335**, 내용 누수 **0 그룹**. 🔴 **제거 순서가 규칙의 일부다** — 중복을 먼저 돌리면 교차 규칙이 무력화돼 잔존이 **2304**가 된다.
- **[실측, 계수 단위 = source 수 / 클립 수] 4 · 현 매니페스트 대비 변경 규모**: source `doorbell` **87** · `knock` **170** · `fire_alarm` **130**, **test 클립 424 → 335**. ⇒ 5.2(c)가 예고한 얽힘의 실측판이며 **갱신은 재학습 소관**이다. 데이터셋은 **1바이트도 바뀌지 않았다**(파일 수·mtime 3회 대조).
- **[사용자 결정 + 논증] 5 · 🔴 운영 규칙 1건 신설**: **파이프라인 실데이터 재실행은 재학습과 한 세트**다. 재학습 전에 돌리면 새 test(335)에 현 모델이 학습한 클립이 섞여 **성적이 부푼다**. 재실행 시 33.2 · 33.6 · 33.7 수치와 게이트 축 스윕 기준값 갱신이 필요하며 **그 갱신은 재학습 소관**이다. **본 Set이 등재한 유일한 규칙**이다.
- **[실측 + 사용자 결정] 6 · 8.7(c) 해소 — 근본 원인은 설정 공백 1곳**: A안(1곳 치환)의 전제가 전수 조사에서 깨져(**잔존 4곳 + 변종 2곳**) 철회되고 B안(`lib/utils.ts` 1파일 / +24 −1, 호출부 0줄)이 확정됐다. 🔴 **기본 font-size 그룹에 넣었으면 악화 10건**이었다(거짓 충돌로 `leading-none` 소거) — 전용 그룹으로 **메커니즘 등가 치환**. 검증 = 차이표 28행 중 차이 8 · 악화 0 / 토큰 **677종** 전역 증명 / NC 2건 / 빌드 산출 **55,828 B 동일**. ⚠️ **브라우저 실측은 C2 1곳(18px)뿐**이다.
- **[실측] 7 · 8.7(c) 정정 5건**: 잔존 1곳 → **4곳 + 변종 2** / 「심각도 낮음 · 커지는 방향」 → **C4 · C6은 작아지는 방향**(5060 페르소나 기준 등급 상향) / grep 「6곳(출현 줄 수)」 → **실사용 6 · 출현 줄 7** / 인용 `info.className` → 실물은 **구조분해된 `className`** / PR #57의 우회 6곳은 **존치**(정리 금지).
- **🟡 [신규 미결] 8 · 커스텀 토큰 목록 이중 관리**: `index.css`의 커스텀 글자 크기 토큰 6개와 `lib/utils.ts`의 등록 목록 6개가 따로 존재하고 **자동 가드가 없다**. 🔴 **처리 방침 미확정 — 해소책을 적지 않는다.**
- **[실측, 계수 단위 = 파일 수] 9 · 0바이트 발견 2건**: 로컬 FSD50K 덤프에 **0바이트 wav 2,414개** ⇒ 실가용 **38,552**(33.7(h) 한계 ⑧의 0바이트 축 보강). 그중 `01_extracted`의 **0바이트 30개가 30 / 30 전건 포함** ⇒ **`00_source_raw`에서 이미 0바이트였고 조각내기 결함이 아니다**(5.2(f) · 33.7(g)의 원인 규명). ⚠️ **이름 대조 기준**이다.
- **[실측, 계수 단위 = 클립 수] 10 · 네거티브 후보 1,100**: 선택 라벨 **179종**, ⓐ213 · ⓑ200 · ⓒ544 · ⓓ143(13.0%), PP 1,053 / PNP 3 / NA 44, train 997 / val 103. 제외 target **842** · 사용 불가 **2,193** · 보류 **2,324** · 적격 풀 **35,607**, **합계 검산 40,966**. 🔴 **부모 라벨을 제외 기준으로 쓰지 않는다** — `Alarm` · `Door`로 제외하면 겨냥한 hard negative(`Telephone` · `Ringtone` · `Slam` · `Sliding_door`)가 통째로 사라진다. 청취 검수(1인 30건) **target 오염 0 / 30**이며 **기호 반전 1건을 확인 후 뒤집고 원 입력을 보존**했다.

**원본 대조 결과 (위임 본문 vs PR 본문 · repo 밖 로그 — 어긋난 것 전건 보고)**

- 🔴 **1 · 「데이터셋 무변경(A8 3회 대조)」의 근거 절 번호가 원본과 다르다**: PR #69는 그 증명을 **17.2**(검증 자산 전건 베이스라인)로 적고, PR #70은 **A5**로 적는다. **A8은 두 PR 본문 어디에도 없다.** ⇒ **원본을 택해** 33.12 서문에 **17.2** 축으로 등재했다(3회 대조 = 착수 전 ≡ 시뮬레이션 후 ≡ 최종이라는 **사실 자체는 PR 본문과 일치**한다).
- **2 · PR #70 본문은 보정 전후 두 벌의 집계를 싣는다**: 앞부분 표(후보 1,100 · 라벨 **184**종 · ⓐ206 · ⓑ212 · ⓒ538 · ⓓ144 · 보류 1,564 · 제외 ① 841 · 사용 불가 2,250 · PP 1,054 · train 994 / val 106)는 **1차 실행분**이고, PR이 스스로 *「아래 수치가 최신이다」*라 밝힌 보정 후 값이 **위임 본문과 전건 일치**한다. ⇒ **위임 수치 채택**이며 어긋남이 아니다. repo 밖 `summary.md`와도 전건 일치했다(범주 비율은 로그에만 있어 그대로 인용하지 않았다).
- **3 · 「결함 수정 2건」 중 tie-break(부모 라벨 `Animal` → 희소도 우선)는 PR 본문에 기재가 없다**: 🔴 **위임 본문이 원본**이며 `ml/curation/select_negatives.py`의 `classify()` 주석·구현(범주 우선 → 희소도 → 이름순)으로 **실물 확인**했다. 33.14(e)에 **출처를 병기**했다.
- **4 · 8.7(c)의 grep 수치는 위임이 맞고 SSoT가 틀렸다**: `git grep`을 PR #57 머지 시점 커밋에서 실행하면 **7줄**이고 실사용 6이다(현재 HEAD는 8줄). 위임의 「실사용 6 · 주석 포함 7줄」과 **일치**한다.
- **5 · PR #69 본문 표에 닫는 괄호 오타 1건**(`fire_alarm` 행의 실제 비율 칸)이 있으나 **수치는 다른 칸과 정합**하며 **등재 대상이 아니다.**
- **6 · 로그 열람 성공**: repo 밖 `~/ddingdong-측정결과/2026-09-21/negatives/`의 `summary.md` · `listen_qa.csv`(30행) · `listen_qa_note.md`를 **읽었다**(쓰기 0). 「로그 미열람」 해당 없음.

**미결 분해 (위임 §4 지정 — 전부 닫힘 / 부분만 닫힘 / 무접촉)**

- **33.7(e)** = **전부 닫힘**(대응 방향 축). 근거 = 사용자 결정 ① 채택. ⚠️ **실행·수치 갱신은 재학습 소관**이라 절 자체는 남는다.
- **33.7(i)** = **부분만 닫힘**. 파급 축(「①을 고르면 `01_clips`를 손으로 고치는 수밖에 없다」)은 D1의 split 단계 필터로 **무효화 = 닫힘**. 처리 방침 축(조각내기 코드를 본 repo에 둘지)은 **미결 유지**. 🔴 **「본 repo에 없다」 사실 서술 무변경.**
- **33.6(a)** = **전부 닫힘**(대응 방향 축 = `other` 신설). ⚠️ 합격 수치 · 재학습 · `smoke_detector` 보류는 **남는다**.
- **5.2(g)** = **항목별로 갈린다**. 사용 순서 **전부 닫힘**(D5) / split 대안 선택 **전부 닫힘**(D2) / 33.7 누수 대응과의 순서 **전부 닫힘**(①) / `SAMPLE_WEIGHT_RANGE` 처리 **부분만 닫힘**(D6 = 서술 축만) / 웹 수집분 클래스 정의 **무접촉** / 명명 · pitch 범위 **부분만 닫힘**(D5로 당면 경로만) / 장치 설치 위치 **무접촉**.
- **8.7(c)** = **전부 닫힘**(PR #68 머지). 🔴 **단 같은 자리에서 신규 미결 1건이 생겼다**(토큰 목록 이중 관리).
- **카테고리 5 머리 `SAMPLE_WEIGHT_RANGE`** = **부분만 닫힘**. 서술 축 확정(D6) / 구현 축 **미확정 유지** — `config.py` 주석이 지금도 그렇게 적는다(실측).

**SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 대상 코드 변경 0)**

- **V1 취소선·볼드 파리티** — `decisions.md` 착수 전 **취소선 포함 줄 101 / 취소선 마커 출현 256 / 볼드 마커 출현 11,506** → 종료 **107 / 274 / 12,574**(계수 단위 = 줄 수 / 마커 출현 수). 취소선 **+18 = 신규 9쌍**(E1 · E4 · E9 · E10 · E12 · E14 각 1쌍 + E2 3쌍, 나머지 12건은 취소선 0개)이고 **짝수**. 볼드 **+1,068 = 534쌍**, **짝수**. 추가분 전 줄의 볼드 마커 개수가 **전건 짝수**임을 별도 스캔했고, **중첩 볼드 5건을 편집 직후 발견해 내부 볼드를 제거**했다(PoC-(54) Set 1 후속 2의 V3 선례 — 미결 제목 안 중첩 볼드와 같은 함정). ⚠️ 본 엔트리 본문은 **취소선 마커를 리터럴로 쓰지 않고 서술로 풀어 썼다**(3세션 연속 계수 오염 방지).
- **V2 삭제 0줄 증명** — `git diff --numstat docs/decisions.md` = **+220 / −8**. 제거 8줄은 **전부 같은 줄의 치환 결과**(E1 · E2 · E4 · E9 · E10 · E11 · E12 · E14 — in-place)이고, 나머지 앵커 7곳(E3 · E5 · E6 · E7 · E8 · E13 · E15)과 말미 append(E16~E18)는 **순수 삽입**이라 제거가 없다. **줄 삭제 0.** 토큰 보존 = **정규화 기준(취소선 · 볼드 · 백틱 마커 제거) 손실 2토큰** / **원시 기준 손실 2토큰** — 둘 다 문장 끝 마침표가 뒤 문장에 이어 붙으며 생긴 **구두점 경계 산물**이고(`5.2.` · `미감사.`) 같은 문자열이 새 문장 안에 그대로 살아 있다. **내용어 손실 0.**
- **V3 앵커 `count == 1` + 실물 되읽기** — 앵커 **15곳 전건 `str.count` == 1 assert 통과**(불일치 시 `sys.exit` abort 설계, 실제 abort 0회)이고 신설 3절은 **파일 말미 append 1회**다. **라인 번호 앵커 사용 0건** — 전부 고유 문구로 잡았다(본 엔트리 본문의 라인 번호 인용도 0건). 신설 절 append는 파일 말미 문자열 **endswith assert**로 고정했다. 편집 후 갱신 구간을 **실물 재조회**해 표 · 소절 머리 · 중첩 마커를 확인했다.
- **V4 🔴 한글 전수 검증** — 추가분에서 **착수 전 파일에 없던 음절 = 21개**: 굵 **U+AD75** · 꽃 **U+AF43** · 낱 **U+B0B1** · 냥 **U+B0E5** · 놀 **U+B180** · 떼 **U+B5BC** · 랍 **U+B78D** · 뤄 **U+B904** · 맨 **U+B9E8** · 뮬 **U+BBAC** · 봇 **U+BD07** · 빼 **U+BE7C** · 뽑 **U+BF51** · 샜 **U+C0DC** · 잎 **U+C78E** · 철 **U+CCA0** · 춤 **U+CDA4** · 탁 **U+D0C1** · 톨 **U+D1A8** · 틸 **U+D2F8** · 희 **U+D76C**(전건 정상 음절 — 굵기 · 불꽃놀이 · 낱말 · 겨냥한 · 떼고 · 서랍 · 미뤄졌을 · 맨 앞 · 시뮬레이션 · 봇 차단 · 빼면 · 뽑은 · 샜다 · 잎에 · 철회 · 맞춤 · 둔탁음 · 온톨로지 · 유틸리티 · 희소도 문맥에서 출현). 「신규 음절 소수」가 충분조건이 아니므로 **신규 한글 단어 350개를 전수 추출해 문맥 통독**했고 **전건 정상 단어 · 모지바케 0**이다. 바이트 보강 = UTF-8 **strict decode 통과** · **`U+FFFD` 0건** · **제어문자 0건**(줄바꿈 · 탭 제외) · **NFC 정규화 동일**.
- **V5 대조 원본** — 등재 수치는 **PR #68 · #69 · #70 본문**(`gh pr view --json body,title,mergeCommit`로 열람)과 **repo 밖 로그 3건**이 원본이다. 본 repo 측 대조는 MCP가 실측했다: 착수 HEAD `a06599d` · 워킹트리 clean · 머지 커밋 3건(`704ddedb…` · `0461a94d…` · `a06599d3…`)이 PR 본문의 `mergeCommit`과 일치 · `git grep "length:--text-"`를 3개 커밋에서 실행(7 / 7 / 8줄) · `NotificationTof.tsx`의 구조분해 실물 · `lib/utils.ts`의 등록 목록 6토큰 · `index.css`의 커스텀 글자 크기 토큰 6개 · `ml/pipeline/split.py`의 `select_clips` · `assign_split` · `guards.assert_no_content_leakage` 실존 · `config.source_key()`의 직접녹음 분기 · `config.py` sample_weight 주석의 현재 문구 · `ml/curation/`의 `taxonomy.py` · `select_negatives.py` · 요일 검산. **로그·실측 없는 수치 0건 기재.**
- **V6 git status** — `docs/` **2파일만**(`M docs/decisions.md` · `M docs/decisions-log.md`). `git add`는 두 파일만 명시 스테이징. 시크릿 스캔(토큰 · 자격증명 · SSID · IP · `.env` 값 패턴) **0건** — 등재된 해시는 **공개 데이터셋 배포본의 md5 2건**과 **git 커밋 해시**뿐이다.

**비범위**: 🔴 **코드 전체 무수정 · 무커밋** — `firmware/` · `server/` · `dashboard/` · `ml/` **전부 읽기만**(수정 0). `docs/git-convention.md` **무접촉**. **노션 무접촉**. 🔴 **repo 밖은 읽기만** — `~/ddingdong-측정결과/2026-09-21/negatives/` 3파일 열람, 쓰기 0. `~/ML 학습 데이터/` **무접촉**. 🔴 **파이프라인 실행 0회 · 데이터셋 무접촉**. 🔴 **새 정책 · 임계 · 합격 수치 · 권고 · 해소책 신설 0건** — 등재한 규칙은 **사용자가 확정한 운영 규칙 1건**(33.12(e))뿐이다. 🔴 **닫지 않은 것**: 33.7(i) 처리 방침 · 5.2(g) 클래스 정의 · 명명·pitch 범위 · 장치 설치 위치 · 카테고리 5 sample_weight 구현 축 · 8.7(a) ② `SystemHealthSummaryCard` · 7.7(l) · 카테고리 21 `.env` 중복 · 6.2 G29 · 카테고리 20 `.claude/` 방침은 **전건 무접촉**. 신규 미결 **1건 신설**(8.7(c-1) 토큰 목록 이중 관리 — 사용자가 결정 대상으로 올린 사실의 등재이지 해소 지시가 아니다). 브랜치 · PR **생성 0**(문서 전용 = `main` 직 push).

**학습 적용**: **학습 13**(출처 catch — 인용 전 grep. 절 목차를 `git show HEAD:docs/decisions.md`로 먼저 뽑아 **33.11이 최대 절 · 27.8 최대 소절이 (o)**임을 확인했고, 편집 앵커 **18곳 전건 `count == 1` assert**, **라인 번호 앵커 0건**. 「관련」 줄 절 번호는 기재 전 실존 확인. 위임이 준 수치도 **PR 본문 · repo 밖 로그 · 코드 3층으로 대조**해 어긋난 1건(A8)을 원본으로 바꿨다) / **학습 16**(기존 컨벤션 우선 + 등가 치환 — **두 방향 모두**. ① **5.2(c)의 실측 표에 취소선을 걸지 않았다**: 날짜 박힌 실측 이력이라 이 파일의 「사실인 서술에 취소선 = 날조」 컨벤션(33.7(d) · 33.6(b) · PoC-(54) Set 1 후속 2 E4)과 충돌한다 — **현재값 append로 대체**했다. ② **취소선 범위를 문장 조각까지 좁혔다**: 33.7(e)는 「방향 미확정」 절에만, 8.7(c)는 세 조각에만 걸고 실측 문장은 남겼다. ③ 등재 대상 자체가 **MCP가 위임의 수단을 불변식으로 바꿔 치환한 사례 2건**(D2의 내용 해시 union → 내용 누수 가드 / 부모 제외 → target 자식만 제외)이라 그 판정 근거를 절 본문에 실었다) / **학습 19**(근본원인 재검증 — **catch 2건**. ① 8.7(c)의 「잔존 1곳」은 **SSoT 문장을 위임이 그대로 옮긴 것**이었고 그 SSoT 문장의 근거가 **전수 조사가 아닌 관측 1건**이었다 — **「SSoT에 있으니 맞다」가 성립하지 않는 자리**이며 27.8(p)②로 등재했다. ② 위임이 33.7(i)를 「D1이 닫는지 판정하라」고 넘겼는데, 실물을 보면 **미결이 2축**이라 한쪽만 닫힌다 — **닫힘 / 안 닫힘 이분법으로 답하지 않고 축을 갈라** 보고했다(6.5(f) 선례)) / **학습 21**(미결도 유령일 수 있음 — 위임 §4의 미결 분해 6대상을 **전부 닫힘 / 부분 / 무접촉**으로 갈랐고, **부분만 닫힌 3건**(33.7(i) · 5.2(g) 일부 · 카테고리 5 sample_weight)을 **한 덩어리로 쓰지 않고 축을 명기**했다. 다음 세션이 「닫혔다」로 오독하면 열려 있는 축이 조용히 사라진다)

**관련 카테고리**: **33.12 (신설 — split 정책 정비 · 제거 순서 · 새 배정 · 운영 규칙)** / **33.13 (신설 — OOD 대응 방향 `other` · 보류 3건 · 착수 전 비용)** / **33.14 (신설 — 네거티브 후보 1,100 · 0바이트 발견 2건 · 청취 검수)** / 33.7 ((e) 대응 방향 ① 확정 · (g)(h) 0바이트 원인·부분 해소 · (i) 파급 축 닫힘·미결 유지) / 33.6(a) (OOD 대응 방향 확정) / 33.2 · 33.6(b) (재실행 시 갱신 대상) / 33.10(b)(g) (`01_clips` FSD50K 유래 원본 · `smoke_detector` 보류 근거) / 8.7 ((c) 정정 5건 · (c-1) 신설 · (a) `enrich_status` 3분기와 `not_target`의 별개 축) / 8.4(f) (`ssr-nc` 한계 ② 도달 불가) / 8.3 (5060 페르소나 — 글자 크기 등급 판정) / 5.2 ((b)(c)(f)(g) 갱신) / 카테고리 5 (머리 `SAMPLE_WEIGHT_RANGE` 부분 확정 · 5.1 4유닛 프로토콜) / 26.10(d) (부스 대화 소리 파급) / 27.8(p) (위임·판단 오류 7건) / 17.2 (검증 자산 베이스라인 — 데이터셋 무변경 증명 축) / 카테고리 20 (negative control 도달 불가 판정) / 카테고리 3 (G12 게이트 순서) / 학습 13 · 16 · 19 · 21.

**관련 commit**: `704dded`(PR #68 `cn()` 커스텀 토큰 결함 정정) · `0461a94`(PR #69 split 정책 정비) · `a06599d`(PR #70 네거티브 후보 선별, **착수 시점 HEAD**) — 3건 모두 2026-09-21 머지이며 본 Set의 등재 대상이다. 인용만 한 선례 = `9b32183`(PR #57, `text-(length:--text-*)` 우회 6곳 — grep 재측정 기준점). 문서 반영 = 본 커밋(docs-only, **PR 없음, `main` 직 push**).

## 2026-09-23 (수) — PoC-(56) Set 1 — M5-c 방향 확정 3단계 · 계측 PR #73 등재(6.3(r) 신설) + 글리치 vs 진짜 클립 모양 판별 실측(6.3(q) 신설) + 2차 타임아웃 15,000ms 값 축 해소(7.7(l)) · 6.3(n) 해결책 방향 확정 · 시연 각본 대응 3건(26.10(e)) + 프로토콜 ② 부분 수행(9.4(e)) · 1차 rtt 교대 재측정(6.2) + 8.7(c-1) 브라우저 5곳 확인 · 안의 신규 미결 닫힘 + `.claude/` gitignore · ssr-nc 서술 정정(카테고리 20) + 27.8(q) 위임·판단 오류 8건 (카테고리 3/6.2/6.3/6.5/6.7/7.6/7.7/8.7/9.4/16/20/26.2/26.10/27.8/30.9/33.14)

2026-09-23 하루에 PR 3건(#71 · #72 · #73)이 머지됐고, 사용자 결정 7건(오전 4 · 오후 2 · PR #72의 타임아웃 확정 1)과 repo 밖 실측 5세션(1차 rtt 교대 재측정 · 브라우저 확인 · ToF 정지 사물 대조 · 마이크 클립 모양 판별 · M5-c ⓐ ④런타임)이 나왔다. 전부 SSoT 미등재였다(`git log 5affc31..HEAD -- docs/` 빈 출력). 본 Set은 `docs/decisions.md`의 6.3에 (q) · (r) 두 소절을, 27.8에 (q) 소절을 신설하고, 기존 22곳에 append · 해소 · 정정을 적용한 뒤 본 엔트리를 남긴 것이 전부다(문서 2파일 · 앵커 치환 24곳 + 사후 문구 조정 9곳). 🔴 닫힌 미결 = 7.7(l)(전부 닫힘 — 값 축) · 8.7(c-1) 안의 신규 미결(전부 닫힘 — 수용된 위험) · 26.10(e) 각본 · 상수 대기(전부 닫힘). 부분만 닫힌 것 = 6.3(n) 해결책(방향만) · 카테고리 20 `.claude/` 처리 방침(`.gitignore` 축만). 임계 · 필터 세부 · 7.6(i) 정책은 확정하지 않았다.

Step 0 가정 대조 — 10항 중 확인 대상 5항 포함 전건 O(불일치 0건): 착수 시점 HEAD `b379c4a` · `main` 단독(`git ls-remote` = `refs/heads/main` 1줄) · 워킹트리 clean · 열린 PR 0 · PR 최대 #73 · `git log 5affc31..HEAD -- docs/` 빈 출력. repo 밖 원본 R1~R6 전건 판독(읽기만). PR #71 · #72 · #73 본문과 머지 커밋 해시 대조 일치. 위임 §4 앵커 12종 계수 전건 일치(3 · 1 · 1 · 1 · 1 · 1 · 1 · 1 · 1 · 2 · 9 · 6). 7.7(l) = 🔴 신규 미결 열린 상태(해소 표기 없음). 8.7(c-1) 안의 신규 미결 · 카테고리 20 관측 절 · 6.3(n) 해결책 · 26.10(e) · 33.14(b)의 대기 서술 전건 열린 상태(학습 21 — 유령 아님). 신설 번호 후보 = 6.3 최대 소절 (p) · 27.8 최대 소절 (p). decisions-log 마지막 엔트리 = 2026-09-21 (월) PoC-(55) Set 1. ⇒ §9 정지 트리거 미발동.

요일 검산: `date -j -f "%Y-%m-%d" "2026-09-23" "+%A"` = 수요일 ⇒ 헤더 (수) 정합.

신설 번호 결정 (근거유형 = 실측 grep): 6.3(q) · 6.3(r). 근거 = 6.3 소절은 볼드 소절 표기 (a)~(p)이고 (p)가 6.4 머리 바로 앞 마지막 소절이다(`^### ` 목록 + 6.3 구간 소절 표기 전수 확인). 글리치 모양 판별과 M5-c 계측은 둘 다 6.3 마이크 M 시리즈의 계측 → 실측 → 판정 흐름이라 같은 절에 잇는다. 번호 체계는 6.3이 쓰는 알파벳 소절 그대로이며 다른 체계를 섞지 않았다. 27.8(q) — 27.8 소절은 (a)~(p)이고(물리 순서상 (g)가 (f)보다 앞이나 문자 최대는 (p)) 다음 문자 (q)를 붙였다. 충돌 검사 = 6.3 · 27.8 구간 안 (q) 사전 0건.

엔트리 형식 · 날짜 처리 판정 (근거유형 = 실물 형식 인용): 2026-09-23 엔트리는 본 건이 처음이라 「후속」 접미가 필요 없다(같은 파일의 첫 엔트리 선례 = `## <날짜> (<요일>) — PoC-(N) Set 1 — …`). 회차 = PoC-(56)이며 새 채팅방 기준이다. 🔴 직전 엔트리들은 본문에 볼드 · 취소선 마커를 썼으나 본 엔트리는 위임 지정대로 두 마커를 리터럴로 쓰지 않고 서술로 풀었다(파리티 계수 오염 방지).

find-skills: `"decision record update"` · `"markdown append"` · `"korean documentation"` 3질의 전건 0건(③). 대조 질의 `git` = 1,450건으로 도구 생존 확인. 채택 0 · install 0.

### 편집 건별 (앵커 24곳 전건 `str.count` == 1 assert 통과 후 적용, 불일치 시 스크립트 abort — 실제 abort 1회는 마커 짝 검사 설계 오류로 파일 쓰기 전 중단, 라인 번호 앵커 0건)

- E1 — 6.2 1차 rtt 단서 블록 끝 (가) append: 교대 재측정 조건 · rtt · 서버 구간 · 판정(가설 반증, 9/18 원인 미지정 유지) · 한계. 취소선 0.
- E2 — 7.7(l) (나) 값 축 해소: 제목 줄 끝에 해소 표지 append + 말미에 근거 3 · 수용된 위험 · 재판정 트리거 3 · 닫힘 분해. 전부 닫힘. 7.7(g) · 7.6(i)는 무접촉. 이력 「미결 유지」 3곳 취소선 0.
- E3 — 7.6(i) (가) 선행 조건 충족 append. 정책 미결 유지.
- E4 — 7.7(g) (가) 서버 기준 2차 소요 append + 비교 조건 경계(논증 분리 표기).
- E5 — 6.5(f) · 6.7(a) (가)(나) 현재값 append 3곳: 경계 갱신(전부 닫힘 6 · 부분 1 · 무접촉 1 · 다른 자리 부분 해소 1) · ⑧ 세션 상한 재검토 예고(값 무변경). 🔴 「부분만 닫힘 2항」 · 「부분만 닫힌 것은 2항」 두 원문은 날짜 박힌 2026-09-18 경계 서술이라 (라) 규칙상 틀린 클로즈가 아니다 ⇒ 취소선 0, 현재값 append.
- E6 — 8.7(c-1) (나) 2곳: 브라우저 확인 현재값 append(근거유형 = 실측(사용자 보고, 수치 미기록)) · 안의 신규 미결 처리 방침 확정 append. 🔴 「C1 · C3 · C4 · C6은 미측정」은 사용자 측정 일시가 미기록이라 작성 시점에 거짓이었는지 판정할 수 없다 ⇒ 틀린 클로즈로 확정할 근거가 없어 취소선 0(R2 원본의 「이력 보존, 확인 사실 append」 지정과도 정합).
- E7 — 카테고리 20 관측 절 (라)(나): 파급 문장 1곳에 취소선 1쌍 + 정정(코드 대조 — `assertCleanSrc` 범위 = `dashboard/src`) · 처리 방침 부분 확정 append. 워크트리 lock 미해소.
- E8 — 6.3(n) (나) 해결책 방향 확정 append. 방식 · 값 축 무접촉 = 부분만 닫힘.
- E9 — 6.3(q) 신설 (다).
- E10 — 6.3(r) 신설 (다)(나) + 포인터 2곳(카테고리 3 음향 트리거 줄 · 6.3(j) 보류 문장).
- E11 — 9.4(e) (가)(나) 부분 수행 현재값 append.
- E12 — 26.10(e) (나) 사용자 결정 append + 26.2 배치 포인터 append.
- E13 — 33.14(b) (가) 보류 유지 · 판정 입력 append.
- E14 — 30.9 (가) 감시 기준점 09-23 append.
- E15 — 카테고리 16 env 13 → 14 현재값 append · 6.3(o) 호스트 테스트 7종 현재값 append.
- E16 — 27.8(q) 신설 (다).
- 사후 문구 조정 9곳: 신설 문구가 인용으로 기존 앵커 8종(「프로토콜 ② 미수행」 · 「워킹트리 clean을 요구하므로」 · 「부분만 닫힌 것은 2항」 · 「부분만 닫힘 2항」 · 「보드 타임아웃 결정이」 · 「시연 각본 조정」 · 「처리 방침 미확정」 · 「13개」)을 복제한 것을 사후 계수에서 발견해, 본 Set이 추가한 문장만 같은 뜻의 다른 표현으로 바꿨다. 기존 문장 무접촉.

### 등재 내용 요약 (사실 · 결정)

- [사용자 결정] M5-c = 10월 착수 3단계(ⓐ 계측 → ⓑ 판정 — 임계 · 디바운스 · 2차 세션 상한 재검토 → ⓒ 부스 자동 트리거 = `other` 재학습 이후, 그 전 수동 트리거 기본값). 트리거 계산 방향 = 클램프 제외 RMS. 값 · 날짜 미정.
- [실측] ⓐ PR #73 `b379c4a` = `env:mic_trigprobe` 5파일 647 insertions, `trig_line_test` 74 checks · NC 3종. ④런타임 8,220버퍼 누락 0 · drop 11 → 11 · gaps 0 · fmt_err 0. 조용 구간 클램프 제외 RMS 최대 154, 노크 사건 9개 최약 217(1.4배), 말소리 143버퍼 초과 ⇒ 원래 RMS 트리거 불성립 · 클램프 제외 RMS 방향 지지 · 임계 확정 금지.
- [실측] 글리치 클램프 104개(+102 / −2) 전부 고립 단일 샘플 vs 진짜 클립 68~294샘플 연속 ⇒ 모양으로 판별 가능. 보통 세기 노크는 ToF OFF에서 클립 0(6 / 6).
- [사용자 결정] 6.3(n) 해결책 = ① 펌웨어 먼저 · ② 배선 보류 · ③ 병행 비권고. 필터 세부 미확정 · 오디오 페이로드 필터 보류.
- [사용자 결정] 2차 타임아웃 15,000ms 확정(PR #72 `d1a2384`, 값 변경 0) — 수용된 위험 약 19.5~20초 > 15초 · 재판정 트리거 3개.
- [실측] 1차 rtt 교대 재측정: skip 201~466ms · pending 561~1,246ms ⇒ 「게이트 구조만으로 4초+」 가설 반증, 2026-09-18 원인 미지정 유지. 서버 기준 2차 소요 8,280~13,049ms(예산의 62~87%).
- [실측] 프로토콜 ② 부분 수행: 정지 책이 첫 NONE까지 16.9~48.0초 · 정지 사람도 NONE 구간 발생 · ndet 분포로 가를 근거 없음.
- [사용자 결정] ToF 상수 무변경 + 시연 각본 대응 3건.
- [실측(사용자 보고, 수치 미기록)] 8.7(c-1) C1 · C3 · C4 · C6 브라우저 정상.
- [사용자 결정] 토큰 목록 이중 관리 = 자동 가드 미신설(수용된 위험) + `index.css` 역방향 경고 주석(PR #71). `.claude/` gitignore(PR #71 `510fd01`, 사유 = public repo 노출 방지).
- [실측 코드 대조] 카테고리 20의 ssr-nc 파급 서술은 코드와 불일치 — 취소선 + 정정.
- [사용자 결정] `Speech_synthesizer` 보류 유지 · 판정 입력 = `fire_alarm` 학습 클립 10 + 후보 10 청취.
- [실측 툴팁 값] NCP CSR 09-23 = 3 / 0 / 45 = 예상 일치.
- [실측] env 14개 · 호스트 테스트 7종.
- [실측 + 실물 대조] 27.8(q) 위임 · 판단 오류 8건(catch 주체 병기). 27.8(d) 누적 숫자 무접촉.

원본 대조 결과 (위임 본문 vs repo 밖 원본 · PR 본문 — 어긋난 것 전건 보고)

- 수치 불일치 0건. R1 · R3 · R4(정정 블록 포함) · R5 · R6의 수치가 위임 E1 · E4 · E9 · E10 · E11 · E14와 전건 일치했다.
- 관찰 1 — 타임아웃 확정(E2)은 R6(사용자 결정 6건 원문)에 없다. 출처는 PR #72 본문(「2026-09-23 사용자 결정」)이며 위임 서술과 일치한다. 결정 주체 표기는 PR #72 본문 기준으로 적었다.
- 관찰 2 — R3의 거리 표기는 「약 44cm 같은 자리」(center 432~492mm)이고 한계 줄이 「약 44~49cm」다. 위임의 「약 44~49cm」를 그대로 쓰고 center 실측 범위를 병기했다.
- 관찰 3 — R6 5번은 2차 세션 상한을 「12」로 명시한다. 6.7(a)⑧의 현재값 ≤12와 일치.

미결 분해 (전부 닫힘 / 부분만 닫힘 / 무접촉)

- 7.7(l) = 전부 닫힘(방식 축 2026-09-18 + 값 축 2026-09-23). 수용된 위험 · 트리거 3개로 재개 가능.
- 7.7(g) 2차 15초 예산 = 무접촉. 7.6(i) 서버측 자막 실패 정책 = 무접촉(선행 조건 충족 사실만 append).
- 6.5(f) 경계 = 전부 닫힘 6 · 부분 1(녹음 길이 pre:post) · 무접촉 1(카메라 init 시점) · 다른 자리 부분 해소 1(6.3(n)).
- 6.3(n) 해결책 = 부분만 닫힘(방향 축 닫힘 · 방식 · 값 축 무접촉).
- 8.7(c-1) 안의 신규 미결 = 전부 닫힘(수용된 위험).
- 카테고리 20 `.claude/` 처리 방침 = 부분만 닫힘(`.gitignore` 축 닫힘 · 워크트리 정리 시점 축 무접촉).
- 26.10(e) 각본 · 상수 재검토 대기 = 전부 닫힘(상수 무변경 + 각본 3건).
- 33.14(b) `Speech_synthesizer` = 무접촉(보류 유지 — 판정 입력만 등재).
- 카테고리 3 음향 트리거 = 방향만 등재, 값 무접촉.

### SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 대상 코드 변경 0)

- V1 취소선 · 볼드 파리티 — `decisions.md` 착수 전 취소선 포함 줄 107 / 취소선 마커 출현 274 / 볼드 마커 출현 12,574 → 종료 108 / 276 / 13,270(계수 단위 = 줄 수 / 마커 출현 수). 취소선 +2 = 1쌍(E7만), 짝수. 볼드 +696 = 348쌍, 짝수. 추가 줄 118개 전부 줄 단위로 두 마커 개수가 짝수임을 별도 스캔. 본 엔트리는 두 마커를 쓰지 않았다.
- V2 삭제 0줄 증명 — `git diff --numstat docs/decisions.md` = +118 / −2. 제거 2줄은 같은 줄의 in-place 치환(7.7(l) 제목 줄 끝 append · 카테고리 20 파급 줄 취소선)이다. 토큰 보존 = 정규화 기준(취소선 · 볼드 · 백틱 마커 제거) 손실 0 / 원시 기준 손실 2토큰 — 둘 다 취소선 마커가 붙어 토큰 경계가 바뀐 산물이고 같은 문자열이 새 줄에 그대로 살아 있다. 내용어 손실 0.
- V3 앵커 계수 사후 재확인 — 위임 §4 앵커 12종이 편집 후에도 착수 전 계수와 같다(신설 문구의 앵커 복제 8종을 사후 조정으로 해소). 라인 번호 앵커 · 인용 0건.
- V4 한글 전수 검증 — 추가분에서 착수 전 파일에 없던 음절 = 4개: 뮤 U+BBA4 · 앉 U+C549 · 얇 U+C587 · 퀴 U+D034(뮤텍스 · 앉아서 · 얇다 · 바퀴 문맥, 전건 정상 음절). 신규 음절 수가 충분조건이 아니므로 신규 한글 단어 215개를 문맥과 함께 전수 통독했고 전건 정상 · 모지바케 0이다. 바이트 보강 = `U+FFFD` 0건 · 제어문자 0건 · NFC 정규화 동일 · 역슬래시 u 이스케이프 0건.
- V5 대조 원본 — repo 밖 R1~R6 · PR #71 · #72 · #73 본문(`gh pr view`) · `dashboard/tools/ssr-nc/run.mjs`(`assertCleanSrc` · `ROOT`) · `firmware/platformio.ini` env 수 14 · `git show --stat b379c4a` · PR #72 커밋 목록(`1e7bb28` 실재) · `firmware/include/uplink_common.h` 확정 주석 실재. 로그 · 실측 없는 수치 0건 기재.
- V6 git status — `docs/` 2파일만 스테이징(`git add docs/decisions.md docs/decisions-log.md`).

### 비범위

- 코드 · 설정 파일 무수정. 노션 · 지침 무접촉. `docs/git-convention.md` 무접촉. repo 밖은 읽기만(쓰기 0). 셸 실측 재실행 0.
- 확정하지 않은 것: 7.6(i) 정책 · 8.7(a) 잔존 · M5-c 임계 · 디바운스 · 착수 날짜 · 6.3(n) 필터 세부 · 2차 세션 상한 값 · 장치 설치 위치 · 초인종 실물 · 카테고리 3 임계값 · 6.5(f) 카메라 init 시점 · 녹음 pre:post 비율 · 워크트리 정리 시점.
- 발견했으나 손대지 않은 것: 8.4(f) 환경 사실 ①의 「워킹트리 diff」 서술이 실제 범위(`dashboard/src`)보다 넓다 — 카테고리 20 정정 줄에 기록만 했다. 6.3(n) · (p)의 「해결책 ①②③은 여전히 사용자 판단 대기」 서술 2곳은 날짜 박힌 이력이라 무접촉(현재값은 6.3(n) 말미).

학습 적용: 학습 13(출처 catch — 앵커 전건 grep 후 인용, 라인 번호 0건, 9.4(e)의 「프로토콜 ②(정지 사물 대조) 미수행」은 괄호 · 볼드가 앵커 문자열을 갈라 grep 3건에 잡히지 않았다 — 마커를 걷어낸 재조회로 현재값 자리를 찾았다) / 학습 16(기존 컨벤션 우선 — 엔트리 형식은 직전 엔트리 구조를 따르되 마커만 위임 지정대로 뺐다. 카테고리 16의 값 사슬은 선례상 취소선이었으나 본 위임의 (가) · §7 규칙이 우선해 append로 이었고 그 이탈을 본문에 적었다) / 학습 19(위임 근거 재검증 — E7의 코드 불일치를 `run.mjs` 실물로 다시 확인, E2 근거 ②의 n=1 사건이 「연결 종료 후 처리 계속」의 근거가 아님을 PR #72 후속 커밋으로 확인) / 학습 21(대기 미결 5건이 지금도 열린 상태인지 실물 확인 후 처리, 부분만 닫힌 2건은 축을 나눠 적었다).

### 관련 카테고리

6.3 ((q) 신설 — 글리치 모양 판별 · (r) 신설 — M5-c 3단계 · PR #73 · ④런타임 · (n) 방향 확정 · (o) 호스트 테스트 7종 · (j) 포인터) / 7.7 ((l) 값 축 해소 · (g) 서버 기준 2차 소요) / 7.6(i) (선행 조건 충족) / 6.2 (1차 rtt 교대 재측정) / 6.5(f) · 6.7(a) (경계 현재값 · ⑧ 재검토 예고) / 8.7(c-1) (브라우저 확인 · 신규 미결 닫힘) / 카테고리 20 (ssr-nc 서술 정정 · `.claude/` 처리 방침) / 9.4(e) (프로토콜 ② 부분 수행) / 26.10(e) · 26.2 (각본 대응 · 배치) / 33.14(b) (`Speech_synthesizer` 판정 입력) / 30.9 (감시 기준점 09-23) / 카테고리 16 (env 14) / 카테고리 3 (음향 트리거 방향 포인터) / 27.8(q) (오류 8건) / 학습 13 · 16 · 19 · 21.

### 관련 commit

`510fd01`(PR #71 `.claude/` gitignore + `index.css` 역방향 주석) · `d1a2384`(PR #72 2차 타임아웃 확정, 내부 후속 커밋 `1e7bb28` 근거 문구 정정) · `b379c4a`(PR #73 `env:mic_trigprobe`, 착수 시점 HEAD, 머지 전 ④런타임 커밋 `0209506`) — 3건 모두 2026-09-23 머지이며 본 Set의 등재 대상이다. 문서 반영 = 본 커밋(docs-only, PR 없음, `main` 직 push).

## 2026-09-25 (금) — PoC-(57) Set 1 — 작업일 09-23 · 09-24 · 09-25의 사용자 결정 · 실측 · PR #74 등재 — 직접녹음 수집 경로 신설(5.3 — 녹음 기기 · 노크 프로토콜 · `record_receiver`) + AI Hub 앞머리 멘트 청취 · 조각 계수 · 학습 제외 결정(33.15 신설) + `SAMPLE_WEIGHT_RANGE` 구현 축 닫힘(카테고리 5) + 조각내기 도구 방향 확정(33.7(i)) · pad 규칙 보강(33.10(b)) + `other` 계약 PR 범위 · 확인할 자리 실측(33.13(d) 신설) + `Speech_synthesizer` 보류 해소(33.14(b)) + 8.4(f) 환경 사실 ① 정정 + ml 테스트 5종 현재값 + 27.8(r) 위임·판단 오류 6건 (카테고리 5/5.2/5.3/8.4/17.2/20/27.8/33.3/33.6/33.7/33.10/33.12/33.13/33.14/33.15)

PoC-(57)은 2026-09-23 오후부터 09-25까지 사흘에 걸친 회차다. 그 사이 사용자 결정 7건(09-23 5건 = 조각내기 도구 · 노크 / 화재경보 녹음 · SAMPLE_WEIGHT_RANGE · `other` 계약 PR 범위 · Speech_synthesizer, 09-24 2건 = 녹음 기기 · 멘트 조각 제외), repo 밖 청취 · 계수 실측 2세션(09-23 Speech_synthesizer 판정 청취, 09-24 AI Hub 앞머리 멘트 확인 청취 + 셸 계수), PR 1건(#74 `0edda5b`, 09-24 생성 · 09-25 머지)이 나왔고 전부 SSoT 미등재였다. 본 Set은 `docs/decisions.md`에 5.3 · 33.15 두 절과 33.13(d) · 27.8(r) 두 소절을 신설하고, 기존 줄 15곳에 append · 포인터를, 기존 소절 2곳(33.7(i) · 33.14(e))에 bullet을 추가하고, 1곳(8.4(f) 환경 사실 ①)에 취소선 + 정정을 적용한 뒤 본 엔트리를 남긴 것이 전부다(문서 2파일, 코드 0줄). 닫힌 미결 = 카테고리 5 머리 SAMPLE_WEIGHT_RANGE(전부 닫힘 — 구현 축) · 33.14(b) Speech_synthesizer 판단 보류(전부 닫힘). 부분만 닫힌 것 = 33.7(i) 처리 방침(방향 축 닫힘 · 실행 축 남음). 33.7(h) 제작 경위는 청취 입력만 더하고 닫지 않았다.

Step 0 가정 대조 — 7항(확인 대상 3항 포함) 전건 O, 불일치 0건: 착수 시점 HEAD `0edda5b` · `main` · 워킹트리 clean. decisions-log 마지막 엔트리 = 2026-09-23 (수) PoC-(56) Set 1(이후 엔트리 없음). PR #74 = MERGED, 머지 커밋 `0edda5b19cf0`, 파일 1개(`server/tools/record_receiver.py`, +504 / −0). 위임 §4 인용 12종 전건 실재 — 이 중 5종(33.13(c) 3클래스 가정 여부 · 33.14 판단 보류 · 판정 입력 10 + 10 · 카테고리 5 직접 녹음 계획 · 33.12(c) 합계 2299)은 원시 grep 0건이고 볼드 · 백틱 마커를 걷어낸 문자열로 1건씩 잡혔다(마커 분할). 6.5(f)도 볼드 소절 표기라 같은 방식으로 실재 확인. 확인 대상 ① `ml/curation` · `select_negatives` · `taxonomy.py` 문자열 SSoT 0건 확인, ml 테스트 파일 실물 5개(`find ml -name 'test_*.py'`) — 17.2 · 33.12(d)의 4종은 각 시점 이력. 확인 대상 ② 8.4(f) 환경 사실 ①은 「커밋되지 않은 워킹트리 diff」라 적고, `git log -S assertCleanSrc`의 유일한 커밋 `8d574dd`(PR #51, 2026-09-09)부터 `-- src` 한정이었으며 그 줄은 2026-09-12(`9389b0d`)에 쓰였다 ⇒ 작성 시점에 이미 넓게 적힌 틀린 클로즈. 확인 대상 ③ 5.2(g) 표의 명명 · pitch 대상 범위 행은 5.2(b) 부분 해소가 가리키는 웹 수집분 대상 ⇒ 녹음 기기 결정과 다른 미결, 5.2(g) 무접촉. ⇒ §9 정지 트리거 미발동.

요일 검산: `date -j -f "%Y-%m-%d"` = 2026-09-23 수요일 · 2026-09-24 목요일 · 2026-09-25 금요일.

신설 번호 결정 (근거유형 = 실측 grep): 5.3 — 카테고리 5는 5.1 · 5.2까지 있고 성격(데이터 수집 경로 — 제품 동작 불변)이 ML 데이터 카테고리에 속해 다음 번호를 붙였다. 서버 도구지만 6.x(제품 서버 절)에 두면 제품 경로로 오독될 수 있어 피했다. 33.15 — 카테고리 33은 33.14까지이고 멘트 청취 · 계수 · 결정은 33.7(h) · 33.12 · 33.14의 동성격 후속이라 다음 번호. 33.13(d) — 33.13 소절이 (a)~(c)라 다음 문자. 27.8(r) — 27.8 소절 문자 최대가 (q)라 다음 문자. 충돌 검사 = 각 구간 안 신설 번호 사전 0건.

엔트리 형식 · 날짜 처리 판정 (근거유형 = 실물 선례): 작업일이 사흘이지만 한 엔트리로 두고 헤더는 반영일 2026-09-25로 달았다. 근거 = 이 로그는 엔트리 헤더가 반영일이고 발견일은 본문에 분리 표기하는 선례가 있다(2026-09-19 엔트리가 2026-09-18 발견분을 「발견 2026-09-18 / 문서 반영 2026-09-19」로 담음). 「후속」 · 「Set 1 후속 N」은 같은 날 두 번째 이후 엔트리의 접미라 2026-09-25 첫 엔트리인 본 건에는 붙이지 않았다. 09-23 작업분을 이미 있는 2026-09-23 PoC-(56) 엔트리에 보강하지 않은 이유 = 회차가 다르다(PoC-(56) Set 1 반영 뒤 새 채팅방 PoC-(57)의 작업). 본 엔트리는 볼드 · 취소선 마커를 리터럴로 쓰지 않았다(파리티 계수 오염 방지).

find-skills: `npx skills search` 3질의(`decision log update` · `markdown strikethrough` · `korean text integrity`) 전건 0건(③). 대조 질의 `git` = 1,449건으로 도구 생존 확인. 채택 0 · install 0.

### 편집 건별 (앵커 21곳 + 파일 끝 1곳, 앵커 전건 `str.count` == 1 assert 통과 후 적용, 불일치 시 스크립트 abort — 실제 abort 0회, 라인 번호 앵커 0건)

- 카테고리 5 머리 직접 녹음 계획 줄 — 현재값 append(노크 진행 · 화재경보 240 학습용 보류). 계획값 이력 취소선 0.
- 카테고리 5 머리 SAMPLE_WEIGHT_RANGE 줄 — 구현 축 닫힘 append(구현 안 함 · 논증 3 · 재검토 조건 · 코드 주석 stale은 재학습 준비 PR 소관). 2026-09-21 「부분만 닫힘」 취소선 0.
- 5.2(e) 보드 재녹음 경로 부재 줄 — PR #74 수집 경로 현재값 append. 작성 시점 사실이라 취소선 0.
- 5.3 신설 — (a) 녹음 기기 · (b) 노크 · 화재경보 · pitch 마커 충돌 · (c) PR #74.
- 8.4(f) 환경 사실 ① — 「워킹트리 diff」 클로즈 1곳에 취소선 1쌍 + 정정(작성 시점 판정 근거 = `git log -S`). 줄 뒤 사유 · 검증 순서 문장 무접촉.
- 카테고리 20 관측 절 정정 줄 — 8.4(f) 반영 포인터 append(정정 서술 중복 없음).
- 17.2 ml 줄 · 33.12(d) — ml 테스트 5종 현재값 append. 4종 이력 취소선 0.
- 33.3① 결정 줄 — 부분 문자열 매칭 단서 포인터 append.
- 33.6(a) 판정 줄 · 33.6(f) 대응 방향 줄 · 33.7(b) 누수 79 줄 — 33.15 포인터 append.
- 33.7(h) 제작 경위 미확정 줄 — 청취 입력 append, 닫지 않음.
- 33.7(i) — 처리 방침 방향 확정 · 실행 대기 sub-bullet 신설(규칙 5줄 + 닫힘 분해).
- 33.10(b) 유일한 분기 줄 — zero-pad · 3초 미만 버림 보강 append.
- 33.12(b) 제거 순서 줄 — D1 확장(멘트) 포인터 append.
- 33.13(d) 신설 — 계약 PR 범위 결정 · 확인할 자리 실측 6건 · 사용자 판단 대기 3건 · 지표 귀속 교란 권고.
- 33.14(b) 판단 보류 줄 — 해소 append(청취 수치). 33.14(e) — 코드 소재 bullet 신설. 33.14(f) — 한국어 말소리 네거티브 미결 승격 append.
- 33.15 신설 (파일 끝).
- 27.8(r) 신설.
- 사후 조정 3곳: 8.4(f) · 33.3① 두 곳은 삽입이 줄 중간(뒤에 기존 문장이 이어지는 자리)에 들어가 읽는 순서가 어색해 삽입분만 같은 줄 끝으로 옮겼고, 그에 맞춰 8.4(f) 정정 안의 「아래 사유」를 「같은 줄의 사유」로 고쳤다. 기존 문장 무접촉.

### 등재 내용 요약 (사실 · 결정)

- [사용자 결정 09-23] 조각내기 도구 = 본 repo에 새로 작성(규칙만 계승) · 신규 유입분만 · 과거 01_clips 재생성 안 함. 네거티브 모드 = FSD50K 규칙(3초 미만 zero-pad 1클립 / 이상 비중첩 · 잔여 버림), 직접녹음 모드 = 소리 시작점 기준 3초, 원본당 조각 상한(값 미정) · 경로 인자 필수 · 멱등 skip · 투입 manifest, 실데이터 실행 = 재학습 당일.
- [실측 코드 — 공개 pretest repo `7d93743` = 원격 HEAD `907c950`의 scripts] fsd50k만 3초 미만 zero-pad(`apad`), aihub · audioset은 3초 미만 버림.
- [사용자 결정 09-23] 노크 180 = 먼저 녹음, 유닛 = 문 × 두드리는 사람 6개 이상, `direct_knock_{유닛}_{테이크}.wav`. 화재경보 240 = 학습용 보류, 주택용 화재경보기 시험음으로 외부 평가 먼저.
- [사용자 결정 09-24] 녹음 기기 = 보드 INMP441 경로. 폰 · USB 스트리밍 기각.
- [실측 코드] pitch 마커가 부분 문자열 매칭이라 `("direct_",)`면 직접녹음 화재경보도 pitch 대상 — 클래스별 마커는 재학습 준비 PR 소관.
- [실측] PR #74 = 1파일 504줄, 펌웨어 등 0줄, 65,536 B / 163,840 B, 응답 92 B ≤ 767 B, 상수 대조 21건, NC 5종, 회귀 106. 반영 시 self-test 재실행 PASS 48. ④런타임 미수행.
- [사용자 결정 09-23] SAMPLE_WEIGHT_RANGE 구현 안 함(증강 6 → 8, 약 1.33배 — 논증).
- [사용자 결정 09-23] 계약 PR 프로즌 예외 = `server/inference/constants.py` 한 파일, `other` = 인덱스 3, 33.6(e) 분리, 머지 순서 불변. [실측 코드] 확인할 자리 6건(model_runner ValueError · fixture (1, 3) · SKIP_REASON_LABEL · 펌웨어 0 · BG_NOISE_SNR_DB KeyError · labels.json 재생성 순서).
- [사용자 결정 09-23 청취 후] Speech_synthesizer = 네거티브 유지. [실측 청취 1인] 표본 16 = fire_alarm 10(AI Hub 7 + 그 외 3) · 후보 6. 목소리 있음 3 / 없음 7 · 비슷함 0 / 6 · 경보음 섞임 0 / 6.
- [실측 청취 + 셸 계수 09-23 · 09-24] AI Hub 화재 녹음 앞 약 9초 = 사람 말소리. 조각 시작별 171 · 171 · 171 · 171 · 164 · 139 · 118 · 95 · 50 · 40 · 34 · 25, 녹음 171, 9000ms 미만 513(train 345 / val 87 / test 81).
- [사용자 결정 09-24] 멘트 조각(AI Hub fire_alarm 시작 9000ms 미만 = 녹음당 3조각) 학습 제외, 순서 = 무음 → 멘트 → 교차 → 중복, `other` 재활용 안 함. [산술] 2792 − 5 − 513 − 10 = 2264 = 2299 − 35.

원본 대조 결과 (위임 본문 vs repo 밖 원본 · PR 본문 · 코드 — 어긋난 것 전건 보고)

- 수치 불일치 0건. `qa.csv`(22행 = fire_alarm 조각 10 + 후보 6 × 2문항) · `listen_result_note.md` · `note.md` · `selection.csv` · `aihub_slate/listen_note.md`의 수치가 위임과 전건 일치했다. PR #74 본문의 65,536 B · 163,840 B · 21건 · NC 5종 · 회귀 106도 일치.
- 관찰 1 — 응답 92 B는 PR 본문에 없다(본문은 767 B 이하만). 반영 시 `--self-test` 재실행 출력 「응답 길이 ≤ 767 B — 92 B」로 확인해 적었다.
- 관찰 2 — 위임은 「33.10 규칙 요약에 pad 서술 0건」이라 했으나 33.10(b)에는 이미 「fsd50k만 3초 미만일 때 단일 클립」 분기가 있었다. 빠진 것은 zero-pad 방식과 나머지 두 스크립트의 3초 미만 버림이다 ⇒ 위임 지정대로 취소선 없이 보강 append.
- 관찰 3 — 표 합계 1,349 = pretest 커밋 메시지의 AI Hub 1,355클립 − 길이 0 wav 6개. 이 수는 33.7(h) 「산술 정합」(171 × 약 9.6 ≈ 1642, fire_alarm 전체를 AI Hub로 본 산술)과 맞지 않는다 — 33.15(b)에 관찰만 적고 33.7(h)는 고치지 않았다(다음 Set 판정 대상).
- 관찰 4 — 33.15(c)의 487(AI Hub 9그룹 파일 수)은 33.7(a)의 순수 잉여 487(502 − 15)과 수만 같고 다른 양이다 — 혼동 방지 문장을 넣었다.
- 관찰 5 — 재검토 조건의 「같은 클래스 웹 데이터」는 5.2의 한국 웹 음원(외부 평가 전용)과 섞여 읽힐 수 있어 「같은 클래스의 기존 데이터(공개 데이터셋 유래)」로 풀어 적었다.

미결 분해 (전부 닫힘 / 부분만 닫힘 / 무접촉)

- 카테고리 5 머리 SAMPLE_WEIGHT_RANGE = 전부 닫힘(서술 축 09-21 + 구현 축 09-23). 코드 주석 · 상수 정리는 재학습 준비 PR 소관(기록만).
- 33.14(b) Speech_synthesizer 판단 보류 = 전부 닫힘(네거티브 유지).
- 33.7(i) 처리 방침 = 부분만 닫힘(방향 축 닫힘 · 실행 축 남음 — 도구 PR · 조각 상한 값). 미결 유지.
- 33.7(h) AI Hub 제작 경위 = 무접촉(청취 입력만 append — 내용은 들었고 경위는 모름).
- 33.13 계약 PR = 범위 결정 · 확인할 자리 실측까지. 목록 표시 · SkipReasonCounts 키 · mock 출현 3건은 사용자 판단 대기.
- 33.14(f) 한국어 말소리 네거티브 = 한계에서 미결로 승격(출처 = 사용자 판단 대기, 별도 절 신설 없음).
- 지표 귀속 교란 = 33.13(a) E4 대기 항목의 입력으로 등재(Claude 권고 · 사용자 미결정, 신규 미결 분리 없음).
- 5.2(g) 웹 수집분 명명 · pitch 대상 범위 = 무접촉(다른 미결).

### SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 대상 코드 변경 0)

- V1 취소선 · 볼드 파리티 — `decisions.md` 착수 전 취소선 포함 줄 108 / 취소선 마커 출현 276 / 볼드 마커 출현 13,270 → 종료 109 / 278 / 13,880(계수 단위 = 줄 수 / 마커 출현 수). 취소선 +2 = 1쌍(8.4(f)만), 짝수. 볼드 +610 = 305쌍, 짝수. 추가 줄 전부 줄 단위로 두 마커 개수가 짝수임을 별도 스캔. 본 엔트리는 두 마커를 쓰지 않았다.
- V2 삭제 0줄 증명 — `git diff --numstat docs/decisions.md` = +126 / −16. 제거 16줄은 전부 같은 줄의 in-place 수정이다(줄 끝 append 15 — 사후 이동 1곳 포함 · 취소선 1). 15줄은 원문이 새 줄에 부분 문자열로 그대로 남는다. 토큰 보존 = 원시 기준 손실 0 / 정규화 기준(볼드 · 취소선 · 백틱 제거) 손실 0.
- V3 앵커 계수 사후 재확인 — 위임 §4 인용 12종의 마커 제거 기준 계수가 편집 전후 동일(3 · 1 · 1 · 1 · 1 · 1 · 1 · 1 · 1 · 2 · 1 · 1). 신설 문구가 인용 문자열을 복제하지 않도록 「위 🔴 대기 문장」 · 「계획된 판정 입력(10 + 10)」 · 「제작 경위」처럼 다른 표현을 썼다. 라인 번호 인용 0건.
- V4 한글 전수 검증 — 추가분에서 착수 전 파일에 없던 음절 4개: 뚜 U+B69C · 렷 U+B837 · 슷 U+C2B7 · 탄 U+D0C4(뚜렷이 · 비슷함 · 탄다 문맥, 전건 정상 음절). 신규 한글 단어 185개를 전수 출력해 통독했고 전건 정상 · 모지바케 0. 바이트 보강 = U+FFFD 0 · 제어문자 0 · NFC 정규화 동일 · 역슬래시 u 이스케이프 0 · UTF-8→latin1 깨짐 패턴 0.
- V5 대조 원본 — repo 밖 5파일(위) · `gh pr view 74` · `gh api` pretest compare(`7d93743...907c950` = docs 1파일) 및 scripts 3개 · `record_receiver.py --self-test` 재실행(임시 디렉터리) · `model_runner.py` · `model_serving.py` · `test_detect_regression.py` fixture · `notification-meta.ts` · `stats.ts` · `ml/pipeline/config.py` · `augment.py` · `export.py` · `enrich_wire.h` · `run.mjs` · `git log -S assertCleanSrc`. 로그 · 실측 없는 수치 0건 기재.
- V6 git status — `docs/` 2파일만 스테이징(`git add docs/decisions.md docs/decisions-log.md`).

### 비범위

- 코드 · 설정 파일 무수정(`config.py` 주석 stale · 상수 제거 · 클래스별 pitch 마커 · 멘트 제외 규칙 · 조각내기 도구 = 재학습 준비 PR 소관). 노션 · 지침 무접촉. 재학습 런북 초안(repo 밖) 무접촉. repo 밖은 읽기만(쓰기 0).
- 확정하지 않은 것: 계약 PR 직전 판단 3건(목록 표시 · SkipReasonCounts 키 · mock 출현) · PR #74 성격 어휘 · 3클래스 비교 학습 채택 · 한국어 말소리 네거티브 출처 · 조각 상한 값 · 멘트 제외 사유 라벨명 · 유닛 배정 정책.
- 발견했으나 손대지 않은 것: 33.7(h) 산술 정합의 전제(fire_alarm 전체 = AI Hub)가 계수 1,349와 맞지 않음 — 33.15(b)에 관찰만.

학습 적용: 학습 13(인용 12종을 마커 제거 문자열로 재조회 — 5종이 원시 grep 0건이었다 · 라인 번호 0건 · 17.2 · 33.12의 「4종」과 5.2(e) 「없다」는 이력으로 보고 현재값 append) / 학습 16(유닛 대소문자는 위임보다 SSoT 관례 · PR 성격 어휘는 8.7(b) 선례대로 판정 안 함 · 위임의 「pad 서술 0건」은 실물(단일 클립 분기 실재)에 맞춰 보강으로 적음) / 학습 17(위임 수치 전건을 원본 · PR 본문 · 코드와 교차 검산) / 학습 21(33.7(i) · 33.14(b) · 카테고리 5 머리 미결이 열린 상태인지 실물 확인 후 처리, 한국어 말소리 네거티브는 새 절 대신 기존 한계 자리에서 승격, 지표 귀속 교란은 33.13(a) E4 대기와 같은 자리로 판정).

### 관련 카테고리

카테고리 5 머리(직접 녹음 계획 현재값 · SAMPLE_WEIGHT_RANGE 구현 축 닫힘) / 5.2(e)(수집 경로 현재값) / 5.3(신설 — 녹음 기기 · 노크 · 화재경보 · PR #74) / 8.4(f)(환경 사실 ① 정정) / 카테고리 20(정정 포인터) / 17.2 · 33.12(d)(ml 테스트 5종) / 33.3①(pitch 마커 단서) / 33.6(a)(f) · 33.7(b)(원인 · 정체 후보 포인터) / 33.7(h)(청취 입력) · (i)(처리 방침 방향 확정) / 33.10(b)(pad 보강) / 33.12(b)(D1 확장 포인터) / 33.13(d)(신설) / 33.14(b)(e)(f) / 33.15(신설) / 27.8(r)(신설) / 학습 13 · 16 · 17 · 21.

### 관련 commit

`0edda5b`(PR #74 녹음 전용 수신기 `record_receiver`, 착수 시점 HEAD — 2026-09-24 생성 · 2026-09-25 머지)가 본 Set의 등재 대상이다. `a06599d`(PR #70 `ml/curation`)는 경로 · 테스트 종수 등재만. 문서 반영 = 본 커밋(docs-only, PR 없음, `main` 직 push).

## 2026-09-25 (금) 후속 — PoC-(58) Set 1 — 재학습 준비 코드 PR 4건 등재(33.16 신설 — #75 네거티브 조각내기 · #76 split 규칙 3건 · #77 Zeroth-Korean · #78 정합성 정정) + 사용자 결정 등재(33.17 신설 — 재학습 착수 기한 · 3클래스 비교 학습 · 한국어 말소리 네거티브 출처 · 조각 상한 잠정 3 · `other` 계약 PR 직전 3건 · Zeroth 화자 그룹 키) + 첫 실녹음 · 직접녹음 `doorbell` 대상 정의(5.3(d) 신설) + 33.7(h) 산술 정합 취소선 + 정정 + 6.3(q) 단일 샘플 클립 첫 실측 · 7.5(e) 재발급 관측 창 · 33.3① 현재값 + 27.8(r) 2건 추가 · 27.8(s) 위임 · 판단 오류 10건 (카테고리 5/5.2/5.3/6.3/7.5/27.8/33.3/33.7/33.12/33.13/33.14/33.15/33.16/33.17)

PoC-(58)은 2026-09-25 낮(새 채팅방)부터 밤까지의 회차다. 사용자 결정 7건(재학습 착수 기한 · 비교 학습 · Zeroth-Korean 출처 · 녹음 대상 정의 · 조각 상한 · 계약 PR 직전 3건 · Zeroth 화자 그룹 키 — 위임 배경의 「결정 6건」과 목록 A1~A7이 한 건 어긋나 목록 7건 전부를 등재), 코드 PR 4건(#75 `2788384` · #76 `0f8b7c2` · #77 `5c48f8b` · #78 `f60a6a4`, 전부 2026-09-25 머지), 읽기 전용 실측 2건(pad 비율 · Zeroth 다운로드 검증), 보드 직접녹음 세션 1건이 나왔고 전부 SSoT 미등재였다. 본 Set은 `docs/decisions.md`에 33.16 · 33.17 두 절과 5.3(d) · 27.8(s) 두 소절을 신설하고, 기존 줄 20곳에 append · 포인터(그중 표 행 2곳은 마지막 셀 끝에 덧붙임, 1곳은 취소선 + 정정)를 적용하고, 27.8(r)에 bullet 2개를 추가한 뒤 본 엔트리를 남긴 것이 전부다(문서 2파일, 코드 0줄).

Step 0 가정 대조 — 착수 시점 HEAD `f60a6a4` = origin/main · `main` · 워킹트리 clean · 원격 브랜치 main 하나(`git ls-remote --heads`). `git log a3b1721..HEAD -- docs/` = 0커밋(docs 무변경 확인). decisions-log 마지막 엔트리 = 2026-09-25 (금) PoC-(57) Set 1. PR #75~#78 = 전부 MERGED, 머지 커밋이 위 4해시와 일치. 수치 교차 검산(PR 본문 · `git show --numstat` · repo 밖 원본) 결과 불일치 0건 — §9 정지 트리거 미발동. 교차 검산이 불가했거나 범위를 좁혀 적은 것은 아래 원본 대조에 적었다.

요일 검산: 2026-09-25 = 금요일(직전 엔트리 헤더와 동일 날짜).

신설 번호 결정 (근거유형 = 실측 grep `^## 카테고리|^### `): 33.16 · 33.17 — 카테고리 33은 33.15까지였다. 코드가 머지된 구현 사실(33.16)과 코드 0줄 결정(33.17)을 두 절로 가른 근거 = 33.12(구현) / 33.13(결정) 선례. 결정 중 녹음 대상 정의 1건은 직접녹음 세션 기록과 함께 5.3(d)에 두었다 — 제품 밖 수집 도구 · 기록은 제품 카테고리를 피한다(5.3(c) 선례). 5.3(d) — 5.3 소절이 (a)~(c)라 다음 문자. 27.8(s) — 27.8 소절 문자 최대가 (r)이라 다음 문자. 결정 번호는 A1 · A2 라벨이 5.1의 도어벨 유닛명 A1 · A2와 겹쳐 SSoT에 옮기지 않고 33.17 소절 문자((b)~(g))로 대체했다.

엔트리 형식 · 날짜 처리 판정 (근거유형 = 실물 선례): 헤더 = 「2026-09-25 (금) 후속 — PoC-(58) Set 1」. 근거 = (1) 같은 날 두 번째 엔트리다(첫 엔트리 = PoC-(57) Set 1) → 요일 뒤 「후속」 (2) PoC-(58)의 첫 Set 1이다 → 「Set 1 후속」을 붙이지 않는다. 판정 기준은 실물 헤더 이력에서 읽었다: 「(요일) 후속」 = 같은 날 두 번째 이후 엔트리(2026-09-18 (금) 후속 · 2026-09-19 (토) 후속 · 2026-09-20 (일) 후속), 「Set 1 후속」 = 같은 회차의 두 번째 이후 Set 1(2026-09-19 (토) — PoC-(53) Set 1 후속 · 2026-09-20 (일) — PoC-(54) Set 1 후속). 날짜 첫 엔트리이면서 「Set 1 후속」인 두 헤더가 이 구분의 실증이다. 본 엔트리는 볼드 · 취소선 마커를 리터럴로 쓰지 않았다(파리티 계수 오염 방지).

find-skills: `npx skills search` 3질의(`decision log update` · `markdown append` · `korean text integrity`) 전건 0건(③). 대조 질의 `flask` = 결과 다수(`flask-setup` 등)로 도구 생존 확인. 채택 0 · install 0.

### 결정 (결정일 2026-09-25, 주체 = 사용자 — Claude 권고 수용, 근거유형 = 사용자 결정)

- 재학습 착수 기한 = 10월 10일 전후. 도어벨 4유닛이 그때까지 안 오면 집 세대 현관 초인종 직접녹음 + 노크 직접녹음으로 1차 재학습 착수(33.17(b)). 논증 = 부스 자동 트리거(6.3(r) ⓒ) · `other` 계약 PR(33.13)이 전부 재학습 뒤.
- 3클래스 비교 학습 채택(33.13(d) 권고 → 확정, 33.17(c)).
- 한국어 말소리 네거티브 출처 = Zeroth-Korean(33.14(f) 출처 축 닫힘, 33.17(d)). 잔존 = 화자당 N · 발화당 상한 값(권고 N = 4 · 상한 1 → 약 459조각, 확정 아님) + 실데이터 실행.
- 세대 현관 호출음 = `doorbell` 녹음 대상 / 공동현관 · 경비실 = 녹음 안 함. 공동현관류의 오경보 네거티브 사용은 미정(5.3(d), 5.2(g) 표 행 부분만 닫힘).
- 네거티브 조각 상한 = 3(잠정) + 균등 간격 선택(33.7(i) 잠정값, 33.17(e)). 재판정 트리거 = 재학습 후 E4 지표 ②.
- `other` 계약 PR 직전 3건 = 목록 표시 한다 · `SkipReasonCounts`에 `not_target` 키 + 클래스 분포 `other` 칸 · mock 출력 한다(`PREDICTED_CLASSES` 단일 출처). 화면 문구 = 계약 PR 때(33.13(d) 해소, 33.17(f)).
- Zeroth 그룹 키 = 화자 단위(33.12(a) D3 확장, 33.17(g)). 멘트 제외 사유 라벨 = `aihub_intro_speech`(33.15(d) 라벨명 해소).

### 구현 (근거유형 = 실측 — PR 본문 · `git show --numstat`)

- PR #75 `2788384`: `ml/curation/slice_negatives.py` +247 · `tests/test_slice_negatives.py` +172 · `ml/curation/README.md` +33 / −1. Ran 10 OK · NC 12변형 전건 검출 · 실데이터 실행 0.
- PR #76 `0f8b7c2`: `ml/pipeline/config.py` +19 / −11 · `split.py` +25 / −3 · `augment.py` +2 / −2 · `README.md` +6 / −6 · `tests/test_pipeline.py` +59. 멘트 제외 · 클래스별 pitch 마커 · `SAMPLE_WEIGHT_RANGE` 삭제. NC 9변형 전건 검출.
- PR #77 `5c48f8b`: `ml/curation/zeroth_negatives.py` +308 · `tests/test_zeroth_negatives.py` +236 · `ml/pipeline/config.py` +11 · `tests/test_pipeline.py` +33 · `ml/curation/README.md` +41. Ran 12 OK · PASS 15 · NC 8변형 전건 검출.
- PR #78 `f60a6a4`: `ml/curation/README.md` +4 / −4 · `ml/training/README.md` +4 / −4 · `gate_axis_sweep.py` +2 / −2 · `record_receiver.py` +1 / −1. docstring 제거 AST 해시 전후 동일.
- 상세 = 33.16. 증감은 numstat 값이며 PR 본문의 표기와 교차 검산했다(--stat 막대 숫자 미사용).

### 실측

- 직접녹음 세션(2026-09-25 20:16~20:50): 수신기 ④런타임 첫 실동작. `/detect` http=200 19건 · `/enrich` http=201 18건(`monitor.log` 계수) · post 18. 11번 = 진짜 소리의 1샘플 클램프 5개(잘린 점 제외 최대 32,262) — 6.3(q) 한계의 첫 실측. 음량 · onset · 한계 = 5.3(d).
- pad 비율(15:57, repo 밖 `pad_ratio/result.txt`): 위임 수치 전건 일치(33.17(e) 표).
- Zeroth 다운로드 검증(학부생 셸 보고): 403 → 미러 200 · 10,339,720,618 B · `gzip -t` 통과 · 화자 105 / 10 · flac 22,263 / 457(33.17(d)). repo 밖 로그 파일이 없어 원본 대조는 못 했다.
- `record_receiver --self-test`(15:20 학부생 노트북): PASS 줄 48 · 「전건 통과」(합계 숫자 미출력).
- [논증] 7.5(e) 재발급 관측 창 ≈ 2026-10-10 01:41 UTC 이후(입력 = 문서 인용 「잔여 1개월 미만」 + 만료 2026-11-10 01:41:19 UTC).

### 신규 미결

- 없음. 글리치 혼입 우려(`enrich_uplink` ToF 조건)는 이 세션 18테이크 · 약 92초에서 미관측이라 신규 미결로 올리지 않고 관찰로 기록했다(5.3(d), 재확인 트리거 = 녹음 세션마다 `take_check.py`).
- 남은 미결(기존): 33.7(i) 실행 축(직접녹음 모드 도구 · 실데이터 실행) · 33.14(f) N · 상한 값 · 5.2(g) 공동현관류 네거티브 사용 · 장치 설치 위치(5.2(a) · 33.13(b)).

### 정정

- 33.7(h) 「산술 정합」 — 취소선 1쌍 + 정정. 판정 근거 = 작성 시점에 이미 틀렸다: 같은 (h)의 G10 · G11~G15 원천 귀속이 `audioset` 원천의 `fire_alarm` 키를 적고 있었고, `02_preprocessed`는 그 뒤 무변경(33.12 · PR #76의 3회 대조). 실측 = 02 `fire_alarm` 1,642 = AI Hub 1,349 + AudioSet 290 + FSD50K 3(파일명 기준, `pad_ratio/result.txt`). 33.15(b)의 「판정은 다음 Set」에 판정 완료 포인터.
- 27.8(r) — PoC-(57) 잔여 2건 추가(⑦ 「33.10에 pad 서술 0건」 부재 오판 · ⑧ 노션 정정 위임의 「+142 정확히 3건」 불변식 겹침). 헤더 「6건」은 작성 시점 계수라 취소선 없이 현재값 8건을 덧붙였다.
- PoC-(57) 잔여 (iii) — a3b1721 커밋의 증감을 노션에 +126이 아니라 +142로 적은 오기(--stat 합계 오독 — 142 = +126 + −16의 변경 줄 수). 노션 3곳 정정 완료(노션 = Set 3 소관, 본 Set은 기록만). 실측 `git show --numstat a3b1721` = `docs/decisions.md` +126 / −16 · `docs/decisions-log.md` +94.
- 직전 엔트리(2026-09-25 PoC-(57) Set 1)의 「엔트리 형식 · 날짜 처리 판정」은 「후속」 · 「Set 1 후속 N」을 둘 다 같은 날 두 번째 이후 엔트리의 접미로 적었으나, 실물 이력에서 「Set 1 후속」은 같은 회차의 두 번째 이후 Set 1을 뜻한다(위 판정 근거의 09-19 · 09-20 헤더). 정정 방식 = 본 엔트리에 기록하고 직전 엔트리 본문은 고치지 않는다 — 근거 = 이 로그는 엔트리를 시간순으로 덧붙이는 기록이고, 로그 본문에는 취소선 마커를 쓰지 않는다(파리티 오염 방지). 그 엔트리의 헤더 자체(첫 엔트리 · 접미 없음)는 두 기준 어느 쪽으로도 맞다.

### 원본 대조 결과 (위임 본문 vs PR 본문 · numstat · 코드 · repo 밖 원본)

- 수치 불일치 0건. 대조 원본 = `gh pr view 75/76/77/78` · `git show --numstat` 4커밋 · repo 밖 `pad_ratio/result.txt` · `direct_rec/`의 `notes.md` · `manifest.csv` · `take_check_result.txt` · `take11_result.txt` · `take_check.py` · `take11.py` · `monitor.log`(네트워크 식별 정보는 옮기지 않음) · `ml/pipeline/config.py` · `ml/pipeline/split.py` · `server/tools/record_receiver.py` · `ml/training/README.md`.
- 관찰 1 — clip 정의: `record_receiver.py`의 manifest `clip` = 값이 32767 또는 −32768인 샘플 수(`v in (32767, -32768)`)다. 위임이 확인을 요청한 `|x| ≥ 32767`과 달리 −32767은 세지 않는다. 분석 스크립트(`take_check.py` · `take11.py`)는 `|x| ≥ 32767`. 11번 5개는 +32767 ×4 · −32768 ×1이라 두 정의 모두 5(5.3(d)에 병기).
- 관찰 2 — PR #76 새 분할의 source 수(괄호 값)는 PR 본문에 없다 — MCP 보고 원본으로 표기하고 교차 검산 불가를 적었다(33.16(b)).
- 관찰 3 — PR #75 Step 0의 「FSD50K dev wav = PCM · 44.1kHz · mono · 16bit」는 헤더 표본 3개 기준이다 — 전수로 읽히지 않게 범위를 적었다(학습 21).
- 관찰 4 — 위임 배경 「결정 6건」 ≠ 결정 목록 7항목. 목록 전부를 등재했다(33.17 = 6건 + 5.3(d) = 1건).
- 관찰 5 — Zeroth 다운로드 · 해제 시각과 바이트 수는 학부생 셸 보고이며 repo 밖 로그 파일이 없어 대조하지 못했다. 화자 · flac 수는 PR #77 Step 0(22,720 전수)과 일치.

### SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 대상 코드 변경 0)

- V1 앵커 — 위임 §4 앵커 전건을 편집 전 `str.count` == 1로 assert했다(다중 매치 앵커는 줄 고유 문자열로 범위를 좁힘 — 아래). 스크립트 abort 0회, 라인 번호 앵커 0건.
- V2 삭제 0줄 증명 — 제거 20줄은 전부 같은 줄의 in-place 수정(줄 끝 append 17 · 표 행 마지막 셀 덧붙임 2 · 취소선 1)이다. 토큰 보존 = 마커 제거 기준 손실 0 / 원시 기준 손실 2(`aihub` · `없음.` — 취소선 마커가 토큰에 붙은 것, 내용 손실 아님). 원시 부분 문자열 기준으로는 17줄이 새 줄에 그대로 남고, 표 행 2줄은 끝 구분자 `|` 바로 앞에 셀 내용이 덧붙어 그 앞부분이 그대로 남는다.
- V3 파리티 — `decisions.md` 착수 전 취소선 포함 줄 109 / 취소선 마커 278 / 볼드 마커 13,880 → 종료 110 / 280 / 14,530(계수 단위 = 줄 수 / 마커 출현 수). 취소선 +2 = 1쌍(33.7(h)만). 볼드 +650 = 325쌍, 짝수. 추가 줄 전부 줄 단위로 두 마커 수가 짝수(홀수 줄 0).
- V4 한글 전수 검증 — 추가분에서 착수 전 파일에 없던 음절 6개: 괄 U+AD04 · 냐 U+B0D0 · 먹 U+BA39 · 섬 U+C12C · 척 U+CC99 · 틱 U+D2F1(괄호 · 아니냐 · 먹지 · 체크섬 · 진척 · 휴리스틱 문맥, 전건 정상). 기존 문서에 없던 한글 단어 190개를 코드포인트 · 문맥과 함께 전수 출력해 통독했고 전건 정상 · 깨진 글자 0. 바이트 보강 = U+FFFD 0 · 제어문자 0 · NFC 정규화 동일 · 역슬래시 u 이스케이프 0 · 낱자모 0.
- V5 수치 재검산(스크립트) — 조각 분포 합 1,100 · 상한별 총 조각 10칸 · 새 분할 합계 · −35 · Zeroth 발화 수 22,720 · 조각 합 51,868 · 459 · 음량 · onset 경계(시작 + 3 > 5.12) · 1,642 = 1,349 + 290 + 3 · 1,349 / 171 ≈ 7.9 · 관측 창 날짜 — 전건 일치.
- V6 공개 문서 검사 — 추가분(두 파일)의 개인 절대경로 0 · 사설 IP(핫스팟 대역 포함) 0 · SSID 0 · 화자 실명 0(Zeroth 이름 칸은 열지 않았다) · 이메일 0.
- V7 git status — `docs/` 2파일만 스테이징(`git add docs/decisions.md docs/decisions-log.md`).

### 비범위

- 코드 · 설정 · 데이터 · 녹음 파일 무접촉. 노션(Set 3) · 지침(Set 2) 무접촉. repo 밖은 읽기만(쓰기 0), Zeroth `AUDIO_INFO` 미열람.
- 기록만 하는 잔존(재학습 PR 소관): `ml/training/README.md`의 「repo 밖 형제 폴더(EPERM)라 실행 불가」 · 「SpecAugment 발표 전 결정」(33.3② 확정과 어긋남) · 「미결」 절 제목 아래 확정 항목.
- 확정하지 않은 것: 화자당 N · 발화당 상한 값 · 공동현관류 네거티브 사용 · 장치 설치 위치 · 직접녹음 조각내기 규칙(창 당기기 · 미검출 기록 — 설계 입력) · 화면 문구 · PR #75 성격 어휘.

학습 적용: 학습 13(앵커를 줄 고유 문자열로 좁혀 count == 1 — 「(f) 한계 (전건 명기)」 4건 · 「결정 확정 (2026-07-07 …)」 2건 · 「2026-10월 초 갱신 시」 2건 · 「171 × 약 9.6조각」 2건을 각각 범위 한정, 소절 제목 속 「(c)」 오독 사례를 27.8(s)①에 기록) / 학습 16(신설 절 형식 · 엔트리 헤더 · 27.8 bullet 형식을 실물 마지막 항목에서 읽음, 결정 라벨은 5.1 유닛명과 겹쳐 소절 문자로 대체) / 학습 19(33.7(h) 취소선 전제를 같은 소절의 G10 · G11~G15 귀속과 02 무변경으로 재검증) / 학습 21(FSD50K 헤더 = 표본 3, 글리치 미관측 = 이 세션 · 이 보드 한정, clip 정의 = 32767 · −32768만).

### 관련 카테고리

카테고리 5 머리(`SAMPLE_WEIGHT_RANGE` 삭제 포인터) / 5.2(g)(클래스 정의 행 부분만 닫힘) / 5.3(b)(pitch 마커 해소) · (c)(④런타임 채워짐) · (d)(신설) / 6.3(q)(단일 샘플 클립 첫 실측) / 7.5(e)(재발급 관측 창) / 27.8(r)(⑦ ⑧) · (s)(신설) / 33.3①(현재값) / 33.7(h)(취소선 + 정정) · (i)(잠정 상한 · 실행 축 진척) / 33.12(a)(D3 확장) · (c)(현재값 포인터) / 33.13(d)(대기 3건 해소 · 권고 채택) / 33.14(f)(출처 축 닫힘) / 33.15(b)(판정 포인터) · (d)(라벨명 · 구현) / 33.16 · 33.17(신설) / 학습 13 · 16 · 19 · 21.

### 관련 commit

`2788384`(PR #75) · `0f8b7c2`(PR #76) · `5c48f8b`(PR #77) · `f60a6a4`(PR #78, 착수 시점 HEAD)가 본 Set의 등재 대상이다. `a3b1721`은 증감 오기 기록 대상. 문서 반영 = 본 커밋(docs-only, PR 없음, `main` 직 push).

## 2026-09-26 (토) — PoC-(59) Set 1 — 레벨 정규화 대조 실험 · 사용자 결정 8건 등재(33.18 신설 — 서빙 정규화 기각 · 보드 배경 `other`(D-a) · `other` 증강 SNR(D-b) · E4 지표 ③④ · 화면 문구 · announcer · EC2 제외 · 재학습 당일 순서) + 재학습 준비 PR 3건 등재(33.16(e)~(g) — #79 직접녹음 조각 · #80 학습 CLI 계약 · #81 보드 배경 조각) + 초안 · 미머지 PR 2건 등재(33.16(h)(i) — #82 · #83) + 33.7(i) 도구 축 닫힘 · 33.13(d) 라벨 재생성 우려 코드상 해소 · 33.17(f) 화면 문구 축 닫힘 + announcer 신규 미결(8.3) + `knock` 관통 재해석 경계(6.7(d) · 6.4(g) · 26.10(d)) + ml 테스트 10종 현재값(17.2) + 27.8(t) 위임 · 판단 오류 9건 (카테고리 5.2/5.3/6.4/6.7/7.4/7.5/8.3/8.7/17.2/26.10/27.8/30.1/33.6/33.7/33.8/33.11/33.12/33.13/33.16/33.17/33.18)

PoC-(59)는 2026-09-26 하루의 회차다. 사용자 결정 9건(서빙 정규화 기각 · D-a · D-b · E4 지표 추가 · 화면 문구 · announcer · EC2 제외 · 재학습 당일 순서 · 카카오 refresh 노션 트리거 이동), 머지 PR 3건(#79 `83888a5` · #80 `4ef872e` · #81 `c6cd4d4`, 전부 2026-09-26 머지), 초안 · 미머지 PR 2건(#82 · #83), repo 밖 실측 · 자산 4건(레벨 정규화 대조 실험 · YAMNet 사전 점검 · E4 평가기 · 작업 전수 조사)이 나왔고 전부 SSoT 미등재였다. 본 Set은 `docs/decisions.md`에 33.18 한 절과 33.16(e)~(i) · 27.8(t) 소절을 신설하고, 기존 절 20곳에 새 줄로 append · 포인터를 넣은 뒤 본 엔트리를 남긴 것이 전부다(문서 2파일, 코드 0줄). 기존 줄은 한 줄도 고치지 않았다(아래 V2).

Step 0 가정 대조 — 착수 시점 HEAD `c6cd4d4` · `main` · 워킹트리 clean. `docs/decisions.md` · `docs/decisions-log.md` 마지막 변경 커밋 = 둘 다 `ebcdfa1`. 두 파일의 「#79」 ~ 「#83」 출현 = 전부 0. 카테고리 33 마지막 절 = 33.17, 27.8 마지막 소절 = (s). repo 밖 원본 4폴더 · `gh pr view` 5건 전부 읽기 가능(#79 ~ #81 = MERGED, 머지 커밋이 위 3해시와 일치 / #82 · #83 = OPEN · draft). 불일치 0건 — 정지 트리거 미발동. 인용 앵커 부재는 2건(아래 원본 대조 관찰 1 · 2)이며 둘 다 신규 등재 · 다른 절로 pivot했다.

요일 검산: 2026-09-26 = 토요일.

신설 번호 결정 (근거유형 = 실측 grep `^### 33\.` · 27.8 소절 문자): 33.18 — 카테고리 33은 33.17까지였다. 같은 날 산출을 성격으로 갈랐다 — 측정 · 결정은 33.18(33.13 · 33.17 선례, 코드 0줄 결정 절), 구현(PR)은 33.16에 소절 (e) ~ (i)로 이었다(33.16의 성격 = 재학습 당일 도구 · 규칙을 미리 머지한 PR 행). 초안 · 미머지 PR 2건도 같은 구현 성격이라 33.16 (h) · (i)에 두되 소절 제목과 33.16 머리 추가 인용 줄에 초안 · 미머지를 명기했다. 27.8(t) — 27.8 소절 문자 최대가 (s)라 다음 문자. 33.16의 제목 「4건」은 신설 시점 계수라 고치지 않고 머리 추가 줄에 그 사실을 적었다.

엔트리 형식 · 날짜 처리 판정 (근거유형 = 실물 선례): 헤더 = 「2026-09-26 (토) — PoC-(59) Set 1」. 근거 = (1) 2026-09-26의 첫 엔트리다 → 요일 뒤 「후속」 없음 (2) PoC-(59)의 첫 Set 1이다 → 「Set 1 후속」 없음. 판정 기준은 직전 엔트리(PoC-(58))가 적은 실물 헤더 이력 판정을 그대로 따랐다. 본 엔트리는 볼드 · 취소선 마커를 리터럴로 쓰지 않았다(파리티 계수 오염 방지).

find-skills: `npx skills search` 3질의(`decision log update` · `markdown append` · `korean text integrity`) 전건 「No skills found」(명령 실행 성공 · 결과 0건). 대조 질의 `flask` = 25건으로 도구 생존 확인. 채택 0 · install 0. macOS에 `timeout` 명령이 없어 첫 호출은 명령 부재로 실패했고, timeout 없이 재실행한 결과가 위다(「0건」과 「명령 부재」를 가름).

### 결정 (결정일 2026-09-26, 주체 = 사용자 — 권고 수용 또는 위임, 근거유형 = 사용자 결정)

- 서빙 정규화 안 함 — 데이터 근거 = 레벨 정규화 대조 실험(33.18(c)). 5.2(d) 비대칭은 사실로 남긴다.
- D-a 보드 배경음(INMP441 잡음 바닥)을 `other` 네거티브에. 소재 = 소리 없이 `s`만 누른 전용 배경 테이크의 post(5.12초) → 3초 창, pre(2.048초)는 3초 미만이라 안 씀. 그룹 키 = 유닛(`boardbg_{유닛}`), 장소 · 날짜별 유닛 2~3개 권고, 평가는 학습에 안 쓴 세션(33.18(d)).
- D-b `other` 증강 SNR (10.0, 20.0) — 양성만 증강하면 증강 흔적이 가짜 단서(33.18(d)).
- E4 지표 ③ 보드 무음 창 오발송 수 · ④ 보드 실초인종 검출 추가. 합격 수치는 재학습 후 그대로(33.13(a) · 33.18(e)).
- 화면 문구(사용자 위임): `other` = 「기타 소리」 · `not_target` = 「알림 대상이 아닌 소리」 → 33.17(f) 화면 문구 축 닫힘, 반영은 초안 #83(미머지)(33.18(f)).
- 스크린리더 announcer는 `other` 행을 읽지 않는다(사용자 위임, 초안 #83 커밋 `93748ce`, 미머지). 근거 = 33.13(a) E1(33.18(f)).
- EC2 = 이번 개발 범위(10월)에서 제외 · 발표 서빙 = Cloudflare 터널(권고 수용) — 7.4(c) 경계에 대한 명시적 결정. 30.1의 11/13 트리거는 발표일이 그 뒤일 때만 유효, 유지(33.18(g)).
- 재학습 당일 순서 = 초안 PR #82 본문 순서: #82 머지 → `run_all`(실데이터) → 3 · 4클래스 학습 → 모델 확인 → #83 머지 + `gate_axis_sweep` 4클래스 대응 같은 날 → ④런타임(33.18(h)).
- 카카오 refresh 노션 DB3 트리거 10/01 → 10/11(관측 창 = 10/10 01:41 UTC 이후와 정합). 노션 편집 = Set 3 소관, decisions.md는 7.5(e) 한 줄만.

### 구현 (근거유형 = 실측 — PR 본문 · `git show --numstat`, 초안은 문서 인용)

- PR #79 `83888a5`: `ml/curation/slice_direct.py` +277 · `tests/test_slice_direct.py` +243 · `ml/curation/README.md` +42. Ran 13 OK · NC 10변형 전건 검출 · 실데이터 쓰기 0. 9/25 dry-run 대조 = planned 17 · `rejected_no_onset` 1(19번) · shifted 3(02 · 05 · 17). 33.7(i) 도구 축 닫힘, 값 3개와 실데이터 실행은 재학습 당일.
- PR #80 `4ef872e`: `ml/training` 11파일(README +66 / −21 · `config.py` +74 / −20 · `data.py` +29 / −25 · `evaluate.py` +34 / −21 · `export.py` +21 / −11 · `train.py` +29 / −22 · `__init__.py` +3 / −3 · `model.py` +3 / −3 · `tests/test_class_subset.py` +213 · `tests/test_export_smoke.py` +8 / −5 · `tests/test_training_smoke.py` +6 / −6). `--classes` 필수 · 산출 폴더 필수(repo 기본 경로 fallback 제거) · 덮어쓰기 거부 · 빈 클래스 즉시 실패 · 실행별 `labels.json` 단일 출처. 서빙 모델 폴더 9파일 sha256 전후 동일. NC 11건 전건 검출.
- PR #81 `c6cd4d4`: `ml/curation/slice_boardbg.py` +220 · `tests/test_slice_boardbg.py` +198 · `ml/curation/README.md` +48 · `ml/pipeline/config.py` +12 · `tests/test_pipeline.py` +31. `BG_NOISE_SNR_DB["other"]` · `source_key` `boardbg_` 분기(기존 2,792 stem 키 불일치 0). 9/25 post dry-run = `rejected_sound` 16 · `rejected_clamp` 1(11번) · 통과 1(19번, 피크 614). NC 7건 전건 검출.
- 초안 · 미머지 PR #82(`feat/ml-other-class`, 커밋 4개): `CLASSES` 4 · 더미 `other` 1500(테스트 픽스처) · 3클래스 가정 단언 4클래스화 · 서버 계약 순서 테스트 · 4클래스 더미 관통 · curation 서술 정정 · 더미 `PER_CLASS` 20 → 22(해시 고정 배정이라 특정 split 0이 실제로 난다).
- 초안 · 미머지 PR #83(`feat/server-other-contract`, 커밋 3개): 게이트 맨 앞 `not_target` · 추론 상수 프로즌 예외 · `PREDICTED_CLASSES` 리터럴 복제 + 일치 테스트 · mock 분배 2칸 고정 KeyError 수정 · 대시보드 `AudioLines` · 중립색 · `STATS_CLASS_ORDER` 분리 · 회귀 112(106 + 6).
- 상세 = 33.16(e) ~ (i). 증감은 numstat 값이며 PR 본문과 교차 검산했다.

### 실측 (repo 밖 원본 = `~/ddingdong-측정결과/2026-09-26/`)

- 레벨 정규화 대조 실험(12:57 KST, `level_norm/summary.md` · `summary.json` · `run.log`): 대조군 재현 = test 376 / 424 · 367 / 27 / 30 · OOD 102 / 20 · OOD 행 불일치 0 / 126 · 실모델 2회 일치. 초인종 창 17 = top-1 16 → 16 · 발송 16 → 16 · `p_doorbell` 평균 0.9171 → 0.9362. 보드 배경 19 = 원본 `knock` 19(presence=true 발송 19 · presence=false 0) → 정규화 `fire_alarm` 14 / `knock` 5 · presence=true 15 · presence=false 12. OOD presence=true 102 → 57 · presence=false 20 → 14. test 424 무변화(최대 확률 차이 2.31e-05). 반올림 갈림 = 정규화 조건 보드 배경 1행(09번)뿐(33.18(b)).
- 파생 append: 33.6(e) 반올림 갈림 첫 실입력 사례(정규화 조건 한정, 현 서빙 경로 0) · 5.2(d) 보드 쪽 영향 첫 측정 · 26.10(e) 수동 `s` 순서 = 소리 → `s` → 말(논증) · 6.7(d) · 6.4(g) · 26.10(d) = 2026-09-18 #1은 `knock` 분류 입력의 게이트 경로 실기기 관통 관측이나 1차 입력이 실제 노크였는지 미확정, 「knock 실기기 분류 검증」으로 쓰지 않는다, `doorbell` 실기기 관통은 여전히 미수행.
- YAMNet 사전 점검(14:25 KST, `yamnet_precheck/note.md`): 로드 11.5 s · 사본 `~/ML 학습 데이터/yamnet_hub/yamnet_1` · md5 `ad0726cb90288589ac217bfb2426cfb9` · 17M · 당일 `DDINGDONG_YAMNET_HANDLE` 사용 · 재학습 전 ml venv 업그레이드 금지(33.18(h)).
- E4 평가기(16:34 KST, `e4_eval/usage.md` · `dryrun_current/summary.md` · `metrics.json`): 서빙 동일 경로 · 클래스 짝 가드 · 2회 `metrics.json` 바이트 동일(`cmp`) · 시운전값 = 레벨 정규화 실험 원본 조건과 전 항목 일치 · 3클래스 모델은 test `other` 폴더를 평가하지 않음 · 4클래스 경로 미시운전(33.18(e)).
- 작업 전수 조사(`task_inventory/inventory.md`): 고유 137행(106 · 13 · 18) · 판정 보류 9 · DB3 83행 = 🔴 9 / 🟡 38 / 🟢 36. 33.11에 소재 · 계수만 append, 개별 항목 반영은 다음 Set 1.

### 신규 미결

- announcer가 `notification_status`를 보지 않는다 — 미발송 행(저신뢰 `fire_alarm` 포함)도 announce되고 저신뢰 `fire_alarm`은 assertive로 읽힌다. 기존 동작 · 사용자 판단 대기. 위치 = 8.3(Phase B 접근성 절 — announcer B-1a가 등재된 자리). 근거 = main `NotificationAnnouncer.tsx` 실측(분기 기준 = `predicted_class`뿐) + PR #83 본문.
- 남은 미결(기존): 33.7(i) 실행 축(실데이터 실행 · 값 3개) · 33.14(f) N · 상한 값 · 5.2(g) 공동현관류 네거티브 · 장치 설치 위치(5.2(a) · 33.13(b)) · 33.6(e) 반올림(재학습 후 판단).

### 정정 · 현재값

- 취소선 0건. 이력 서술은 전부 무변경이고, 바뀐 사실은 새 줄의 현재값 append로만 적었다 — 33.13(d) 「`CLASSES`를 먼저 4로 바꾸면 … 어긋난다」(PR #80으로 코드상 해소) · 17.2 「5종 동시 실행 기록은 없다」(PR #81 세션에서 10종 전건 통과로 해소) · 33.12(d) 종수 포인터 · 33.7(i) 실행 축 진척 · 33.17(f) 화면 문구 축.
- 33.8: `server/tools/gate_axis_sweep.py`의 `labels_path()` 코드 안 repo 기본 경로 fallback + docstring 「`ml.training.config.model_dir()`과 같은 규칙」이 PR #80으로 거짓이 된 사실을 append(수정 = 계약 PR · 재학습 후 4클래스 대응 소관).
- 8.7(a): 2차 부재 경로 3 → 4는 초안 #83에만 있어 표는 고치지 않고 「머지 시 갱신」 포인터만 넣었다.

### 원본 대조 결과 (위임 본문 vs PR 본문 · numstat · 코드 · repo 밖 원본)

- 수치 불일치 = 위임 교차 검산값 전건 일치, 단 아래 1건은 원본이 없어 등재하지 않았다. 대조 원본 = `gh pr view 79 ~ 83` · `git show --numstat` 3커밋 · 초안 브랜치 파일(원격 ref 읽기, 체크아웃 없음) · repo 밖 `level_norm/` · `e4_eval/` · `yamnet_precheck/` · `task_inventory/` · `ml/curation/slice_direct.py` · `ml/curation/README.md` · `ml/curation/slice_boardbg.py` · `server/tools/record_receiver.py` · `server/tools/gate_axis_sweep.py` · `dashboard/src/components/notifications/NotificationAnnouncer.tsx` · `firmware/include/uplink_common.h`.
- 관찰 1 — 「3클래스 최악 673B · other 656B」: PR #83 본문 · diff · 커밋 메시지 · 코멘트 어디에도 없다. 로그 없는 수치라 등재하지 않았고, 원본이 있는 4클래스 최악 686 B · 버퍼 768 B(펌웨어 상수 `UPLINK_RESP_BUF_BYTES`) · 여유 10.7%만 적었다.
- 관찰 2 — 「6.x 응답 버퍼 절」: SSoT에 `/detect` 응답 버퍼를 다루는 절이 없다(「768」 · `UPLINK_RESP_BUF_BYTES` 0건, 가장 가까운 것은 5.3(a)의 「보드 수신 한계 767 B」). 새 절을 만들지 않고 초안 #83 소절(33.16(i))에 적었다.
- 관찰 3 — 위임의 「R03」은 SSoT에 0건인 인계 목록 식별자다. 33.16(i)에 식별자임을 밝혀 한 번만 적었다.
- 관찰 4 — 보드 배경 원본 `knock` conf 범위 0.80~1.00은 summary 표 19행의 최소 0.80(13번) · 최대 1.00과 일치.
- 관찰 5 — 17.2의 직전 현재값은 5종(2026-09-25 PoC-(57))이었고 그 뒤 PR #75 · #77이 2종을 더해 오늘 착수 전 7종이었다(PR #79 본문 「기존 ml 7종」). 오늘 신규 3종을 더한 10종을 `find` 실측과 대조했다.
- 관찰 6 — 위임의 announcer 신규 미결 근거는 PR 본문이었으나 main 코드 실측으로 보강했다(분기 기준이 `predicted_class`뿐).

### SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 대상 코드 변경 0)

- V1 앵커 — 편집 스크립트가 앵커 23개 전건을 `str.count` == 1 및 줄 단위 1건으로 assert한 뒤 삽입했다. abort 0회, 라인 번호 앵커 0건. 인용 문구 재조회에서 원문과 어긋난 자작 인용 4건(33.11 원칙 문구 · 카테고리 20 lock 문구 · PR #83 머리 문구 · 7.4(c) 경계 문구)을 원문 그대로 고쳤다.
- V2 삭제 0줄 증명 — `decisions.md` 추가 170줄 · 삭제 0줄(SequenceMatcher + numstat). 토큰 보존 = 원시 기준 손실 0 / 취소선 마커 정규화 기준 손실 0. 기존 줄의 in-place 수정 0건.
- V3 파리티 — `decisions.md` 착수 전 취소선 포함 줄 110 / 취소선 마커 280 / 볼드 마커 14,530 → 종료 110 / 280 / 15,228(계수 단위 = 줄 수 / 마커 출현 수). 취소선 증감 0. 볼드 +698 = 349쌍, 짝수. 추가 줄 전부 줄 단위 파리티 짝수(홀수 줄 0).
- V4 한글 전수 검증 — 추가분에서 착수 전 파일에 없던 음절 5개: 눠 U+B220 · 뗀 U+B5C0 · 뛴 U+B6F4 · 뜀 U+B700 · 밑 U+BC11(나눠 · 뗀 · 건너뛴다 · 건너뜀 · 밑줄 문맥, 전건 정상). 기존 문서에 없던 한글 단어 204개를 코드포인트 · 문맥과 함께 전수 출력해 통독했고 전건 정상 · 깨진 글자 0. 바이트 보강 = U+FFFD 0 · 제어문자 0 · NFC 정규화 동일 · 역슬래시 u 이스케이프 0 · 낱자모 0.
- V5 인용 재조회 — 추가분의 고유 낫표 인용 36개를 마커 제거 원본(SSoT 착수 전 사본 · PR 본문 5건 · 코드 · 초안 브랜치 `kakao.py` 원격 ref · repo 밖 md)에 재조회: 원본 적중 30 / 위임 본문 인용 6(위임 문장 자체를 catch 대상 또는 금지 표현으로 인용). 원본 부재 0.
- V6 초안 표기 — 「#82」 · 「#83」이 나오는 추가 줄 17줄 전부에 초안 또는 미머지 표기(grep).
- V7 공개 문서 검사 — 추가분의 개인 절대경로 0(repo 밖 경로는 `~` 표기) · 사설 IP 0 · SSID 0 · 토큰 · 시크릿 0.
- V8 git status — `docs/` 2파일만 스테이징(`git add docs/decisions.md docs/decisions-log.md`).

### 비범위

- 코드 · 설정 · 데이터 · 녹음 파일 무접촉. 노션(Set 3) · 지침(Set 2) 무접촉. repo 밖은 읽기만(쓰기 0), 초안 브랜치는 체크아웃하지 않고 원격 ref로만 읽었다.
- 다음 Set 1로 넘긴 것(인벤토리가 찾은 낡은 서술): 7.1 「미반영 유지」 · 30.4 「자연 수렴」 · 8.6의 7.7(k) 포인터 · 5.2(g) · 33.12(a) SAMPLE_WEIGHT · 8.7(a) 목록 API 필터 · 26.2 「PoC 5~6주차」 · 코드 주석 낡음 2건. 지침 8항 ①(33.7(h) 취소선 범위) · ④(포인터 없는 미결 6곳).
- 확정하지 않은 것: 보드 배경 유닛 수(권고 2~3) · 직접녹음 값 3개 · 화자당 N · 발화당 상한 · E4 합격 수치 · announcer `notification_status` 처리 방침.

학습 적용: 학습 13(인용 36개 전수 재조회 — 자작 인용 4건이 원문과 어긋나 원문으로 교체, 「6.x 응답 버퍼 절」 부재를 `768` · 상수명 · 「수신 한계」로 재조회한 뒤 pivot) / 학습 16(신설 절 · 소절 · 엔트리 형식을 실물 마지막 항목에서 읽음) / 학습 19(announcer 미결 근거를 PR 본문에서 main 코드로 재검증) / 학습 21(673B · 656B는 원본 부재라 미등재, 「knock 관통」은 1차 입력 미확정으로 범위 한정).

### 관련 카테고리

5.2(d)(보드 쪽 영향 · 서빙 정규화 기각) / 5.3(d)(설계 입력 채택) / 6.4(g) · 6.7(d) · 26.10(d)(`knock` 관통 재해석 경계) / 7.4(c) · 30.1(EC2 제외 결정) / 7.5(e)(노션 트리거 이동) / 8.3(announcer 신규 미결) / 8.7(a)(초안 #83 4번째 경로 포인터) / 17.2 · 33.12(d)(ml 테스트 10종) / 26.10(e)(수동 `s` 순서) / 27.8(t)(신설) / 33.6(e)(반올림 갈림 첫 실입력 사례) / 33.7(i)(도구 축 닫힘) / 33.8(`gate_axis_sweep` fallback) / 33.11(작업 전수 조사 소재 · 계수) / 33.13(a)(E4 ③④) · (d)(라벨 재생성 우려 해소) / 33.16(e)~(i)(신설) / 33.17(f)(화면 문구 축 닫힘) / 33.18(신설) / 학습 13 · 16 · 19 · 21.

### 관련 commit

`83888a5`(PR #79) · `4ef872e`(PR #80) · `c6cd4d4`(PR #81, 착수 시점 HEAD)가 본 Set의 등재 대상이다. 초안 · 미머지 PR #82 · #83의 브랜치 커밋은 33.16(h)(i)에만 적었다. 문서 반영 = 본 커밋(docs-only, PR 없음, `main` 직 push).

## 2026-09-27 (일) — PoC-(60) Set 1 — 작업일 2026-09-26의 초인종 등록 · 해제 재도입 · 등록 설계 8건 · 시연 소리 결정 등재(33.19 신설 — 노트북 통합 확인 · 판정 PR 신규 미결 포함) + 등록 구현 PR 5건 등재(33.20 신설 — #85 ~ #89) + `gate_axis_sweep` 4클래스 사전 대응 PR #84 등재(33.16(j)) · 초안 #83 본문 정정 · 재학습 당일 런북 포인터(33.18(h)) + 카카오 commit 가드 SSoT 등재(7.5(j)) + 검증 자산 기준선 · clean 빌드 축 해소(17.2) + 27.8(u) 위임 · 판단 오류 7건 (카테고리 4/5.1/7.5/8.3/8.4/8.7/17.2/26.2/26.3/26.6/26.9/26.10/27.8/33.5/33.8/33.13/33.16/33.18/33.19/33.20)

PoC-(60)은 2026-09-26(토) 하루의 작업이고, 자정을 넘겨 2026-09-27에 반영했다(발견일 · 결정일 = 2026-09-26 / 반영일 = 2026-09-27). 사용자 결정 3묶음(시연 소리 · 목표, 등록 · 해제 재도입, 등록 설계 8건 + 수집 트리거 문답), 머지 PR 6건(#84 `8312928` · #85 `8eaf5ea` · #86 `16ee0f7` · #87 `1b208f6` · #88 `29ef85c` · #89 `0990789`), 초안 · 미머지 #83 본문 정정 1건, repo 밖 자산 · 실측 4건(재학습 당일 런북 · 등록 기능 전수 조사 · 노트북 통합 확인 · 검증 자산 기준선)이 나왔고 전부 SSoT 미등재였다. 본 Set은 `docs/decisions.md`에 33.19 · 33.20 두 절과 7.5(j) · 33.16(j) · 27.8(u) 소절을 신설하고, 기존 절 19곳에 새 줄로 append · 포인터를 넣은 뒤 본 엔트리를 남긴 것이 전부다(문서 2파일, 코드 0줄). 기존 줄은 한 줄도 고치지 않았다(아래 V2).

착수 전 대조 — HEAD = origin/main = `0990789`(#89) · 워킹트리 clean · 원격 = main + `feat/ml-other-class`(`52e993d`, 초안 #82) + `feat/server-other-contract`(`93748ce`, 초안 #83) · docs 마지막 변경 = `a33cb45`(PoC-(59) Set 1). `gh pr view` 84 ~ 89 = 전부 MERGED(머지 커밋이 위 6해시와 일치), 83 = OPEN · draft. repo 밖 원본 폴더 전부 읽기 가능, 런북 393줄 · md5 `536efe2623d5284a6357a42921a8c37a` 일치, 전수 조사 `inventory.csv` 46행 + 헤더. 사전 계수 11항목(IndexError 3 · exit 2 1 · clean 빌드 2 · 등록 기능 제품 코드 1 · 구매 요건 1 · 도어벨 4유닛이 1 · FastDTW 1 · commit_is_safe 0 · backend exact 0 · 폴러 통합 5 · 한계 ④ 3)은 전부 실물과 같았다. 오늘 결정의 중복 등재 여부 = 「탁상」 · 「호출벨」 · 「registration」 · 「재도입」 0건, 「다이소」 1건은 2026-05의 브레드보드 구매 줄이라 무관. 불일치 0건 — 정지 트리거 미발동.

요일 검산: 2026-09-27 = 일요일.

신설 번호 결정 (근거유형 = 실측 grep `^### ` 전수 · 소절 문자): 카테고리 33의 마지막 절이 33.18이라 33.19 · 33.20. 성격으로 갈랐다 — 코드 0줄 결정 · 측정은 33.19(33.13 · 33.17 · 33.18 선례), 구현(PR)은 33.20(33.12 · 33.16 선례). 등록 기능을 카테고리 33에 둔 이유 = SP/DTW 스파이크(33.5)와 재학습 · 계약 PR 계열(33.13 ~ 33.18)이 모두 여기 있고, 판정 PR이 초안 #83(33.16(i))에 묶여 있다. PR #84는 등록 기능이 아니라 재학습 당일 도구를 미리 머지한 PR이라 33.16의 다음 소절 (j)로 이었다(33.16 머리에 추가 줄). 33.8(`gate_axis_sweep` 자산 절)은 fallback bullet 해소 append로만 연결했다. 7.5(j) = 7.5 소절 문자 최대가 (i)라 다음 문자, 가드 도입 PR이 7.5의 PR #40이다(`git log -S`). 27.8(u) = 27.8 소절 문자 최대가 (t)라 다음 문자.

엔트리 형식 · 날짜 처리 판정 (근거유형 = 실물 선례): 헤더 날짜 = 반영일 2026-09-27. 근거 = (1) 2026-09-25 PoC-(57) 엔트리가 여러 작업일을 반영일 헤더 아래에 「작업일 …」로 적은 선례 (2) 2026-09-27의 첫 엔트리라 요일 뒤 「후속」 없음 (3) PoC-(60)의 첫 Set 1이다. 본 엔트리는 볼드 · 취소선 마커를 리터럴로 쓰지 않았다(파리티 계수 오염 방지).

find-skills: `npx skills search` 4질의(`decision log update` · `markdown append` · `korean text integrity` · `changelog entry`) 전건 「No skills found」(명령 실행 성공 · 결과 0건). 대조 질의 `flask` = 25건으로 도구 생존 확인. 단어 질의 `changelog` = 98건이나 자동 생성류(`changelog-generator`)라 과거 미채택 사유(단일 decisions.md 누적 · 취소선 + append 규약과 충돌)와 같다. 채택 0 · install 0.

### 결정 (결정일 2026-09-26, 근거유형 = 사용자 결정 — 출처 = 인계 위임 본문)

- 시연 소리 · 목표(저녁, 재도입보다 앞): 시연 초인종 = 다이소 탁상 호출벨(개수 = 런북 1절 기재 4개) + 집 초인종 녹음 스피커 재생, 둘 다 `doorbell`로 학습. 목표 = 시연에서 잘 보이는 것. 집 녹음은 전부 학습에 쓰고 별도 평가 세션 없음, 발표 수치 = 공식 test set. train 고정 구현 방식은 재학습 당일 사용자 판단 대기. 파급 = 5.1 구매 요건 · 26.2 배치 · 33.17(b) 전제(append만, 5.1 요건 폐기 여부는 사용자 확인 대기). 탁상벨 미구매 · 녹음 0(33.19(b)).
- 등록 · 해제 재도입(21:18): 26.3 진입점 2의 2026-09-12 축소 결정을 뒤집는다. 시연 흐름 = 등록 → 등록한 벨 + 사람 있음 = 알림 / 다른 벨 + 사람 있음 = 차단 / 사람 없음 = ToF 차단 / 해제 → 다른 벨 등록(33.19(c)).
- 등록 설계 8건(21:40, Claude 권고 일괄 승인): ① numpy 재구현 ② 대시보드 시작 → 다음 N회 `/detect` 수집 · 만료 ③ 수집 중 1 · 2차 억제 ④ ToF AND 소리 일치, 확인 위치 = 정책 함수 밖 · 정책 뒤 · 카카오 앞 ⑤ 등록 없음 = 현행 / 불일치 = 차단 + 새 사유 ⑥ 설정 카드 ⑦ 전역 단일 등록 · 새 테이블만 · PCM BLOB ⑧ 판정은 #83(초안 · 미머지) 머지 뒤. 「지금」분 = #85 ~ #88로 이행 완료, ③ ④ ⑤ 미구현(33.19(d)).
- 등록 수집 트리거 = 수동 `s`(21:39 문답). 자동 트리거 도입 시 등록 코드 무변경(논증).
- Claude 설계 · 사용자 이의 없음(명시 승인 아님)으로 따로 적은 것: #86 수집 중 · 등록됨 상태의 재시작 = 409 · #87 잠정 요청값 목표 3 · 만료 300초.

### 구현 (근거유형 = 문서 인용 — `gh pr view` 본문 + 실측 repo 밖 로그)

- PR #84 `8312928`: `gate_axis_sweep` self-test 폭 = 클래스 수 · `not_target` 축 · 기준값 없는 축 기대 0 정확 비교 · 라벨 경로 fallback 제거(env 없으면 exit 3) · self-test 19 PASS · NC 8종 · 3클래스 실스윕 rows 바이트 동일(33.16(j)). 초안 · 미머지 #83 본문 「알려진 영향」 절 제목을 「범위 밖 — #84로 선해소」로 정정(전후 · 재조회 = repo 밖 `pr83_body/`).
- PR #85 `8eaf5ea`: `sound_match.py` numpy 멜 + exact DTW-cosine, 프로즌 동등성 픽스처, 테스트 7 · NC 8변형. 디지털 무음 프레임 자기 거리 ≠ 0(미수정). 기준값 실험은 `--backend exact`(33.20(a)).
- PR #86 `16ee0f7`: 새 테이블 2개 · 저장 층 commit 0 · API 3개 · 상태 본문 6키 · 상태 4종(같으면 expired) · 가드 1 ~ 10 / 1 ~ 600(안전 상한) · 비교 시간 실측표 · 옛 스키마 무영향 · 테스트 21 · NC 12(33.20(b)).
- PR #87 `1b208f6`: 설정 카드 · 표시 / 컨테이너 분리 · 서버 state 단일 판정 · 문구 잠정 · CSS 무변경 · NC 6 · 브라우저 ④런타임(33.20(c)).
- PR #88 `29ef85c`: `/detect` 계측 훅(카카오 뒤 · commit 앞) · 템플릿 수집 · `registration_matches` 거리 기록 · 예외 격리 + ERROR 로그 · 테스트 8 · NC 4 · 계측 비용(33.20(d)).
- PR #89 `0990789`: 낡은 주석 · README 정리, 동작 0줄(docstring 제거 AST 해시 동일)(33.20(e)).
- 서버 회귀 3모듈 합계 113 → 134 → 142, `test_detect_regression`은 106 그대로.

### 실측 (repo 밖 원본 = `~/ddingdong-측정결과/2026-09-26/`)

- 노트북 통합 확인(`registration_runtime/notes.md`, 기록 23:43:28 KST, main `29ef85c`): 실모델 · presence=false · 카카오 0 · 등록 3회 → 등록됨 → 거리 3행(a1 동일 0.0 · b1 다른 초인종 0.4417 · k1 노크 0.1806). 노크가 템플릿 a3와 0.18로 같은 초인종 다른 구간(0.26)보다 가까움 → 신규 미결. 공개 클립 · 각 1회 · 보드 경로 아님이라 기준값 근거 아님(33.19(f)).
- 검증 자산 기준선(`test_baseline/baseline.md`, HEAD `29ef85c`): 전건 rc 0 · 서버 142 × 2 venv · ml 10종 · 호스트 7종 checks(6.3(o) 일치) · 대시보드 3종 · 펌웨어 14 env clean 뒤 빌드 전건 SUCCESS(17.2 경계 ② 「clean 빌드 관통은 미검증」 해소 판정). env 인자 없는 `pio run`은 `default_envs = poc`만 빌드.
- 재학습 당일 런북(`retrain_runbook/runbook.md`): 단계 A ~ N 요약 · 경로 · md5만 33.18(h)에 포인터.
- 등록 기능 전수 조사(`registration_survey/`): 한 줄 결론 · 핵심 난관 5 · PR 분할 · 게이트 순서만 33.19(e)에.

### 신규 미결

- 등록 판정 묶음(판정 PR, 초안 · 미머지 #83 머지 뒤): 불일치 차단 · 수집 중 억제 · `skip_reason` 어휘와 파급(대시보드 타입 · 라벨 · `_build_stats` · 8.7(a)) · 응답 바이트 재계산(보드 버퍼 768 B) · announcer · 기준값 · N · 다중 템플릿 집계 · 수집 수락 조건 · 집 녹음 train 고정 구현. 위치 = 33.19(g). 방향 · 숫자 미정.
- 노크 ↔ 등록 템플릿 한 구간 근접(33.19(f)) — 벨 녹음 세션에서 집계 방식 · 수락 조건 · 기준값과 함께 본다.
- 사용자 확인 대기: 5.1 도어벨 4유닛 구매 요건 폐기 여부 · 26.2 새 배치.

### 정정 · 현재값

- 취소선 0건. 이력 서술은 전부 무변경이고, 바뀐 사실은 새 줄의 현재값 append로만 적었다 — 33.8 라벨 경로 fallback(해소) · 33.16(i) `IndexError`(선해소) · 33.18(h) 「같은 날」 대응(코드 대응 선해소) · 33.13(d) 반올림 판단 입력(`[raw vs rounded]` 줄 계수) · 17.2 clean 빌드 축(해소) · 26.10(a) 등록 코드 0줄(현재값: 저장 · API · UI · 계측 있음, 판정 0) · 26.10(c) · 26.3(재도입) · 26.6 · 26.9(G3 부분 · G5 실재) · 26.10(e)(수집 단계 순서, 논증) · 33.5(ToF AND라 SP/DTW 단독 아님 · 1.713 유지) · 카테고리 4(서버 = exact DTW) · 8.3 폴러 +1 · 8.4(f) 한계 ④ 카드 추가 · 8.7(a) 판정 PR 때 경로 1 ~ 2 증가 예고(표 무변경) · 5.1 · 26.2(시연 소리 파급).
- 7.5(j): `_assert_commit_is_safe`는 decisions.md 0건이던 SSoT 틈이라 코드 docstring(출처 계층 ②)을 근거로 사실만 등재했다.

### 원본 대조 결과 (위임 본문 vs PR 본문 · 코드 · repo 밖 원본)

- 수치 · 해시 · 시각 = 아래 6건 외 전건 일치. 원본에 없는 수치는 옮기지 않았다.
- 관찰 1 — 노트북 통합 확인 시각 「23:36 ~ 23:44」: `notes.md`에는 기록 시각 23:43:28과 카드 「오후 11:40 등록」만 있고 23:36은 없다. 원본대로 「23:40 전후 · 기록 23:43:28」로 적었다.
- 관찰 2 — 탁상벨 개수: 위임 본문에는 개수가 없고, 런북 1절이 「다이소 탁상 호출벨 4개」로 적었다. 출처를 런북으로 밝혀 괄호로 적었다.
- 관찰 3 — 33.6(e)에는 「exit 2 건수로 판단」 서술이 0건이다(그 문구는 33.13(d)에만 있다). 33.6(e)는 무접촉, 33.13(d)에만 현재값을 붙였다.
- 관찰 4 — 런북 6절 「새로 넣은 것」의 「1절 결정 ①(평가 세션 분리)」 표기가 1절 본문(별도 시험용 세션 없음)과 어긋나 보인다. 런북은 repo 밖이라 고치지 않고 33.18(h) 포인터에 한 줄로 적었다.
- 관찰 5 — #86 본문은 10% 판정에 「사용자 확인」을 적었고, #89가 주석을 「점검선 · 정책 아님」으로 고쳤다. 두 사실을 갈라 적었다 — 사용자가 확인한 것은 측정 경로(보드 65,536 B)이고, 10%는 정책이 아니다(33.20(b)).
- 관찰 6 — PR #84 커밋 3개 트레일러 = 전부 `Claude Sonnet 5`(실측 `gh api`). 보정 위임 경위는 대화 보고로 표기(27.8(u)⑦).

### SSoT 정합 검증 (문서 전용이라 코드 자가검증 3단계 ①②는 무관 — 대상 코드 변경 0)

- V1 앵커 — 편집 스크립트가 앵커 22개 전건을 `str.count` == 1 및 줄 단위 1건으로 assert한 뒤 삽입했다(abort 0회, 라인 번호 앵커 0건). 신설 2절은 파일 끝 `---` 확인 후 덧붙였다.
- V2 삭제 0줄 증명 — `decisions.md` 추가 202줄 · 삭제 0줄(SequenceMatcher + numstat). 토큰 보존 = 원시 기준 손실 0 / 취소선 마커 정규화 기준 손실 0. 기존 줄 in-place 수정 0건.
- V3 파리티 — `decisions.md` 착수 전 취소선 포함 줄 110 / 취소선 마커 280 / 볼드 마커 15,228 → 종료 110 / 280 / 15,812(계수 단위 = 줄 수 / 마커 출현 수). 취소선 증감 0. 볼드 +584 = 292쌍, 짝수. 추가 줄 전부 줄 단위 파리티 짝수(홀수 줄 0).
- V4 한글 전수 검증 — 추가분에서 착수 전 파일에 없던 음절 6개: 갑 U+AC11 · 깝 U+AE5D · 뀔 U+B014 · 벡 U+BCA1 · 앤 U+C564 · 잭 U+C7AD(둔갑 · 가깝다 · 바뀔 · 영벡터 · 없앤다 · 트랜잭션 문맥, 전건 정상). 기존 문서에 없던 한글 단어 309개를 전수 출력해 통독했고 전건 정상 · 깨진 글자 0. 바이트 보강 = U+FFFD 0 · 모지바케 바이트 패턴 0 · 제어문자 0 · NFC 동일 · 역슬래시 u 이스케이프 0 · 낱자모 0.
- V5 인용 재조회 — 추가분의 고유 낫표 인용 50개를 마커 제거 원본(SSoT 착수 전 사본 · PR 본문 7건 · 코드 4파일 · repo 밖 md · 로그)에 재조회: 원본 적중 49 / 위임 본문 인용 1(「예산 10%」 — catch 대상 문구). 원문과 어긋난 자작 인용 4건(26.10(c) 위상 문구 · 7.5(j) 발송 실패 문구 · 33.13(d) exit 2 문구 · 33.16(j) 같은 날 문구)을 원문 그대로 고쳤다.
- V6 초안 표기 — 「#82」 · 「#83」이 나오는 추가 줄 13줄 전부에 초안 또는 미머지 표기(grep).
- V7 공개 문서 검사 — 추가분의 개인 절대경로 0(repo 밖 경로는 `~` 표기) · 실명 0 · 사설 IP 0 · SSID 0 · 토큰 · 시크릿 0 · 인계 위임 라벨 0.
- V8 git status — `docs/` 2파일만 스테이징(`git add docs/decisions.md docs/decisions-log.md`).

### 비범위

- 코드 · 설정 · 데이터 · 녹음 파일 무접촉. 노션(Set 3) · 지침(Set 2) 무접촉. repo 밖은 읽기만(쓰기 0), 초안 브랜치 체크아웃 0. 서버 기동 0.
- 다음 Set 1로 넘긴 이월 4건: 33.7(h) 취소선 범위 · 2025-09-25 해소분 포인터 6곳 · 27.8(t)① 발생 회차 · 작업 전수 조사 인벤토리의 낡은 서술.
- 등재하지 않은 것: 카테고리 20 「계측 → 실측 → 판정」 실증 횟수 갱신(판정 PR 전이라 3단계 미완주) · 6.3(m) 「INFO 미출력」 항목의 대체 관측 경로 병기(33.20(d)에서 6.3(m)을 가리키는 것으로 대신) · 5.1 구매 요건 폐기 선언(명시 결정 없음).
- 확정하지 않은 것: N · 같은 소리 기준값 · 다중 템플릿 집계 · 수집 수락 조건 · 집 녹음 train 고정 구현 · 5.1 요건 폐기 여부 · 26.2 새 배치 · 차단 행 announcer.

학습 적용: 학습 13(낫표 인용 50개 전수 재조회 — 자작 인용 4건이 원문과 어긋나 원문으로 교체, 「exit 2」 앵커는 33.6(e)에 없어 33.13(d)로만 pivot) / 학습 16(신설 절 번호 · 소절 문자 · 엔트리 형식을 실물 마지막 항목에서 읽음) / 학습 21(「소재 확인」과 「해소」를 가름 — 17.2 clean 빌드 축은 clean 뒤 빌드 로그로 해소 판정, 26.9 G3은 실측 입력만 있어 부분으로 둠).

### 관련 카테고리

카테고리 4(서버 = exact DTW) / 5.1 · 26.2 · 33.17(b)(시연 소리 파급) / 7.5(j)(신설 — 카카오 commit 가드) / 8.3(폴러 +1) / 8.4(f)(한계 ④ 카드) / 8.7(a)(판정 PR 예고) / 17.2(기준선 · clean 빌드 축) / 26.3 · 26.6 · 26.9 · 26.10(a)(c)(e)(재도입 · 현재값) / 27.8(u)(신설) / 33.5(ToF AND · 1.713 유지) / 33.8(fallback 해소) / 33.13(d)(반올림 판단 입력) / 33.16(i)(j)(선해소 · #84 신설) / 33.18(h)(선해소 · 런북 포인터) / 33.19 · 33.20(신설) / 학습 13 · 16 · 21.

### 관련 commit

`8312928`(PR #84) · `8eaf5ea`(PR #85) · `16ee0f7`(PR #86) · `1b208f6`(PR #87) · `29ef85c`(PR #88) · `0990789`(PR #89, 착수 시점 HEAD)가 본 Set의 등재 대상이다. 초안 · 미머지 PR #82 · #83의 브랜치 커밋은 무접촉. 문서 반영 = 본 커밋(docs-only, PR 없음, `main` 직 push).
