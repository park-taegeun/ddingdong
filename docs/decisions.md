...
- 🔴 **④런타임 미수행** — 보드 실기기 시험 녹음 0회. 첫 실녹음이 이 도구의 첫 실동작 확인이다.
...
**관련**: 카테고리 5 머리(직접 녹음 계획 · `SAMPLE_WEIGHT_RANGE`) / 5.1(4유닛 프로토콜 · 명명) / 5.2(a)(유입 경로 수동) · (e)(보드 재녹음 경로 → 본 절) · (g)(웹 수집분 — 다른 미결) / 6.3(j)(시리얼 절단) / 6.5(f) · 6.7(a)(세션 상한 12) / 33.3①(pitch 대상) / 33.7(i)(조각내기 도구 방향) / 33.12(a)(f)(D3 유닛 키 · 유닛 수 한계) / 27.8(r)
...
**(r) PoC-(57) 위임 · 판단 오류 6건 — 위임 자기모순 1 · 근거 출처 계층 오인 1 · 실측 미대조 권고 1 · 원인 추정 오류 1 · 위임 인용 오기 1 · MCP 부재 오판 1 (2026-09-25 PoC-(57) 추가, 근거유형 = 실측 + 실물 대조, 계수 단위 = 오류 건수)**
...
- 🔴 **처리 방침 미확정 = 사용자 판단 대기.** 해소책을 적지 않는다(스코프 제외 · §9 대상).
  ...
  - 🆕 **🟡 처리 방침 방향 확정 · 실행 대기 (사용자 결정 2026-09-23 PoC-(57), 반영일 2026-09-25, 근거유형 = 사용자 결정 + 실측 코드)**: **조각내기 도구는 본 repo에 새로 작성**한다 — pretest 코드는 **이식하지 않고 규칙만 계승**한다. **신규 유입분만** 본 repo 도구로 만들고 **과거 `01_clips`는 재생성하지 않는다**(과거분의 재발 방지는 33.12 split 단계 규칙이 맡는다). 확정된 규칙:
    ...
    - 공통 = **원본당 조각 상한**(값 미정) · **경로 인자 필수**(33.10(h) 경로 stale 재발 방지) · **멱등 skip** · **투입 manifest** 기록. **실데이터 실행은 재학습 당일**(33.12(e) 운영 규칙).
    ...
    - 🔴 **닫힘 분해**: 처리 방침 중 **방향 축 닫힘**(어디에 둘지 · 과거분 재생성 여부) / **실행 축 남음**(도구 PR 미작성 · 조각 상한 값 미정) ⇒ **미결 유지(실행 대기)**. 위 🔴 대기 문장은 **작성 시점에 사실**이라 취소선 대상이 아니다.
...
- 🔴 **사용자 판단 대기 (계약 PR 직전 — 본 절은 정하지 않는다)**: ① 알림 목록에 `other` 행을 표시할지 ② 통계 `SkipReasonCounts`에 `not_target` 키를 추가할지 ③ mock 추론에서 `other`가 나오게 할지.
- 🔴 **[권고 — 사용자 미결정] 지표 귀속 교란 (근거유형 = 논증)**: 재학습에는 `other` 신설과 **멘트 조각 제외 · 중복 제거(33.15 · 33.12)**가 함께 들어간다 ⇒ (a) E4의 OOD 발송 수가 좋아져도 **어느 쪽 덕인지 가를 수 없다**. Claude 권고 = **같은 새 split에서 3클래스 비교 학습**을 함께 돌린다. **채택 여부 = 사용자 판단 대기**이며 (a) E4의 대기 항목(합격 수치는 재학습 후)과 **같은 자리의 입력**으로 둔다 — 신규 미결로 분리하지 않는다.
...
- **FSD50K 음성은 대부분 영어**다 ⇒ **한국어 대화 네거티브가 없다**(아파트 안내방송 같은 한국 생활음도 없다 — AudioSet 보충 또는 직접 녹음 소관). 🆕 **[판정 — 2026-09-25 PoC-(57), 근거유형 = 논증, 입력 실측 = 33.15(b)]** 이 한계가 **핵심 병목이 됐다** — 33.15(d)의 멘트 제외로 `fire_alarm` 학습분에서 **한국어 말소리가 빠지면**, 한국어 말소리를 `other`로 끌어올 네거티브가 없다(현 후보는 대부분 영어). 🔴 **한국어 말소리 네거티브 출처 = 사용자 판단 대기** — 본 한계를 **미결로 승격**해 이 자리에서 추적한다(별도 절 신설 없음).
...
- [산술] 표 합계 = **1,349** = pretest 커밋 `7d93743` 메시지의 AI Hub **1,355클립** − 길이 0 wav **6개**(5.1)와 일치한다. ⚠️ ⇒ 33.7(h) 「산술 정합」의 *171 × 약 9.6조각 ≈ 1642*는 `fire_alarm` **전체**를 AI Hub로 본 산술이라 이 수와 맞지 않는다(`fire_alarm` 1642 = AI Hub 1,349 + 그 외 293 — 뺄셈). 🔴 **본 반영은 33.7(h)를 고치지 않고 관찰만 적는다** — 판정은 다음 Set.
...
- 방식 = 33.12 D1과 같다 — **`02_preprocessed` 파일은 지우지 않고 split 대상에서만 제외**, `dedup_manifest.csv`에 사유 기록(사유 라벨명 미정).
...
- 🔴 **구현 = 재학습 준비 PR 소관** — 본 반영 코드 0줄. 33.12의 D1~D3 구현 서술 · 잔존 2299는 **무변경**(2026-09-21 구현 기준).
