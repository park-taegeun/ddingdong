...
- **PR #25 (firmware, ESP32 업로드 측정 하네스, `f0e4163`)**: 합성 int16 PCM(PSRAM) + multipart POST + 지연측정(N=14, ≥6초 간격, p50/p95). 컴파일 성공(RAM 13.5%/Flash 22.2%, Stage 2 트리거 미달). **⏳ 런타임 미실측**(보드 USB 미연결 → compile-only + prereq 체크리스트 핸드오프). whitelist env(env:poc blacklist 미접촉). secrets = gitignored blind-append 플레이스홀더(실 값 = 학부생 런타임 전 교체).
