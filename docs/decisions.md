...
- **음향 트리거**: 단순 RMS 임계값 (옵션 A), PoC 1주차 자체 측정 후 80% 지점 확정
  ...
  - 🆕 **[방향 확정 · 값 미확정 — 사용자 결정 2026-09-23 PoC-(56), 상세 = 6.3(r)]** 트리거 계산 방향 = **클램프 제외 RMS**(64ms 버퍼 단위). 🔴 **임계값 · 디바운스는 여전히 미확정**이다 — 위 「단순 RMS 임계값」 문장은 교체할 확정값이 아직 없어 **무변경**이며, 값 확정은 M5-c ⓑ 판정 PR 소관이다.
- **클래스별 ToF 정책 분기 (서버)**:
  - 화재경보: ToF 우회 (무조건 발송)
  - 노크: ToF 사람 검증
...
- 🔴 **[단서 — 2026-09-18 PoC-(53), 근거유형 = 실측 + 논증] 실기기 1차 rtt가 위 산술과 어긋난다. 🔴 원인은 지정하지 않는다.**
  ...
  - 🆕 **[교대 재측정 — 실측 2026-09-23 10:55~11:01 KST / 발견일 = 반영일 = 2026-09-23 PoC-(56), 계수 단위 = 이벤트 수]** 위 **판정 방법**이 지정한 skip ↔ pending **교대 배치**를 수행했다. 로그 원본 = repo 밖 `~/ddingdong-측정결과/2026-09-23/rtt_alt/`(`note.md` · `server_ts.txt` · `monitor.log`).
    - **조건 (근거유형 = 실측)**: 랩실 · 아이폰 핫스팟 · 실모델(RSS **462,624 KB** · TF 매핑 **54**) · `env:enrich_uplink` **2026-09-18 플래시분**(재플래시 0) · 터널 · 대시보드 미기동. 실제 배치(시간순) = skip 3회 연속 → pending → skip → pending → skip → pending(계획한 1:1 교대에서 **첫 skip만 3회 연속으로 이탈**).
    - **[실측] 1차 rtt(보드 시리얼)**: skip n=5 = **345 / 377 / 374 / 201 / 466ms** · pending n=3 = **1,246 / 1,013 / 561ms**(5초 예산의 최대 **25%**). 시간순 **단조 증가 없음**.
    - **[실측] 서버 `primary_sent_at − detected_at`**(디코드 + 추론 + 카카오 1차, pending 3건) = **665 / 296 / 306ms**.
    - **[논증] 보드 rtt − 서버 구간 = 581 / 717 / 255ms**를 망 · 업로드 몫으로 읽는다 — 근거 = `detected_at`이 **핸들러 시작 시각**이라 그 앞 구간이 보드 쪽 몫이 된다.
    - **[실측] 카카오톡 도착 6 / 6**(1차 텍스트 3 + 2차 사진 3, 학부생 화면).
    - **판정 (근거유형 = 실측)**: 「게이트 구조(카카오 1차 왕복)만으로 4초+가 불가피」 가설은 **반증**됐다 — 같은 구조에서 pending 최대 **1,246ms**다. 🔴 **2026-09-18 pending 4,301 / 4,902 / 5,814ms의 원인은 여전히 지정하지 않는다** — 환경 차이(장소 · 핫스팟 상태 · 날짜)가 후보이나 **확정 아님**.
    - **한계**: pending **n=3** · **1세션** · 2026-09-18 조건 재현 아님 · 위 교란 ③(서버 작업량 차이)은 교대 배치로도 **갈리지 않는다**(위 판정 방법의 ⚠️ 그대로).
...
- ★ peak 마진이 rms의 약 2배이나, 본 값은 **3.2초 윈도우 집계**이고 실제 트리거는 64ms 버퍼 단위이므로 그대로 적용 불가 — **카테고리 3 SSoT("단순 RMS 임계값") 변경 판단은 M5-c로 보류**(근거유형 = 논증, 본 Set에서 SSoT 미변경).
  - 🆕 **[후속 — 2026-09-23 PoC-(56)]** 64ms 버퍼 단위 실측과 M5-c 방향 확정(클램프 제외 RMS) = **6.3(r)**. 🔴 **카테고리 3 SSoT의 값(임계)은 여전히 미확정**이다.
...
- 🆕 ✅ **[방향 확정 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** **① 펌웨어 쪽을 먼저** 한다 / **② 배선 조치는 보류**(재결선 금지 · INMP441 예비 0개 · 기전 미분리) / **③ 동시 병행은 비권고**. 🔴 **값 · 방식 세부(필터 판별 기준 · 임계)는 미확정**이다 — 오디오 페이로드 글리치 필터는 **보류**이고, 트리거 경로는 **클램프 제외 RMS**로 우회한다(**6.3(r)**). 판단 입력 = **6.3(q)**(글리치 vs 진짜 클립 모양 판별 실측). ⇒ 닫힘 분해 = 방향 축 **닫힘** / 방식 · 값 축 **무접촉** = **부분만 닫힘**. ⚠️ 아래 가설 검증 bullet과 (p)의 「해결책 ①②③은 여전히 사용자 판단 대기」는 **각 시점(2026-09-12 · 09-15) 이력**이라 취소선 대상이 아니다.
...
- 🟡 **[등재 — 2026-09-15 PoC-(48) 실측] 호스트 테스트는 ~~3종~~ → 4종이다**: `noise_stats_test` · `tof_judge_test` · **`firmware/tools/jsonpeek_test.cpp`** · 🆕 **`noise_modes_test`**. 앞의 둘은 SSoT에 등재돼 있으나 **`jsonpeek_test`는 decisions.md 0건**이었다(2026-09-15 실측, 대조군 = 같은 grep에서 `noise_stats_test` · `tof_judge_test` 생존). ~~⇒ **존재 사실까지만** 등재하며 checks 수·커버리지·최종 실행 결과는 **미측정**이다. **확정하지 말 것.**~~
  ...
  - 🆕 **[현재값 갱신 2026-09-23 PoC-(56), 근거유형 = 실측 + 문서 인용, 계수 단위 = checks 수]** **7종**이다 — **`firmware/tools/trig_line_test.cpp`가 PR #73(`b379c4a`)으로 합류**했다(상세 = **(r)**). checks 수 = `trig_line_test` **74** / `enrich_wire_test` **90** / `camera_probe_test` **97** / `noise_modes_test` **46** / `noise_stats_test` **61** / `tof_judge_test` **196**이고, `jsonpeek_test`는 정적 `assert` **13**(단위가 달라 더하지 않는다). 7종 전건 실행 OK(PR #73 본문). `trig_line_test`는 보드가 쓰는 `trig_line.h` · `noise_stats.h`를 **그대로 include**하고 파일 머리 주석에 컴파일 명령이 있다. ⚠️ 위 「6종」은 2026-09-18 PoC-(53) 시점 이력이다.
...
**(q) 글리치 vs 진짜 클립 판별 실측 — 연속 폭(모양)으로 가를 수 있다 (2026-09-23 PoC-(56) 신설, 실측 13:27~13:35 KST, 발견일 = 반영일 = 2026-09-23, 근거유형 = 항목별 병기 — 실측 / 논증, 계수 단위 = 스냅샷 수 / 클램프 샘플 수 / 샘플 폭)**

> 랩실. `env:mic_noiseprobe`(**코드 무변경**, 13:21 플래시) · 결선 무변경 · 서버 · WiFi 불요. 로그 원본 = repo 밖 `~/ddingdong-측정결과/2026-09-23/mic_clipshape/`(`note.md` 끝의 「정정」 블록 포함 · `summary.txt` · `monitor.log`, `.gitignore` 차단분 — SSoT엔 요약만). 스냅샷 **22개**.

- **조건 (근거유형 = 실측)**: 조건 A = m0 + 노크(보드 옆 책상) / 조건 B = m2 + 조용 / 조건 C = m2 + 노크 / 조건 D = m0 + 마이크 약 10cm 근접 박수. 배치 = 1바퀴 A B C, 2 · 3바퀴 A D B C(D는 1바퀴 결과를 보고 추가 — 자극 세기 1변수).

  | 조건 | n | clip(스냅샷별) | 방향 +/− | `len_max` | peak | `rms_x`(클램프 제외) | 기타 |
  |---|---|---|---|---|---|---|---|
  | A (m0 + 노크) | 6 | 0 전건 | 0 / 0 | — | 10,208~20,485 | 304~570 | `tz` 6 |
  | B (m2 + 조용) | 6 | 6~12 | +59 / −1 | 1 전건 | 32,767~32,768 | 85~97 | rms(원래) **451~634** · `low6nz` 1~2 · `tz` 5 |
  | C (m2 + 노크) | 6 | 4~13 | +43 / −1 | 1 전건 | 32,767~32,768 | 349~564 | rms(원래) **564~863** · `low6nz` 0~2 · `tz` 5 · 6 |
  | D (m0 + 근접 박수) | 4 | 81~265 | +265 / −487 | 68~294 | 32,768 | 1,797~3,562 | `low6nz` 0 · `tz` 6 |

- **[실측] 판정 1 — 모양으로 가를 수 있다**: 글리치(조건 B · C) 클램프 **104개**(+102 / −2)는 **전부 고립 단일 샘플**(`len_max` = 1 전건)이고, 진짜 클립(조건 D)은 **68~294샘플 연속 · 양방향**이다.
- **[실측] 판정 2 — 보통 세기 노크는 ToF OFF에서 클립 0**(6 / 6)이다. 조건 C의 클립 수는 조건 B 범위 안이다 ⇒ 노크가 클립을 **추가하지 않았다**.
- **[논증] 판정 3 — 원래 RMS 트리거는 무음에서 먼저 발동한다**: 조용 + 글리치의 rms **451~634** ≥ ToF OFF 노크의 rms **304~570**(입력 = 위 실측 — 조건 A는 클램프 0이라 rms와 `rms_x`가 같다). ⇒ 카테고리 3의 「단순 RMS 임계값」을 현 펌웨어에 그대로 넣으면 오발동한다. 64ms 버퍼 단위 실측 = **(r)**.
- **[논증] 오전 `enrich_uplink` 노크 이벤트의 peak 32,767도 글리치일 가능성이 높다** — 펌웨어가 다르고 폭은 미측정이다.
- **정정 기록 (근거유형 = 실측)**: 해석 노트 1차 판정의 「클램프 102개」는 **+ 방향 개수를 합계로 적은 오기**였고(실물 **104**), 「rms 451~634」는 조건 B 6개 중 4개만 본 범위였다(전수 재계산 결과 **범위 동일**) — 원본 `monitor.log` 기계 재계산으로 정정했다(**27.8(q)①②**).
- **한계 (전건 명기)**: 조건당 **n 4~6** · 노크 세기 = **1인** 손(같은 자리 · 같은 세기 의도) · 박수는 **진짜 클립 대용**(실제 초인종 · 화재경보음 아님) · 🔴 **「최대치에 겨우 닿는 소리」의 단일 샘플 클립 여부 미관측**(조건 D 최소 폭 68) ⇒ 필터를 만든다면 **이웃 진폭 조건이 필요**하다(설계 입력이지 확정 아님) · 글리치 이웃 샘플 진폭은 하네스 **미출력** · 조건 D의 rms(원래)는 미기재. ToF 시야는 1바퀴 조건 B 구간 near 8~9 → 이후 5~7(비교 변수와 별개, 기록만).
- 🔴 **판정 상수 · 필터 설계 확정 0건** — 해결책 방향은 **(n)** 말미(사용자 결정), 필터 세부는 미확정이다.

**(r) M5-c 방향 확정(3단계) + ⓐ 계측 PR #73 — `env:mic_trigprobe` + ④런타임 (2026-09-23 PoC-(56) 신설, PR #73 `b379c4a`, 발견일 = 반영일 = 2026-09-23, 근거유형 = 항목별 병기 — 사용자 결정 / 실측 / 논증, 계수 단위 = 64ms 버퍼 수 / 사건 수 / checks 수 — 항목별 병기)**

- **[사용자 결정 2026-09-23] M5-c(음향 자동 트리거) = 10월 착수, 3단계**:
  - **ⓐ 계측 PR** — 64ms 버퍼 단위 raw RMS + 클램프 제외 RMS 로그만, **판정 0줄**(= 아래 PR #73).
  - **ⓑ 판정 PR** — 임계 · 디바운스 · **2차 세션 상한(6.7(a)⑧, 현재 ≤12) 재검토**. 무음 · 노크 · 초인종 · 인터폰 실측 **후**.
  - **ⓒ 부스 자동 트리거** — **`other` 재학습 모델 이후**(33.13). 그 전에는 **수동 트리거가 기본값**이다.
  - **트리거 계산 방향 = 클램프 제외 RMS**(글리치 우회, (q)). 오디오 페이로드 글리치 필터 = **보류**((n) 말미). 🔴 **값(임계 · 착수 날짜)은 미정**이다.
- **[실측] ⓐ 산출 = PR #73 `b379c4a`** (`git show --stat` = **5파일 647 insertions / 0 deletions**): 신설 4 = `firmware/include/trig_line.h`(순수 헤더) · `firmware/src/mic_trigprobe_main.cpp` · `firmware/tools/trig_line_test.cpp` · `firmware/MIC_TRIGPROBE_RUNBOOK.md` + `platformio.ini` 파일 끝 env 블록 추가(+37 / −0, 현재값 = 카테고리 16).
  - 64ms 버퍼(1,024샘플 / 16 kHz)마다 순번 · raw RMS · 클램프 제외 RMS · 클램프 수를 낸다. 계산은 `noise_stats.h`의 `noiseAnalyzeI16` **재사용**(새 계산식 0, (o) 자산 무수정).
  - 마이크 태스크 → FreeRTOS 큐(32) → loop 출력이며 **마이크 태스크의 시리얼 출력은 0줄**이다. 큐가 차면 **드롭을 세어** 5초 health 줄로 낸다.
  - 한 줄 = 버퍼 3개, 최악 **66B**(≤80B, (j)) — `static_assert` + 호스트 테스트의 정확 일치 단언.
  - 호스트 테스트 `trig_line_test` **74 checks** · NC 3종 전건 검출(현재값 = (o)). 빌드 `mic_trigprobe` RAM **28,432** / Flash **391,053** B, `mic_noiseprobe` · `enrich_uplink` 수치는 전후 동일.
  - 성격 = **방법론 자산**((o) `mic_noiseprobe` 선례 — 제품 코드 · 판정 상수 0줄).
- **[실측] ④런타임 (2026-09-23 14:32~14:37 KST, 랩실, 머지 전 브랜치 커밋 `0209506`, 계수 단위 = 64ms 버퍼 수)** — 로그 원본 = repo 밖 `~/ddingdong-측정결과/2026-09-23/mic_trigprobe/`(`note.md` · `analysis.txt` · `monitor.log`).
  - 하네스 무결성: 순번 **1679~9898** · **8,220버퍼 · 누락 0** / health `drop` **11 → 11**(11건은 모니터 부착 전 발생분 — 부착 중 증가 0) · `gaps` **0** · `fmt_err` **0**.

  | 구간 | n | 클립 버퍼 비율 | 조용 최대 초과 버퍼(클램프 제외 > 154 · raw > 1,455) |
  |---|---|---|---|
  | 조용 1 | 203 | 7.9% | 0 · 0 (클램프 제외 최대 **154**) |
  | 노크 10회 | 338 | 5.9% | **24** · **2** |
  | 근접 박수 5회 | 300 | 14.3% | 63 · 25 |
  | 말소리(약 50cm) | 270 | 5.6% | 143 · 8 |
  | 조용 2 | 118 | 10.2% | 0 · 0 (raw 최대 **1,455** · 클램프 제외 최대 150) |

  - 사건(클램프 제외 RMS > 154인 버퍼 묶음, 간격 3버퍼, 계수 단위 = 사건 수): 노크 **9개**(10회 수행) · 최약 **217 = 조용 최대의 1.4배** / 박수 **9개**(5회 수행) · 최약 172 / 말소리 **7개** · 최약 160.
- **판정 (근거유형 = 실측)**:
  - **원래 RMS 트리거는 불성립** — 조용 구간 글리치로 raw 최대 **1,455** ⇒ 오발동을 피하는 임계에선 노크 버퍼 **2개**만 넘는다.
  - **클램프 제외 RMS 방향 지지**(64ms 단위) — 조용 ≤ 154이고 노크 · 박수 · 말소리는 넘는다.
  - **말소리도 넘는다**(143버퍼) ⇒ 분류가 걸러야 한다 — **ⓒ 조건(`other` 재학습 이후)을 실측이 뒷받침**한다.
  - 🔴 **노크 여유 1.4배로 얇다** + 노크 10회 중 사건 9개(1회 미달인지 병합인지 구분 불가) ⇒ **임계 확정 금지** — 부스 수준 배경 소음 + 문 모형 노크 실측 후 **ⓑ**에서 정한다.
  - **정정**: 앞선 「글리치가 64ms 버퍼의 20~40%를 오염」 산술은 **과대**였다 — 실측 **7.9~10.2%**(조용 구간, **27.8(q)⑤**).
- **[논증] 관측**: 플래시 후 모니터 부착 전에는 출력이 막혀 큐가 차고 드롭이 난다 ⇒ 측정 전 `drop` 기준값을 기록할 것(런북 보강 후보, 미반영).
- **한계 (전건 명기)**: **1세션 · 1인** · 랩실 조용 구간 **약 20초** · 노크 = **책상**(문 모형 아님) · **초인종 없음**(실물 미도착) · 인터폰 없음 · **WiFi off**(제품과 CPU 경합 조건이 다르다) · 박수 사건 9개(5회)의 초과분 원인 미확인 · 태스크 간 줄 섞임 없음은 HWCDC `write` 뮤텍스 기반 **논증**(미실측).
...
- 🆕 **[현재값 갱신 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** **타임아웃 값이 확정**됐다(`UPLINK_ENRICH_HTTP_TIMEOUT_MS = 15000`, PR #72 `d1a2384`, 상세 = **7.7(l)** 말미). ⇒ 현재 경계 = **전부 닫힘 6항**(해상도 · skip 발송 · `presence=false` 촬영 · additive · 세션 상한 · **타임아웃**) / **부분만 닫힘 1항**(녹음 길이 — **pre:post 비율 축만 남음**) / **무접촉 1항**(카메라 init 시점) / **다른 자리에서 부분 해소 1항**(6.3(n) 해결책 — 같은 날 **방향만** 정해졌다, 상세 = **6.3(n)** 말미). ⚠️ 위 bullet의 부분 닫힘 계수(2항)와 첫 bullet의 「값은 잠정 15,000ms — 미결 유지」는 **2026-09-18 시점 이력**이라 취소선 대상이 아니다 — **현재값은 본 bullet**이다. 세션 상한은 **닫힘 유지**이며 재검토 예고 = **6.7(a)**.
...
- 🆕 **[재검토 예고 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** ⑧ 세션 상한(2차 ≤12)은 **M5-c ⓑ 판정 PR에서 재검토 예정**이다 — 6.7(a) 확정분의 **재개 예고**(상세 = **6.3(r)**). 🔴 **값은 무변경**이며 재검토 전까지 **≤12가 현재값**이다.
...
- 🔴 **6.5(f) 미결과의 대응 (근거유형 = 실측 대조)**: (f) 목록 9항 중 **전부 닫힌 것은 5항**(해상도 · skip 발송 · `presence=false` 촬영 · additive · 세션 상한), **부분만 닫힌 것은 2항**(녹음 길이 = N만 확정·비율 미결 / 타임아웃 = 방식만 확정·값 잠정), **손대지 않은 것은 2항**(카메라 init 시점 · 6.3(n) 해결책 ①②③)이다. ①은 (f) 목록에 **없던 항목**이라 신규 확정이다.
  - 🆕 **[현재값 갱신 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** 타임아웃 항목의 **값 축이 닫혔다**(`UPLINK_ENRICH_HTTP_TIMEOUT_MS = 15000` 확정, PR #72 `d1a2384`, 상세 = **7.7(l)** 말미) ⇒ 현재 **전부 닫힌 것은 6항**(타임아웃 합류), **부분만 닫힌 것은 1항**(녹음 길이 = **pre:post 비율 축만 남음**)이다. ⚠️ 위 대응 bullet의 부분 닫힘 계수(2항)와 (c)의 「🔴 잠정값」 서술은 **2026-09-18 시점 이력**이라 취소선 대상이 아니다 — 현재값은 본 줄이다.
...
- 🔴 **[재판단 입력 — 2026-09-18 PoC-(53), 근거유형 = 실측] 트리거가 발생했다. 🔴 그러나 정책은 확정하지 않는다 — 미결 유지다.**
  ...
  - 🆕 **[선행 조건 충족 — 발견일 = 반영일 = 2026-09-23 PoC-(56), 근거유형 = 문서 인용]** 위 항목이 본 미결의 선행 조건으로 지목한 **보드 타임아웃 값이 결정됐다** — `UPLINK_ENRICH_HTTP_TIMEOUT_MS = 15000` 확정(사용자 결정 2026-09-23, PR #72 `d1a2384`, 상세 = **7.7(l)** 말미). 🔴 **본 (i)의 서버측 자막 실패 정책은 미결 유지**다 — 선행 조건이 채워진 것과 정책이 정해진 것은 **다른 축**이며, 보드 2차 재시도를 추가할 때는 7.7(l) 재판정 트리거와 **함께** 판단한다.
...
- 🆕 **[서버 기준 2차 소요 — 실측 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56), 계수 단위 = 이벤트 수]** `detected_at → secondary_sent_at` = **9,269 / 8,280 / 13,049ms**(15초 예산의 **62~87%**, n=3, 6.2 교대 재측정 세션 · 보드 관통). `enrich_status` **completed 3** · `stt` **null 3**(말소리 없는 노크 입력 = 자막 부재 경로, 7.6(e)).
  - **(근거유형 = 논증)** 두 시각이 모두 서버 시계이고 시작점이 `/detect` 핸들러이므로, 이 값에는 보드의 **캡처 · 5초 녹음 · `/enrich` 업로드 구간이 들어 있다**. ⚠️ 위 **1.755초(11.7%)**는 2026-09-08 **curl 입력의 서버측 구간**이다 — 두 값을 같은 조건으로 비교하지 말 것.
...
**(l) 🔴 [신규 미결] 2차 최악 총합 12.1초 > 펌웨어 `HTTP_TIMEOUT_MS=10000` (발견 2026-09-08, 문서 반영 PoC-(45), 근거유형 = PR 본문 서술 · 미실측)** → ✅ **값 축 해소 2026-09-23 PoC-(56) — 본 소절 말미**
...
- ✅ **[값 축 해소 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** `UPLINK_ENRICH_HTTP_TIMEOUT_MS = 15000`을 **값 그대로 확정**했다(PR #72 `d1a2384`, **값 변경 0** — 주석 · 런북의 「잠정」 표기만 확정으로 교체).
  - **근거 ① (근거유형 = 실측, 계수 단위 = 이벤트 수)**: 정상 종결 2차 rtt(보드) n=5 = **6,710 / 10,014ms**(2026-09-18, 6.7(d)) + **4,284 / 3,131 / 7,790ms**(2026-09-23, 6.2 교대 재측정 세션) ⇒ 범위 **3,131~10,014ms**.
  - **근거 ② (근거유형 = 논증)**: 서버는 카카오 · STT를 **응답 전에 핸들러 안에서 끝내고** 보드는 2차를 **재시도하지 않는다** ⇒ 이 값은 **카카오 발송 여부를 바꾸지 않는다**. 바뀌는 것은 「보드가 결과를 아는가」와 「대기 중 다음 트리거를 받지 못하는 시간」뿐이다.
  - **근거 ② 보조 (근거유형 = 실측, n=1)**: 2026-09-18 #4 = 보드 `-11` ↔ 서버 종결 1건(6.7(d)(e)). 🔴 **「보드 연결이 끊긴 뒤에도 서버가 처리를 계속한다」는 미실측**이다 — 이 건은 시각 대조상 **서버가 먼저 끝났고 응답이 보드에 닿지 못했을 가능성**이 크다(PR #72 머지 전 후속 커밋 `1e7bb28`이 주석 문구를 그 방향으로 정정 — **27.8(q)③**).
  - **근거 ③ (근거유형 = 실측)**: 15초를 넘긴 실측은 **네트워크 도달 실패(`URLError`) 모집단뿐**이다.
  - 🔴 **수용된 위험 (근거유형 = 논증)**: 위 2026-09-18 재판정 입력의 산술로는 네트워크 실패 모집단에서 서버 최악 **≈19.5~20.0초 > 15초**다 — 그때 보드는 결과를 모른 채 끊는다. 사용자가 이 위험을 **알고 수용**했다.
  - **재판정 트리거 3개 (근거유형 = 사용자 결정)**: (1) M5-c 판정 PR에서 **2차 대기 중 트리거 누락**이 관측될 때 (2) 보드 2차 **재시도를 추가**할 때(재전송은 409에 막힌다 — **7.6(i)와 함께** 판단) (3) **EC2(Nginx → Gunicorn) 이관 시** 근거 ②를 1회 재확인하고 다르면 재판정.
  - **닫힘 분해 (근거유형 = 문서 인용)**: **본 (l) 미결 = 전부 닫힘** — 방식 축은 2026-09-18 **6.7(a)④**(C1 전용 상수)로 이미 닫혔고 본 결정으로 **값 축이 닫혔다**(수용된 위험 + 트리거 3개로 재개 가능). 🔴 **무접촉** = **7.7(g) 2차 15초 예산**(본 결정은 예산을 바꾸지 않는다) · **7.6(i) 서버측 자막 실패 정책**(미결 유지). ⚠️ 위 2026-09-17 · 09-18 · 09-20의 「미결 유지」 서술은 **각 시점 이력**이라 취소선 대상이 아니다.
...
- ⚠️ **[실측 — 학부생 브라우저, 계수 단위 = 측정 지점 수] 브라우저 실측은 C2 1곳뿐이다**: DevTools Computed `font-size` = **18px**(도움말 화재 카드 제목). 🔴 **C1 · C3 · C4 · C6은 미측정**이다 — **「4곳 전부 브라우저에서 확인됐다」로 읽지 말 것.**
  - 🆕 **[현재값 갱신 — 발견일 = 반영일 = 2026-09-23 PoC-(56), 근거유형 = 실측(사용자 보고, 수치 미기록)]** 사용자가 브라우저 DevTools Computed로 **C1 · C3 · C4 · C6 4곳 전부 정상**을 확인했다고 보고했다(사용자 발언 2026-09-23). 기대값 = C1 **15px**(`text-caption`) · C3 **20px**(`text-h2`) · C4 **17px**(`text-body`) · C6 **15px**(`text-caption`). ⇒ 글자 크기 지점 **5곳(C1 · C2 · C3 · C4 · C6) 전부 브라우저 확인**(C5는 색 변종이라 대상 아님). 🔴 **개별 px 값 · 측정 일시는 미기록**이다 — 원본 = repo 밖 `~/ddingdong-측정결과/2026-09-23/twmerge_browser_note.md`(발언 기록 · 1인 1회). ⚠️ 위 「C1 · C3 · C4 · C6은 미측정」은 측정 일시가 미기록이라 **작성 시점에 거짓이었는지 판정할 수 없어** 취소선을 걸지 않는다 — **현재값은 본 줄**이다.
- 🟡 **[신규 미결] 커스텀 토큰 목록이 두 곳에 있고 자동 가드가 없다 (발견일 = 반영일 = 2026-09-21, 근거유형 = 실측, 계수 단위 = 토큰 수)**: `dashboard/src/index.css`의 `--text-*` **6개**와 `dashboard/src/lib/utils.ts`의 `TEXT_SIZE_TOKENS` **6개**가 **따로 존재**하고, 어긋나도 경고하는 **자동 가드가 없다**(주석에 명시했을 뿐). 토큰을 추가·삭제하면 **두 곳을 같이** 고쳐야 한다 ⇒ **드리프트 위험**(27.8(m)⑤ 계열). 🔴 **처리 방침 미확정 — 해소책을 적지 않는다.**
  - ✅ **[처리 방침 확정 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** **자동 가드는 만들지 않는다**(수용된 위험 — 토큰 6단은 확정 스케일). 대신 실제 편집 지점인 `dashboard/src/index.css`의 `--text-*` 블록 주석에 `lib/utils.ts` `TEXT_SIZE_TOKENS` **동기화 역방향 경고**를 넣었다(PR #71 `510fd01`). **재판정 트리거 = 토큰 추가 · 삭제**. ⇒ 본 신규 미결 = **전부 닫힘**(수용된 위험으로). ⚠️ 위 「미확정 — 해소책을 적지 않는다」는 2026-09-21 시점 사실이라 취소선 대상이 아니다.
...
- 🆕 **[현재값 갱신 — 프로토콜 ② 부분 수행, 실측 2026-09-23 13:05~13:17 KST / 발견일 = 반영일 = 2026-09-23 PoC-(56), 계수 단위 = 초 / 표본 수]** 로그 원본 = repo 밖 `~/ddingdong-측정결과/2026-09-23/tof_static/`(`note.md` · `transitions.txt` · `ndet_dist.txt` · `monitor.log`). 조건 = `env:enrich_uplink`(2026-09-18 플래시분) · **판정 상수 무변경**(`TOF_MOTION_NDET_MIN = 1` · `TOF_MOTION_LATCH_FRAMES = 75`) · 서버 미기동.
  - **[실측] 배치**: 같은 자리(센서 정면 약 44~49cm, center 실측 432~492mm)에서 기준선 → 책 → 사람 → 책 → 사람 → 책(책 = 손으로 놓고 약 60초 정지 / 사람 = 앉아서 정지).
  - **[실측] 책 3회**: 첫 fused NONE까지 **28.6 / 16.9 / 48.0초** · 체류 중 NONE 비율 **2 / 4 / 15%**.
  - **[실측] 사람 2회**: 첫 NONE까지 **52.5초(정착 후 약 34초) / 14.2초** · NONE 비율 **13 / 42%**(NONE 구간 7~11초).
  - **[실측] 재진입(NONE → PERSON)은 전건 `ndet=1`**이 latch를 75/75로 재충전하며 일어났다.
  - **[실측] ndet 분포**(StageB-1 15프레임마다 1회 표본, 구간 시작 3초 제외): 책(3회 합산) n=127 중 `ndet ≥ 2` **5(3.9%)** / 사람(2회 합산) n=68 중 **7(10.3%)** — 단측 Fisher **p = 0.077**(차이 약함).
  - **판정 (근거유형 = 실측)**: ① 「정지 5초 후 presence=false」(6.4(b) 설계 서술 · 26.10(e))는 **실측과 불일치**한다 — 정지 책이 체류의 **85~98%**를 PERSON으로 유지했다(2026-09-11 n=1 방향 재현, n=3). ② 반대 방향으로 **정지한 사람도 NONE 구간이 생긴다** — 그 구간의 초인종 · 노크는 `tof_rejected`다. ③ **`NDET_MIN` 상향으로 정지 사물과 정지 사람을 가를 근거가 없다**(위 분포 — 책도 `ndet ≥ 2`를 3.9% 낸다).
  - **[논증] 기전** = `NDET_MIN = 1` 단발 재충전(입력 = 재진입 전건 `ndet=1` 실측).
  - **한계**: 책 **n=3** · 사람 **n=2** · 거리 **1개**(감시창 하한 400mm 근처) · **앉은 자세**(현관 선 자세 아님) · 책 크기 · 재질 미기록 · fused 전환 줄 **2건 로그 누락**(책 제거 시점 — Stage A 전환으로 대체 판정, 원인 미규명).
  - ⇒ 본 (e)의 현재값 = **「프로토콜 ② 부분 수행(2026-09-23)」**이다. ⚠️ 위 「미수행」 서술과 9.1 · 33.11의 같은 「미수행」 인용은 **각 시점 이력**이라 무접촉이다. 상수 대응 = **사용자 결정 2026-09-23 무변경**(26.10(e) 말미).
...
- **⚠️ env:poc `build_src_filter` blacklist 회귀 (2026-06-22 PoC-(17) catch)**: `[env:poc]`는 blacklist 방식(`+<*>` 후 camera만 제외)이라, **5/10 mic_test / 5/11 tof_test 추가 후 각자의 `setup()`/`loop()`가 poc 빌드에 흡수 → multiple definition 링크 충돌**. 5/8 이후 poc 미재빌드로 잠복하다 6/22 부팅 검증에서 발현. 1차 검증은 `PLATFORMIO_BUILD_SRC_FILTER` 환경변수 override로 mic/tof 제외 후 우회 빌드(파일 0 수정). ~~**근본 수정 = whitelist 통일 별도 위임 (카테고리 27.6 / DB3 등록 예정)**~~ → **✅ 정정 (해소 PR #4 `c4c8f47` / 발견 2026-09-02 세션 / 문서 반영 2026-09-02)**: 근본 수정은 **이미 완료돼 있었다**. 2026-09-02 `firmware/platformio.ini` 실물 조회 결과 **실존 env 9개 전부 whitelist**(`-<*>` 선행) — `poc` / `camera_v1` / `camera_v2` / `mic_dummy` / `tof_dummy` / `upload_spike` / `upload_spike_tls` / `tof_pinscan` / `tof_lineprobe`. "별도 위임 예정" 서술은 3개월간 잔존한 **stale**(27.6은 6/22 시점에 이미 ✅ 완료 기록). ★ **학습 21 3회차**(등재된 미결도 실물 검증 대상 — 8/12 매크로 유령 → 9/02 poc blacklist). **[단서, PoC-(45) 문서 반영] 위 "9개"는 2026-09-02 시점 이력값** — ~~현재값(2026-09-11)은 `mic_uplink`(PR #52) + `mic_noiseprobe`(PR #54) 추가로 **11개**(`grep -c '^\[env:' firmware/platformio.ini` 실측).~~ → ~~**[현재값 갱신 2026-09-18 PoC-(52), 근거유형 = 실측 + 문서 인용]** `camera_probe`(PR #63 `41cd6e2`) 추가로 **12개**이며, 신설 블록은 **추가만**이고 `platformio.ini` **삭제 줄 0**·기존 11개 env 무수정이다(상세 = **6.6**).~~ → **[현재값 갱신 2026-09-18 PoC-(53), 근거유형 = 실측 + 문서 인용]** `enrich_uplink`(PR #64 `d093d70`) 추가로 **13개**(`grep -c '^\[env:' firmware/platformio.ini` 실측)이며, 신설 블록은 **추가만**이고 `platformio.ini` **삭제 줄 0**·기존 12개 env 무수정이다(상세 = **6.7**). 위 2026-09-11 「11개」·2026-09-18 PoC-(52) 「12개」는 **각각 그 시점 이력**이다. 이력 서술("9개 전부 whitelist") 자체는 그 시점 사실이라 무변경.
  - 🆕 **[현재값 갱신 2026-09-23 PoC-(56), 근거유형 = 실측 + 문서 인용]** `mic_trigprobe`(PR #73 `b379c4a`) 추가로 **14개**(`grep -c '^\[env:' firmware/platformio.ini` 실측)이며, 신설 블록은 파일 끝 **추가만**이고 `platformio.ini` **삭제 줄 0** · 기존 env 13블록 무수정이다(상세 = **6.3(r)**). ⚠️ 위 2026-09-18 PoC-(53) 값(13)은 **그 시점 이력**이다(이력에는 취소선을 걸지 않는다).
...
- **파급**: ~~**`ssr-nc`는 워킹트리 clean을 요구하므로(8.4(f) 환경 사실 ①) 다음 실행에서 걸릴 수 있다.**~~ → 🔴 **[정정 — 발견일 = 반영일 = 2026-09-23 PoC-(56), 근거유형 = 실측 코드 대조] 코드와 불일치한다.** `dashboard/tools/ssr-nc/run.mjs`의 `assertCleanSrc`는 `git status --porcelain -- src`를 `cwd: ROOT`(= `dashboard/`)에서 실행한다 ⇒ 검사 범위는 **`dashboard/src`뿐**이고 repo 최상위 `.claude/`는 **원래 검사 범위 밖**이다. ⚠️ 8.4(f) 환경 사실 ①의 「워킹트리 diff」도 실제 범위(`dashboard/src`)보다 **넓게 적혀 있다** — 본 Set 범위 밖이라 **무접촉**, 기록만 한다.
...
- ✅ **[처리 방침 부분 확정 — 사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** **`.gitignore`에 `.claude/`를 추가**했다(PR #71 `510fd01`). 사유 = **public repo 노출 방지** — 격리 워크트리 · 세션 · 권한 설정 같은 로컬 부산물이 `git add` 실수로 공개되는 것을 막는다(ssr-nc와는 무관 — 위 정정). 🔴 **워크트리 lock 문제는 미해소**다 — git 내부 상태라 ignore 규칙으로 풀리지 않으며(PR #71 본문 「해소하지 않는 것」), 위임 계층에서는 **격리 워크트리 금지를 유지**한다. ⇒ 닫힘 분해 = `.gitignore` 축 **닫힘** / 워크트리 정리 시점 축 **무접촉** = **부분만 닫힘**. ⚠️ 위 방침 미확정 서술은 2026-09-19 시점 사실이라 취소선 대상이 아니다.
...
- 🆕 **[배치 결정 — 사용자 결정 2026-09-23 PoC-(56)]** 초인종 **송신기 = 문 모형(왼쪽) 쪽**, **수신기(차임) = 책상(가운데)**(시연 각본 대응 ③, 상세 = **26.10(e)** 말미).
...
- ✅ **[사용자 결정 2026-09-23, 발견일 = 반영일 = 2026-09-23 PoC-(56)]** **판정 상수 무변경**(`TOF_MOTION_NDET_MIN` · `TOF_MOTION_LATCH_FRAMES`) — 분리 근거 = 9.4(e) 말미의 ndet 분포. **시연 각본 대응 3건**: ① 우리집 초인종 · 노크 시연 = **접근 후 5초 안에 누름** 유지(이번 데이터의 접근 후 첫 NONE 최소 **14.2초**, n=2 — 여유 확정 아님) ② 옆집 억제 시연은 **직전에 시야 점검**(near 0~4) **필수** ③ 초인종 **송신기 = 문 모형 쪽, 수신기(차임) = 책상**(26.2 포인터). ⇒ 본 절의 각본 · 상수 재검토 대기 항목 = **전부 닫힘**(상수 무변경 + 각본 3건). ⚠️ 위 「실제 부스에서 어느 쪽이 나올지는 미확정」은 2026-09-12 시점 서술이며, 방향은 9.4(e) 말미 실측이 채웠다(n 작음).
...
**(q) PoC-(56) 위임 · 판단 오류 8건 — 배경 수치 오기 1 · 부분 표본 범위 1 · 근거 오인용 1 · 순환 논증 1 · 외삽 과대 1 · PR 성격 오판 1 · SSoT-코드 불일치 1 · 약칭-구조 불일치 1 (2026-09-23 PoC-(56) 추가, 근거유형 = 실측 + 실물 대조, 계수 단위 = 오류 건수)**

- **① 배경 수치 오기 — 「클램프 102개」는 + 방향 개수였다 (출처 계층 = ③ 위임 본문 · 해석 노트, catch = MCP)**: Claude가 해석 노트와 PR #73 위임 배경에 글리치 클램프를 **102개**로 적었으나 실물은 **104개(+102 / −2)**다. MCP가 PR #73 Step 0에서 `summary.txt` 원본 대조로 잡았다. 설계 영향 없음. 상세 = **6.3(q)**.
- **② 부분 표본의 범위를 전체 범위로 적었다 — 「rms 451~634」 (출처 계층 = ③ 위임 본문 · 해석 노트, catch = MCP)**: Claude가 조건 B(m2 + 조용) **6개 중 4개만 보고** 범위를 적었다. MCP가 원본에서 검산할 수 없다고 보고한 것을 계기로 전수 재계산했고 **범위는 같았다** — 값은 틀리지 않았고 **근거 표본이 좁았다**. 정정 = 해석 노트 끝 「정정」 블록.
- **③ 근거 오인용 — PR #72 주석 근거 ② (출처 계층 = 세션 중 Claude 판정 → PR #72 코드 주석, catch = Claude 자기)**: 2026-09-18 #4(보드 `-11` ↔ 서버 종결)를 **「연결 종료 후에도 서버가 처리를 계속한다」의 실측 근거**로 인용했으나, 시각 대조상 **서버가 먼저 끝났을 가능성**이 크다. Claude가 스스로 잡아 **머지 전 후속 커밋 `1e7bb28`**로 문구를 정정했고 MCP가 산술을 재검증했다. **결론(발송 여부 불변)은 유지** — (m)④ · (n)① · (o)⑤ · (p)⑥ 「결론은 옳고 근거 문구가 어긋남」 계열의 연속 5회차. 상세 = **7.7(l)** 말미.
- **④ 순환 논증 — 「재진입 7건이 전부 `ndet=1` → 상수로 못 가른다」 (출처 계층 = 세션 중 Claude 판정, catch = Claude 자기)**: `NDET_MIN = 1`이면 재진입은 **구조상 `ndet=1`**에서 일어나므로 이 관측은 상수 판단의 근거가 될 수 없다. Claude가 철회하고 **ndet 분포 실측**(책 3.9% vs 사람 10.3%, p = 0.077)으로 대체했다. 상세 = **9.4(e)** 말미.
- **⑤ 외삽 과대 — 「글리치가 64ms 버퍼의 20~40%를 오염」 (출처 계층 = 세션 중 Claude 판정, catch = ④런타임 실측)**: `mic_noiseprobe`의 2초 스냅샷 클립 빈도를 **다른 펌웨어의 64ms 버퍼 단위로 외삽**한 산술이었다. 실측 = **7.9~10.2%**(조용 구간). ⇒ (l)의 「외삽」 계열. 상세 = **6.3(r)**.
- **⑥ PR 성격 오판 — PR #71을 「배관」으로 판정 (출처 계층 = ④ MCP 보고서, catch = Claude)**: 「배관」의 정의(판정 확정 후 경로 연결 — SSoT 용례 = 6.7 서문)와 맞지 않는다. 실제 성격은 **설정 위생**(`.gitignore` 1절 + 주석 경고)이다. 🔴 **PR 내용 · 대표 Type(🔧 Settings)은 무변경** — 어긋난 것은 보고서의 성격 라벨이다.
- **⑦ SSoT-코드 불일치 — 카테고리 20 ssr-nc 파급 서술 (출처 계층 = ② SSoT 카테고리 20, catch = Claude 코드 대조)**: `ssr-nc`가 워킹트리 전체의 clean을 요구해 `.claude/`가 걸릴 수 있다는 서술은 코드와 다르다 — `assertCleanSrc`의 검사 범위는 `dashboard/src`뿐이다. MCP가 `run.mjs`를 직접 재확인했고 본 Set에서 **취소선 + 정정**했다(카테고리 20 관측 절).
- **⑧ 약칭-구조 불일치 — 「8.7(c-1) 미결」 (출처 계층 = ① 인계 · 지침 계층, catch = Claude)**: (c-1)의 제목은 **「(c) 해소」**이고 미결은 그 **안의 신규 미결 bullet**(토큰 목록 이중 관리)이다. 약칭대로 읽으면 **해소 소절 전체가 미결인 것처럼** 읽힌다. **정리는 지침 쪽에서** 하고 SSoT에는 사실만 적는다 — 본 Set도 「8.7(c-1) 안의 신규 미결」로만 지칭했다.
- ⚠️ **(d)의 누적 숫자는 본 항에서도 건드리지 않았다** — (p) 말미와 같은 판정이다.
...
- 🆕 **[감시 기준점 갱신 — 발견일 = 반영일 = 2026-09-23 PoC-(56), 근거유형 = 실측 툴팁 값, 확인 시각 2026-09-23 11:06 KST(학부생 화면)]** **09-23 = 3 / 0 / 45** — **예상과 일치**(6.2 교대 재측정 세션의 pending 3건 = 2차 3건 × 15초 = 45초, `usage = success × 15` 성립). ⇒ **설명이 필요한 신호 0건.** 원본 = repo 밖 `~/ddingdong-측정결과/2026-09-23/rtt_alt/note.md`.
  - ⚠️ **근거 등급이 낮은 날**: 09-21 · 09-22 = **그래프 판독상 0 근처**(툴팁 미확인) / 09-19 · 09-20 = **미확인**.
  - ⚠️ **한계**: 09-23은 **11:06 시점의 하루 미종료 값**이다.
...
- **`Speech_synthesizer`는 판단 보류**(ⓐ 유지) — **AI Hub 화재경보 안내 음성일 가능성**이 있어 **청취 전에는 정하지 않는다**.
  - 🆕 **[보류 유지 — 사용자 결정 2026-09-23 PoC-(56)]** 판정 입력 = **`fire_alarm` 학습 클립 10개 + 후보 10개 청취**이며 **재학습 착수 전 체크리스트** 항목이다. 🔴 청취 전 확정 0.
