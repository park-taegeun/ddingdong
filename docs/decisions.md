...
**미결 항목 (발표/통합 전)**:
1. **USP 정량 근거 재정립** (발표 전): 8.42 = 클래스 간(2차 필터)로 위상 정정, 개체 구분은 직접녹음 재검증 대기(11~12주차). **발표 슬라이드의 8.42 "옆집 구분 근거" 인용 = 정정 대상.**
2. **인용 논문 위상 재검토** (발표 전): Meliza 2013 (PMC3745477) = 개체 구분 근거로 인용됐으나 개체 구분 자체가 미검증 → 논문 위상 재검토 필요(decisions-log 2025-04 SP/DTW 근거 항목 연동).
