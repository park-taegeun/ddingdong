...
1. **env:poc src_filter blacklist 회귀** → 카테고리 16 회귀 기록 + 카테고리 27.6 방향. whitelist 통일 별도 위임 + DB3 등록 예정.
2. **camera_common.cpp PID 비교 버그**: `sensorIdToName()` 및 OV3660 보정 분기가 `id.PID == 0x36`로 비교하나 실제는 `uint16_t 0x3660` → ① 진단 라벨 `(UNKNOWN)` 오표기 ② **OV3660 dark-image 보정(Khangura #6, 카테고리 25) 미실행**. **캡처 자체는 정상**(QVGA JPEG 정상 출력)이라 1차 부팅엔 무영향이나, 시연 영상 밝기 보정 위해 `0x36 → 0x3660` 수정 필요. 별도 수정 위임 + DB3 등록 예정.
