...
## 카테고리 30: 외부 계정 셋업 SSoT (2026-05-13 신설)

> Day 7 (5/13~5/14 오전 통합) AWS + 카카오 디벨로퍼스 셋업 결과 SSoT. 카테고리 22.6의 "5/9~5/14 외부 계정 셋업 전진 활용" 실제 완료 결과.
> **자격증명(액세스 키 / 시크릿 / 토큰)은 본 문서 절대 미기록** — `firmware/include/secrets.h` (.gitignore) / 환경변수로만 관리.
...
- **Naver Cloud Platform** (STT용, 카테고리 7) — 5/13 시점 미진행. 11~14주차 카테고리 7 STT 작업 진입 시 신규 row 추가 검토 (현 시점 22.6 계획의 부분 완료 상태로 기록)
