...
- ★ **`or=0xFFFFFFC0`, `tz=6`이 전 구간 불변** — 진폭 60배 변동에도 무변화. → 하위 6비트 항상 0 = **raw는 24bit를 6칸 좌시프트한 형태** 확정.
...
- ★ **shift 확정: `int16 = raw >> 14`**
  - 도출(근거유형 = **호스트 산술 검산**): `tz=6` → raw = 24bit << 6. 24bit max `8388607 << 6 >> 14 = 32767 = INT16_MAX` **정확 일치**.
...
- `DDINGDONG_CAPTURE_URL_BASE` env 값만 교체 → `image_url`이 상대경로에서 전체 https URL로 전환. **코드 0 수정**(`server/app/config.py:37`이 이미 `os.environ.get("DDINGDONG_CAPTURE_URL_BASE", "/captures")`로 env 주입식이었기 때문 — 6.1/본 카테고리 스켈레톤 설계가 이미 이 교체를 예정하고 있었음, PR #29 `c30e728`).
...
- PR #40에서 `server/app/tests/test_detect_regression.py`로 자산화(stdlib unittest, 30 케이스). 실행 = `server/`에서 `python3 -m unittest app.tests.test_detect_regression`.
