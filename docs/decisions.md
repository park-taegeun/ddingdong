...
- **체크포인트**: `ml/models/yamnet/best.keras` 저장(git 미커밋, `.gitignore`). ※ **SavedModel export는 untracked resource 버그로 미완**(11주차 배포 전 수정, 미결 33.3-③).

### 33.3 미결 항목 (발표/본학습/배포 전 결정)
...
3. **SavedModel export 버그** (11주차 배포 전): untracked resource 오류로 `best.keras` → SavedModel 변환 미완. 배포 서빙 형식 전환 시 수정 필요.
