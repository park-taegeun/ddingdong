...
//     m5 → m6  주기 캡처로 되돌린다    (히스테리시스 확인 — m2 와 같은 구성인데 값이 같은가)
//     m6 → m7  해상도 VGA             (설계 U2 해상도 판단 입력)
//   ⚠️ m6 은 m2 와 구성이 동일하다(cam=PERIODIC/wifi=off/res=QVGA). 같은 구성을 두 번 지나는 것이
//      낭비가 아니라 **되돌아왔을 때 값이 돌아오는가**(= 관측이 재현되는가)를 보는 지점이다.
#pragma once
