...
//  ② [실측 n=1 + 논증] 보드는 2차를 재시도하지 않고, 서버는 보드 연결이 끊겨도 처리를
//     끝낸다(9/18 보드 -11 / 15,474ms ↔ 서버 종결, 6.7(d)(e)#4). ⇒ 이 값은 카카오 발송
//     여부를 바꾸지 않는다. 바뀌는 것은 「보드가 결과를 아는가」와 「대기 중 다음 트리거를
//     받지 못하는 시간」뿐이다. ⚠️ Flask 개발 서버 기준 실측이다.
