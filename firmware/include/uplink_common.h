...
// === 2차 HTTP 타임아웃 (§2-2(a) — **확정 2026-09-23, 사용자 결정 — 값 변경 없음**) ===
...
// ⚠️ 12.1초는 7.7(l)이 **과소평가**로 판정했다(네트워크 실패 모집단 약 19.5~20초)
//    — 이 값의 근거로 읽지 말 것. 확정 근거는 아래 ①~③이다.
//
...
//    (6.5 실측 + 논증).
// [해소 2026-09-23] 🔴 **PR-C 재판정 트리거**: ④런타임에서 2차 왕복 실측치가 나오면 이 값을 재판정한다.
//    12.1초는 **미실측 산술 상한**이며 성분별 실측 근거는 SSoT에서 추적 불가다(7.7(l)).
//
// 확정 근거 (2026-09-23, PR-C):
//  ① [실측] 정상 종결 2차 rtt n=5 = 6,710 / 10,014ms(9/18, 6.7(d)) + 4,284 / 3,131 /
//     7,790ms(9/23) → 범위 3,131~10,014ms. 15,000은 최대치의 약 1.5배다.
//  ② [논증 + 실측 1건] 카카오 발송·STT는 서버 핸들러 안에서 응답 전에 끝나고(7.7(l)), 보드는
//     2차를 재시도하지 않는다 ⇒ 이 값은 카카오 발송 여부를 바꾸지 않는다. 바뀌는 것은 「보드가
//     결과를 아는가」와 「대기 중 다음 트리거를 받지 못하는 시간」뿐이다.
//     실측 1건 = 9/18 보드 -11 / 15,474ms ↔ 서버 종결(6.7(d)(e)#4) — 두 결과가 갈릴 수 있음을 보인다.
//     ⚠️ 서버가 보드 연결 종료 「후에도」 처리를 계속하는지는 미실측이다(Flask 개발 서버, 프레임워크 동작 논증).
//  ③ [실측] 15초를 넘긴 유일한 실측은 네트워크 도달 실패(URLError) 모집단이다.
// 🔴 재판정 트리거 (확정 후):
//  (1) M5-c 판정 PR에서 2차 대기 중 트리거 누락이 관측될 때
//  (2) 보드 2차 재시도를 추가할 때 (재전송은 409에 막힌다 — 7.6(i)와 함께 판단)
//  (3) EC2(Nginx → Gunicorn) 이관 시 ②를 1회 재확인하고, 다르면 재판정
