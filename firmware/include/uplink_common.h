...
//   - ToF 메타 4필드: 전송하지 않는다(서버 fail-open 경로 사용, mic_uplink_main.cpp 주석).
...
// head(필드 3개 boilerplate) + tail 여유폭. 원본 MULTIPART_OVERHEAD_BYTES(640) 동일 근거.
constexpr size_t UPLINK_MULTIPART_OVERHEAD_BYTES = 640;
